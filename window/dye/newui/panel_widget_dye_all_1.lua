function PaGlobal_Dye_All:initialize()
  ...
  for idx = 0, self._maxEquipSlotCount - 1 do
    ...
    slot:createChild()
    if true == isGameTypeKR2() and (idx == self._enumToggleType.Underwear or idx == self._enumToggleType.AwakenWeapon) then
      removeIdxCount = removeIdxCount + 1
      slotBG:SetShow(false)
      slot.icon:SetShow(false)
    end
    slotBG:SetPosX(slotStartX + (idx - removeIdxCount) % maxColumn * slotGap)
    slotBG:SetPosY(slotStartY + math.floor((idx - removeIdxCount) / maxColumn) * slotGap)
...
function PaGlobal_Dye_All:registEventHandler()
  ...
  if self._isConsole then
    ...
    if nil ~= HandleEventRSDown_Dye_CharacterController_PressedRS or nil ~= HandleEventRSUp_Dye_CharacterController_ReleasedRS then
      ...
      _PA_ASSERT_NAME(false, "Panel_Widget_Dye_CharacterController_All_2 ì´ ë¡œë“œë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤.", "í”„ë¡œê·¸ë¨ 1íŒ€ / ê¹€í˜„ë¹ˆ")
    end
  ...
  for idx = 0, self._maxToggleCount - 1 do
    ...
    if true == isGameTypeKR2() and (idx == self._enumToggleType.Underwear or idx == self._enumToggleType.AwakenWeapon) then
      self._chk_ShowToggle[idx]:SetShow(false)
    end
  end
...
function PaGlobal_Dye_All:openDataSetting()
  ...
  if false == isGameTypeKR2() then
    ...
    self._rdo_AmpleCategory[0]:SetShow(false)
    self._rdo_AmpleCategory[1]:SetShow(false)
    self._rdo_AmpleCategory[2]:SetHorizonCenter()
    self._rdo_AmpleCategory[2]:SetSpanSize(0, 0)
    self._rdo_AmpleCategory[2]:SetCheck(true)
