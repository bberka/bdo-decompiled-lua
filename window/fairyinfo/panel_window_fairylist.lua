...
local FairyHpRateText = {
  [0] = "ì²´ë ¥ íšŒë³µì œë¥¼ ë“±ë¡í•´ì£¼ì„¸ìš”.",
  [10] = "ì²´ë ¥ 10% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©",
  [20] = "ì²´ë ¥ 20% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©",
  [30] = "ì²´ë ¥ 30% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©",
  [50] = "ì²´ë ¥ 50% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©"
}
local fairySubGageRateText = {
  [1] = {
    [0] = "ì •ì‹ ë ¥ íšŒë³µì œë¥¼ ë“±ë¡í•´ì£¼ì„¸ìš”.",
    [10] = "ì •ì‹ ë ¥ 10% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©",
    [20] = "ì •ì‹ ë ¥ 20% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©",
    [30] = "ì •ì‹ ë ¥ 30% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©",
    [50] = "ì •ì‹ ë ¥ 50% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©"
  },
  [2] = {
    [0] = "íˆ¬ì§€ íšŒë³µì œë¥¼ ë“±ë¡í•´ì£¼ì„¸ìš”.",
    [10] = "íˆ¬ì§€ 10% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©",
    [20] = "íˆ¬ì§€ 20% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©",
    [30] = "íˆ¬ì§€ 30% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©",
    [50] = "íˆ¬ì§€ 50% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©"
  },
  [3] = {
    [0] = "ì‹ ì„±ë ¥ íšŒë³µì œë¥¼ ë“±ë¡í•´ì£¼ì„¸ìš”.",
    [10] = "ì‹ ì„±ë ¥ 10% ì´í•˜ì¼ ë•Œ ìë™ì‚¬ìš©",
...
function PaGlobal_FairyList_Question()
  _PA_LOG("ì§€ë¯¼í˜", "ìš”ì • ë„ì›€ë§ì •ë³´ë¥¼ ìš”ì²­í•©ë‹ˆë‹¤. í•¨ìˆ˜ ë‚´ë¶€êµ¬í˜„í•´ì£¼ì„¸ìš”...")
end
...
function FromClient_ShowFairyMessageByType(msgType)
  ...
  if FairyMessageType.eTurnOnLantern == msgType then
    Proc_ShowMessage_Ack("ìš”ì •ì´ ì£¼ë³€ì„ ë°í˜€ì¤ë‹ˆë‹¤.")
  elseif FairyMessageType.eTurnOffLantern == msgType then
    Proc_ShowMessage_Ack("ìš”ì •ì˜ ë°ì€ ë¹›ì´ ì‚¬ë¼ì§‘ë‹ˆë‹¤.")
  else
    _PA_LOG("ì§€ë¯¼í˜", "ì •ì˜ë˜ì§€ ì•Šì€ ê°’ì´ ë“¤ì–´ì™”ìŠµë‹ˆë‹¤. msgType = " .. tostring(msgType) .. " , GameEnginePetManager.h ì— enum ê°’ í™•ì¸í•´ì£¼ì„¸ìš”.")
  end
end
