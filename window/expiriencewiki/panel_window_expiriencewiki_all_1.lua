...
function PaGlobal_Expirience_All:SetUrlTotalAccount(token)
  ...
  if nil == self._directLinkUrl and true == self._isNoticeTapOpen then
    url = url .. "/NewsList"
    url = url .. "" .. tostring(token) .. "&_userNo=" .. tostring(userNo) .. "&isGm=" .. tostring(isGm)
...
function PaGlobal_Expirience_All:SetUrl()
  ...
  if nil ~= self._directLinkUrl then
    ...
    url = url .. "" .. tostring(userNo) .. "&userNickName=" .. tostring(userNickName) .. "&langType=" .. tostring(langType) .. "&isGm=" .. tostring(isGm) .. "&certKey=" .. tostring(cryptKey) .. "&characterNo=" .. tostring(characterNo) .. "&utm_source=link&utm_medium=menu&utm_campaign=note"
  end
  self._isEventTabOpen = false
  self._directLinkUrl = nil
  self._ui.stc_webControl:SetUrl(self._ui.stc_webControl:GetSizeX(), self._ui.stc_webControl:GetSizeY(), url, false, true)
