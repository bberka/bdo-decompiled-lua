...
function PaGlobal_YachtDice_Invite:initializeMainBoardArea()
  ...
  if nil == listManager then
    UI.ASSERT_NAME(false, "ì•¼ë¥´ ì¡±ë³´ ì‹ ì²­ UIì˜ UIList2 Managerê°€ nullptrì…ë‹ˆë‹¤. ë°œìƒí•˜ë©´ ì•ˆë©ë‹ˆë‹¤. ì œë³´ ë¶€íƒë“œë¦½ë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
...
function PaGlobal_YachtDice_Invite:prepareOpen()
  if nil == Panel_Window_MiniGame_YachtDice_PVP_Invite then
    return
  end
  self:resizeBg()
