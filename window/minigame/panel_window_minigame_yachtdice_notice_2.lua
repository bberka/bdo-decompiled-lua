...
function PaGlobalFunc_MiniGameYacht_Notice_Open(requestUserNickName, expireTime, gameBoardKey, useKeepData)
  ...
  if true == useKeepData then
    PaGlobal_YachtDice_Notice:prepareOpen(PaGlobal_YachtDice_Notice._keepOpenInfo._requestUserNickName, PaGlobal_YachtDice_Notice._keepOpenInfo._expireTime, PaGlobal_YachtDice_Notice._keepOpenInfo._gameBoardKey)
  else
    PaGlobal_YachtDice_Notice:prepareOpen(requestUserNickName, expireTime, gameBoardKey)
  end
end
function PaGlobalFunc_MiniGameYacht_Notice_KeepOpenInfo(requestUserNickName, expireTime, gameBoardKey)
  if nil == PaGlobal_YachtDice_Notice then
    return
  end
  PaGlobal_YachtDice_Notice:keepOpenInfo(requestUserNickName, expireTime, gameBoardKey)
end
