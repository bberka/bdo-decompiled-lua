...
function PaGlobal_MiniGame_YachtDice:prepareOpen(isOpenByNpc)
  if nil == isOpenByNpc then
    UI.ASSERT_NAME(false, "í˜¸ì¶œê²½ë¡œê°€ ë¹„ì •ìƒì…ë‹ˆë‹¤! ì•¼ë¥´ë¥¼ í‚¬ ìˆ˜ ì—†ë‹¤.", "ì´ì£¼")
  end
...
function PaGlobal_MiniGame_YachtDice:initTurnBG(name, playerType)
  ...
  function turnBG:SetBubbleText(text)
    UI.ASSERT_NAME(nil ~= text, "bubbleTextê°€ nilì…ë‹ˆë‹¤.", "ì´ì£¼")
...
function PaGlobal_MiniGame_YachtDice:initDiceCardDestInfo()
  for ii = 0, self.__eYachtPlayerType_Count - 1 do
    for jj = 0, __eYachtDiceCount - 1 do
      ...
      _PA_ASSERT(nil ~= fieldCard, "fieldCard nil ì…ë‹ˆë‹¤.")
      for kk = 0, __eTYachtEyeCount - 1 do
        local diceCard = self._ui._diceCardList[ii][jj][kk]
        _PA_ASSERT(nil ~= diceCard, "diceCardê°€ nil ì…ë‹ˆë‹¤.")
        ...
        _PA_ASSERT(diceCard.initSize.x < diceCard.destSize.x, "destSizeê°€ initSizeë³´ë‹¤ ì‘ìŠµë‹ˆë‹¤.")
        _PA_ASSERT(diceCard.initSize.y < diceCard.destSize.y, "destSizeê°€ initSizeë³´ë‹¤ ì‘ìŠµë‹ˆë‹¤.")
      end
    end
  end
end
...
function PaGlobal_MiniGame_YachtDice:startNpcAction()
  ...
  elseif __eNpcYachtActionEventType_Roll == eventType then
    ...
    for ii = beginIndex, endIndex - 1 do
      ...
      _PA_ASSERT(tempEventType == __eNpcYachtActionEventType_RollResultHandRank, "tempEventType ì´ ë¹„ì •ìƒì´ë‹¤.")
  ...
  elseif __eNpcYachtActionEventType_UnKeep == eventType then
    ...
    _PA_ASSERT(false, "eventTypeì´ ë¹„ì •ìƒ ì…ë‹ˆë‹¤.")
  end
end
