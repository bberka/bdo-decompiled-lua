...
function PaGlobal_YachtDice_Notice:keepOpenInfo(requestUserNickName, expireTime, gameBoardKey)
  if nil == Panel_Widget_MiniGame_YachtDice_Notice then
    return
  end
...
function PaGlobal_YachtDice_Notice:prepareOpen(requestUserNickName, expireTime, gameBoardKey)
  if nil == Panel_Widget_MiniGame_YachtDice_Notice then
    return
  end
  ...
  if self._remainExpireTime < 0 then
    UI.ASSERT_NAME(false, "ë§Œë£Œì‹œê°„ì´ ë¹„ì •ìƒì…ë‹ˆë‹¤.", "ì´ì£¼")
  end
