...
function PaGlobal_ImperialSupplyMSG:item1(horseTier)
  local item1 = getItemEnchantStaticStatus(ItemEnchantKey(1020))
  if nil ~= item1 then
    self._Slot1:setItemByStaticStatus(item1, horseTier)
    if true == _ContentsGroup_UsePadSnapping then
      ...
      self._Slot1.icon:addInputEvent("Mouse_On", "PaGlobal_ImperialSupplyMSG:showItemTooltip(true, 1)")
...
function PaGlobal_ImperialSupplyMSG:showItemTooltip(isShow, index)
