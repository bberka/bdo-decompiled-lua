...
function PaGlobal_UseItem_All_PopupItem(itemEnchatWrapper, whereType, slotNo, equipSlotNo)
  ...
  if ToClient_isTotalGameClient() == true then
    PaGlobal_UseItem_All:deferredPopupItemShowRequest(itemEnchatWrapper, whereType, slotNo, equipSlotNo)
    return
  end
...
function FromClient_UseItem_All_PopupItemDescDefferedOpen(itemKey)
  if nil == Panel_UseItem_All then
    return
  end
  PaGlobal_UseItem_All:deferredPopupItemOpen(itemKey)
end
