...
function PaGlobal_ImageSlider_All:updateImageSlider(deltaTime)
  ...
  if 0 < self._audioCount then
    ...
    for i = self._curAudioEndIndex, self._audioCount - 1 do
      local data = ToClient_GetImageSlideAudioDataInEndList(i)
      if data == nil then
        break
      elseif data:getEndTime() <= self._curUpdateTime then
        ToClient_StopImageSlideAudio(i)
        if i >= self._audioCount - 1 then
          self._curAudioEndIndex = self._audioCount
        end
      else
        self._curAudioEndIndex = i
        break
      end
    end
  end
  if self._ui._txt_currTimeForDev:GetShow() == true then
    ...
    self._ui._txt_currImagePercent:SetText(string.format("í˜„ì¬ ì´ë¯¸ì§€ ì§„í–‰ë¥  : %d%%", percent * 100))
  end
end
