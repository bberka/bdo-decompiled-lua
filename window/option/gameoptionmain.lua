...
function PaGlobal_Option:CreateEventControl(category, detail)
  ...
  function CreateEventControlXXX(parent, control, controlTypeName, elementName)
    ...
    if nil == control then
      _PA_LOG("í›„ì§„", "Controlì´ ì œëŒ€ë¡œ ì„¤ì •ì´ ë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤. Xmlì„ í™•ì¸í•´ì£¼ì„¸ìš”. // Element Name : " .. controlName)
      return
    end
...
function PaGlobal_Option:EventComboBoxOpenXXX(elementName, controlIndex)
  ...
  if option._controlType ~= __ePAUIControl_ComboBox then
    _PA_LOG("í›„ì§„", "[EventComboBoxOpenXXX][ RETURN ] comboboxê°€ ì•„ë‹ˆë©´ ì—¬ê¸° ë“¤ì–´ì˜¤ë©´ ì•ˆë©ë‹ˆë‹¤. controlTypeì„ í™•ì¸í•´ì£¼ì„¸ìš”. : " .. elementName)
    return
  end
  if option._comboBoxListCount == nil then
    _PA_LOG("í›„ì§„", "[EventComboBoxOpenXXX][ RETURN ] _comboBoxListCount ê°€ ì…‹íŒ… ë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤. SpectialControlComboBoxInitValue() ì—ì„œ ì²˜ë¦¬í•´ì£¼ì„¸ìš”.: " .. elementName)
    return
  end
  if option._comboBoxList == nil then
    _PA_LOG("í›„ì§„", "[EventComboBoxOpenXXX][ RETURN ] _comboBoxList table ì´ ì…‹íŒ… ë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤. SpectialControlComboBoxInitValue() ì—ì„œ ì²˜ë¦¬í•´ì£¼ì„¸ìš”. : " .. elementName)
    return
  end
...
function PaGlobal_Option:EventXXX(controlName, controlIndex, order, param)
  ...
  if nil == strsplit[1] or nil == strsplit[2] then
    _PA_LOG("í›„ì§„", "[EventXXX][ RETURN ] controlì˜ ë²„íŠ¼ ì´ë²¤íŠ¸ê°€ ì˜ëª»ë˜ì—ˆìŠµë‹ˆë‹¤. ControlNameì„ í™•ì¸í•´ì£¼ì„¸ìš”. : " .. elementName)
    return
  end
  ...
  if "table" ~= type(option) then
    _PA_LOG("í›„ì§„", "[EventXXX][ RETURN ] element ê°€ í…Œì´ë¸”ì´ ì•„ë‹™ë‹ˆë‹¤. Header ë¥¼ í™•ì¸í•´ì£¼ì„¸ìš”.  : " .. elementName)
    return
  end
  ...
  elseif __ePAUIControl_Button == controlType then
    ...
    if nil == option.GetButtonListSize then
      _PA_LOG("í›„ì§„", "[SetControlSettingTable][ BUTTON ] GetButtonListSize í•¨ìˆ˜ê°€ ë§Œë“¤ì–´ì§€ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤. GameOptionHeaderì— ì¶”ê°€í•´ì£¼ì„¸ìš”. " .. elementName)
    elseif 0 == order then
...
function PaGlobal_Option:AntiAliasingContentSet()
  if nil == self._elements.AntiAliasing._eventControl2 then
    _PA_LOG("í›„ì§„", "[SetControlSettingTable][ RETURN ] ë¼ë””ì˜¤ ë²„íŠ¼ ì»¨íŠ¸ë¡¤ (eventcontrol 1~x) ì´ ì •ìƒì ìœ¼ë¡œ ë§Œë“¤ì–´ì§€ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤/ xml ë¼ë””ì˜¤ ë²„íŠ¼ ì…‹íŒ…ì„ í™•ì¸í•´ì£¼ì„¸ìš” : " .. elementName)
    return
  end
  ...
  elseif __ePAUIControl_ComboBox == option._controlType then
    ...
    if __ePAUIControl_Button == option._controlType then
      if nil == option.GetButtonText then
        _PA_LOG("í›„ì§„", "[SetControlSettingTable][ BUTTON ] GetButtonText í•¨ìˆ˜ê°€ ë§Œë“¤ì–´ì§€ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤. GameOptionHeaderì— ì¶”ê°€í•´ì£¼ì„¸ìš”. " .. elementName)
      else
...
function PaGlobal_Option:SearchOption(inputString)
  ...
  for index, option in pairs(self._elements) do
    ...
    if nil == option._string then
      _PA_LOG("í›„ì§„", "[SearchOption][ RETURN ] option ì— stringì´ ì„¤ì •ë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤. GameOptionHeaderì— ì„¤ì •ì´ ë˜ì–´ìˆëŠ”ì§€ í™•ì¸í•´ì£¼ì„¸ìš”. ")
    else
      rv = string.find(string.lower(option._string), string.lower(inputString))
    end
    if nil == option._category or nil == option._detail then
      _PA_LOG("í›„ì§„", "[SearchOption][ RETURN ] option ì— ì¹´í…Œê³ ë¦¬ê°€ ì„¤ì •ë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤. CreateEventControl ì—ì„œ ì˜ëª»ë˜ì—ˆìŠµë‹ˆë‹¤." .. index)
      rv = nil
    end
    if nil ~= rv then
...
function PaGlobal_Option:GetKeyCustomInputType()
  if nil == self._keyCustomInputType then
    _PA_LOG("í›„ì§„", "[GetKeyCustomInputType][ RETURN ] ì´ìƒí•˜ë‹¤....")
    return
  end
...
function PaGlobal_Option:CompleteKeyCustomMode()
  if nil == self._keyCustomInputType then
    _PA_LOG("í›„ì§„", "[CompleteKeyCustomMode][ RETURN ] ê¸°ì¡´ Option ì°½ ì•„ë‹ˆë¼ë©´ ì´ìƒí•˜ë‹¤")
    return
  end
  local elementName = self._keyCustomInputType[1] or nil
  if nil == elementName then
    _PA_LOG("í›„ì§„", "[CompleteKeyCustomMode][ RETURN ] ì´ìƒí•˜ë‹¤")
    return
  end
...
function PaGlobal_Option:InitSetting()
  for name, option in pairs(self._elements) do
    if type(option) ~= "table" then
      _PA_LOG("í›„ì§„", "[ InitSetting ] option ì´ tableì´ ì•„ë‹™ë‹ˆë‹¤.")
    elseif option._initValue == nil then
      _PA_LOG("í›„ì§„", "element init valueê°€ Null ì…ë‹ˆë‹¤. " .. name)
    else
