...
function PaGlobal_GameOption_All:ClickedResetFrame()
  if nil == self._list2._curCategory or nil == self._list2._curDetail then
    _PA_LOG("í›„ì§„", "[GameOption][ClickedResetFrame] í˜„ì¬ ì„ íƒëœ í”„ë ˆì„ì´ ì—†ìŠµë‹ˆë‹¤. ì–´ë””ì„œ í•¨ìˆ˜ë¥¼ ë¶ˆë €ëŠ”ì§€ í™•ì¸í•´ì£¼ì„¸ìš”.")
    return
  end
  ClearFocusEdit()
...
function PaGlobal_GameOption_All:ClickedFindElement(category, detail, searchElement)
  ...
  if nil == category or nil == detail then
    _PA_LOG("í›„ì§„", "[GameOption][ClickedFindElement] ì˜µì…˜ ì„¸íŒ…ì´ ì˜ëª»ë˜ì—ˆìŠµë‹ˆë‹¤. CreateEventControlì„ ë³´ì„¸ìš”.  category : " .. category .. " detail : " .. detail)
  end
