...
function HandleEventLUp_HorseRacingEnter_ShowRankList(isShow)
  ...
  if -1 == PaGlobal_HorseRacing_Enter._selectedMode then
    local url = bAdventureWebUrl .. "/HorseRacingRank/Index" .. tostring(UserNo) .. "&certKey=" .. tostring(cryptKey)
    ...
    local url = bAdventureWebUrl .. "/HorseRacingRank/Index" .. tostring(UserNo) .. "&certKey=" .. tostring(cryptKey) .. "&mapKey=" .. tostring(mapKey)
    PaGlobal_HorseRacing_Enter._ui.web_rankList:SetUrl(PaGlobal_HorseRacing_Enter._ui.web_rankList:GetSizeX(), PaGlobal_HorseRacing_Enter._ui.web_rankList:GetSizeY(), url)
  end
  Panel_HorseRacing_Enter:ComputePos()
end
...
function FromClient_HorseRacing_MatchingSuccess()
  ...
  local messageBoxData = {
    ...
    content = PAGetString(Defines.StringSheet_GAME, "LUA_HORSERACING_ENTER_LOADING"),
    priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
  }
  MessageBox.showMessageBox(messageBoxData)
end
