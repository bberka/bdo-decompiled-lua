...
function PaGlobal_Solrare_MatchNotice:initialize()
  ...
  Panel_Widget_Solare_TierUpgrade:ComputePos()
    ...
    if nil ~= userInfo then
      ...
      if nil ~= self._ui._stc_TeamControl[controlIndex] then
        ...
        if nil ~= memberControlList[teamControlIndex] then
          ...
          if nil ~= PaGlobalFunc_Solrare_Ranking_SetTierIconByScore and solareMatchMode ~= __eSolareMode_Normal then
          ...
          if nil ~= PaGlobalFunc_Solrare_MatchNotice_SetClassBg then
            PaGlobalFunc_Solrare_MatchNotice_SetClassBg(memberControlList[teamControlIndex]._stc_Class, classTypeRaw)
            if 1 == teamNo then
...
function PaGlobal_Solrare_MatchNotice:updateTeamUI_Round(solareMatchMode, redMVPUserNo, blueMVPUserNo)
  ...
  elseif __eInstanceSolareManagerState_Result == roomState then
        ...
        if nil ~= userInfo then
          ...
          if myTeamNo == teamNo then
            ...
            if nil ~= memberControlList[controlIndex] then
              ...
              if nil ~= PaGlobalFunc_Solrare_MatchNotice_SetClassBg then
                PaGlobalFunc_Solrare_MatchNotice_SetClassBg(memberControlList[controlIndex]._stc_Class, classTypeRaw)
              end
            end
          end
        end
    ...
    if solareMatchMode == __eSolareMode_Normal then
      self._ui._stc_RewardBg:SetShow(false)
    end
...
function FromClient_SolareVaryRankRating(prevRating, currentRating)
  ...
  if beforeTier < currentTier then
    PaGlobalFunc_Solrare_MatchNotice_OpenTierUp(currentRating)
  end
end
...
function PaGlobalFunc_Solrare_MatchNotice_OpenTierUp(myRankRating)
  PaGlobal_Solrare_MatchNotice:prepareOpenTierUp(myRankRating)
end
function PaGlobal_Solrare_MatchNotice:prepareOpenTierUp(myRankRating)
  if nil == Panel_Widget_Solare_TierUpgrade then
    return
  end
