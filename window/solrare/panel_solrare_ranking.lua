  ...
  for idx = 0, self._currentRecordCount - 1 do
    ...
    if nil ~= recordInfo then
      ...
      if nil ~= self._recordControl[controlIdx] then
        ...
        PaGlobalFunc_Solrare_Ranking_SetClassBigBg(control.stc_classBg, tempTable._classTypeRaw)
        ...
        for index = 1, 3 do
          if control.mostDetail[index] ~= nil then
            if tempTable._topClassType[index - 1] == __eClassType_Count then
              ...
              PaGlobalFunc_Solrare_Ranking_SetClassBigBg(control.mostDetail[index].stc_classBg, tempTable._topClassType[index - 1])
...
function PaGlobalFunc_Solrare_Ranking_SetClassBigBg(control, classType)
  if nil == control then
    return
  end
  control:ChangeTextureInfoNameDefault("Combine/Etc/Combine_Etc_Solare_War_01.dds")
