function ToClient_SelfPlayerLevelUp_StatusDataSet(hp, mp, stun, weight, lvdd, lvpv)
  ...
  if nil == hp then
    UI.ASSERT_NAME(nil ~= hp, "ToClient_SelfPlayerLevelUp_StatusDataSet hp nil", "ìš°ì •ë¬´")
  end
  if nil == mp then
    UI.ASSERT_NAME(nil ~= mp, "ToClient_SelfPlayerLevelUp_StatusDataSet mp nil", "ìš°ì •ë¬´")
  end
  if nil == weight then
    UI.ASSERT_NAME(nil ~= weight, "ToClient_SelfPlayerLevelUp_StatusDataSet weight nil", "ìš°ì •ë¬´")
  end
  if nil == lvdd then
    UI.ASSERT_NAME(nil ~= lvdd, "ToClient_SelfPlayerLevelUp_StatusDataSet lvdd nil", "ìš°ì •ë¬´")
    return
  end
  if nil == lvpv then
    UI.ASSERT_NAME(nil ~= lvpv, "ToClient_SelfPlayerLevelUp_StatusDataSet lvpv nil", "ìš°ì •ë¬´")
    return
  end
...
function FromClient_SelfPlayerCurrentLevelInfo(learnableSkillCount, learnSkillCount, hp, mp, stun, weight, lvdd, lvpv)
  ...
  if nil == learnSkillCount then
    UI.ASSERT_NAME(nil ~= learnSkillCount, "FromClient_SelfPlayerCurrentLevelInfo learnSkillCount nil", "ìš°ì •ë¬´")
    return
  end
  if nil == learnableSkillCount then
    UI.ASSERT_NAME(nil ~= learnableSkillCount, "FromClient_SelfPlayerCurrentLevelInfo learnableSkillCount nil", "ìš°ì •ë¬´")
    return
  end
  if nil == hp then
    UI.ASSERT_NAME(nil ~= hp, "FromClient_SelfPlayerCurrentLevelInfo hp nil", "ìš°ì •ë¬´")
    return
  end
  if nil == mp then
    UI.ASSERT_NAME(nil ~= mp, "FromClient_SelfPlayerCurrentLevelInfo mp nil", "ìš°ì •ë¬´")
    return
  end
  if nil == weight then
    UI.ASSERT_NAME(nil ~= weight, "FromClient_SelfPlayerCurrentLevelInfo weight nil", "ìš°ì •ë¬´")
    return
  end
  if nil == lvdd then
    UI.ASSERT_NAME(nil ~= lvdd, "FromClient_SelfPlayerCurrentLevelInfo lvdd nil", "ìš°ì •ë¬´")
    return
  end
  if nil == lvpv then
    UI.ASSERT_NAME(nil ~= lvpv, "FromClient_SelfPlayerCurrentLevelInfo lvpv nil", "ìš°ì •ë¬´")
    return
  end
...
function PaGlobal_LevelupInfo_LearnSkillDataSet(skillNo, index)
  ...
  if nil == skillNo then
    UI_ASSERT_NAME(nil ~= skillNo, "skillNoê°€ nilì´ë©´ ì•ˆë†1", "ì •íƒœê³¤")
    return
  end
  PaGlobal_LevelupInfo._ui._txt_learnSkillNoSkill:SetShow(false)
...
function PaGlobal_LevelupInfo_LearnableSkillDataSet(skillNo, index)
  ...
  if nil == skillNo then
    UI_ASSERT_NAME(nil ~= skillNo, "skillNoê°€ nilì´ë©´ ì•ˆë†2", "ì •íƒœê³¤")
    return
  end
  if nil == index then
    UI_ASSERT_NAME(nil ~= index, "indexê°€ nilì´ë©´ ì•ˆë†2", "ì •íƒœê³¤")
    return
  end
  PaGlobal_LevelupInfo._ui._txt_learnableSkillNoSkill:SetShow(false)
  local posIndex = index - 1
