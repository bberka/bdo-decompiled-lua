...
function HandleEventLUp_DailyStamp_All_TabClicked(tabIndex)
  if nil == tabIndex then
    UI.ASSERT_NAME(nil ~= tabIndex, "HandleEventLUp_DailyStamp_All_TabClicked tabIndex nil", "ì •ì§€í˜œ")
    return
  end
...
function HandleEventUp_DailyStamp_All_AcceptReward(rewardIndex)
  ...
  if nil == rewardIndex then
    UI.ASSERT_NAME(nil ~= rewardIndex, "HandleEventUp_DailyStamp_All_AcceptReward rewardIndex nil", "ì •ì§€í˜œ")
    return
  end
...
function HandleEventUp_DailyStamp_All_SetAttendance(rewardIndex)
  if nil == rewardIndex then
    UI.ASSERT_NAME(nil ~= rewardIndex, "HandleEventUp_DailyStamp_All_SetAttendance rewardIndex nil", "ì •ì§€í˜œ")
    return
  end
...
function FromClient_DailyStamp_All_AttendanceUpdate(attendanceKey)
  ...
  if nil == attendanceKey then
    UI.ASSERT_NAME(nil ~= attendanceKey, "FromClient_DailyStamp_All_AttendanceUpdate attendanceKey nil", "ì •ì§€í˜œ")
    return
  end
...
function FromClient_DailyStamp_All_AttendanceUpdateAll(isNextDay)
  ...
  if nil == isNextDay then
    UI.ASSERT_NAME(nil ~= isNextDay, "FromClient_DailyStamp_All_AttendanceUpdateAll isNextDay nil", "ì •ì§€í˜œ")
    return
  end
...
function FromClient_DailyStamp_All_receiveAttendanceReward(attendanceKey)
  ...
  if nil == attendanceKey then
    UI.ASSERT_NAME(nil ~= attendanceKey, "FromClient_DailyStamp_All_receiveAttendanceReward attendanceKey nil", "ì •ì§€í˜œ")
    return
  end
  for index = 1, PaGlobal_DailyStamp_All._dailyStampCount do
    local dailyKey = PaGlobal_DailyStamp_All._dailyStampKeys[index][1]:getKey()
...
function FromClient_DailyStamp_All_regionCheck(regionData)
  ...
  if nil == regionData then
    UI.ASSERT_NAME(nil ~= regionData, "FromClient_DailyStamp_All_regionCheck regionData nil", "ì •ì§€í˜œ")
    return
  end
  if true == _ContentsGroup_RenewUI and GetUIMode() ~= Defines.UIMode.eUIMode_Default then
    return
