...
function PaGlobal_ColorPalette_All:loadColorPresetRadioButtonList()
  ...
  if 14 ~= __eColorPalettePresetCount then
    _PA_ASSERT_NAME(false, "í”„ë¦¬ì…‹ ê°œìˆ˜ ë³€ê²½ë˜ì—ˆìŠµë‹ˆë‹¤. __eColorPalettePresetCount ê´€ë ¨ëœ ê³³ í™•ì¸í•´ì•¼í•©ë‹ˆë‹¤.", "ê¹€í˜„ë¹ˆ")
  end
...
function PaGlobal_ColorPalette_All:panelPressed(panelType)
  ...
  if 0 == panelSizeX or 0 == panelSizeY then
    _PA_ASSERT_NAME(false, "í´ë¦­í•œ panelì˜ ì‚¬ì´ì¦ˆê°€ 0ì…ë‹ˆë‹¤.", "ê¹€í˜„ë¹ˆ")
    return
  end
...
function PaGlobal_ColorPalette_All:hslToRgb(hue, saturation, lightness)
  _PA_ASSERT_NAME(hue >= 0 and hue <= __eRgbColorLimitationHSLRange, "hue " .. hue .. " ì˜ ë²”ìœ„ëŠ” [0, " .. __eRgbColorLimitationHSLRange .. "] ì´ì—¬ì•¼í•©ë‹ˆë‹¤.", "ê¹€í˜„ë¹ˆ")
  _PA_ASSERT_NAME(saturation >= 0 and saturation <= __eRgbColorLimitationHSLRange, "saturation" .. saturation .. "  ì˜ ë²”ìœ„ëŠ” [0, " .. __eRgbColorLimitationHSLRange .. "] ì´ì—¬ì•¼í•©ë‹ˆë‹¤.", "ê¹€í˜„ë¹ˆ")
  _PA_ASSERT_NAME(lightness >= 0 and lightness <= __eRgbColorLimitationHSLRange, "lightness" .. lightness .. "  ì˜ ë²”ìœ„ëŠ” [0, " .. __eRgbColorLimitationHSLRange .. "] ì´ì—¬ì•¼í•©ë‹ˆë‹¤.", "ê¹€í˜„ë¹ˆ")
  ...
  if nil == red then
    _PA_ASSERT_NAME(false, "í´ë¼ë¡œë¶€í„° ì˜¨ hsl ì •ë³´ê°€ ì˜¬ë°”ë¥´ì§€ ì•ŠìŠµë‹ˆë‹¤. ì¼ë‹¨ 0, 0, 0ì„ ë°˜í™˜í•©ë‹ˆë‹¤.", "í”„ë¡œê·¸ë¨ 1íŒ€ / ê¹€í˜„ë¹ˆ")
    return 0, 0, 0
  end
...
function PaGlobal_ColorPalette_All:rgbToHsl(red, green, blue)
  _PA_ASSERT_NAME(red >= 0 and red <= 255, "red " .. red .. "ì˜ ë²”ìœ„ëŠ” [0, 255] ì´ì—¬ì•¼í•©ë‹ˆë‹¤.", "ê¹€í˜„ë¹ˆ")
  _PA_ASSERT_NAME(green >= 0 and green <= 255, "green " .. green .. "ì˜ ë²”ìœ„ëŠ” [0, 255] ì´ì—¬ì•¼í•©ë‹ˆë‹¤.", "ê¹€í˜„ë¹ˆ")
  _PA_ASSERT_NAME(blue >= 0 and blue <= 255, "blue " .. blue .. "ì˜ ë²”ìœ„ëŠ” [0, 255] ì´ì—¬ì•¼í•©ë‹ˆë‹¤.", "ê¹€í˜„ë¹ˆ")
  ...
  if nil == hue then
    _PA_ASSERT_NAME(false, "í´ë¼ë¡œë¶€í„° ì˜¨ hsl ì •ë³´ê°€ ì˜¬ë°”ë¥´ì§€ ì•ŠìŠµë‹ˆë‹¤.", "í”„ë¡œê·¸ë¨ 1íŒ€ / ê¹€í˜„ë¹ˆ")
    return
  end
...
function PaGlobal_ColorPalette_All:saveColorPreset()
  ...
  if false == ToClient_setColorPresetInformation(self._targetPresetSlot - 1, hslInfo) then
    _PA_ASSERT_NAME(false, "ìƒ‰ìƒ í”„ë¦¬ì…‹ ì €ì¥ ì‹¤íŒ¨", "ê¹€í˜„ë¹ˆ")
  else
