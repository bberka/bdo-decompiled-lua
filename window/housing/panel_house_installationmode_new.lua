...
function PaGlobal_HouseInstallation:update(nowInterval)
  ...
  if true == self._houseInstallationMode then
    ...
    if nil == houseWrapper then
      _PA_ASSERT(false, "housing_getHouseholdActor_CurrentPosition()ê°€ nullptr ì´ë©´ ì•ˆë©ë‹ˆë‹¤.")
      return
    end
...
function PaGlobal_HouseInstallation:open()
  _PA_LOG("ì§€ë¯¼í˜", "PaGlobal_HouseInstallation:Open()")
  ...
  if housing_isBuildMode() then
    ...
    if 0 ~= rv then
      _PA_LOG("ì§€ë¯¼í˜", "housing_changeHousingMode Failed...")
      return
    end
