...
function PaGlobalFunc_MarketPlaceSell_SelectPriceByIndex(index)
  local self = MarketPlaceSell
  if nil == self then
    _PA_ASSERT(false, "íŒ¨ë„ì´ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤!! : MarketPlaceSell")
    return
  end
...
function PaGlobal_MarketPlaceSell_Req()
  ...
  local function sellExcute()
    ...
    _PA_LOG("ìµœí•œì§„", "" .. tostring(lastOtpConfirm) .. " ::::: " .. tostring(utcTime))
  ...
  local function checkbox_sell()
    ...
    local lastOtpConfirm = getWorldMarketLastOtpConfirm()
    local utcTime = Int64toInt32(getUtc64())
    _PA_LOG("ìµœí•œì§„", "isOtpItem : " .. tostring(self._isOtpItem) .. " ContentsGroup_WorldMarketOTP :" .. tostring(_ContentsGroup_WorldMarketOTP))
    if true == self._isOtpItem and lastOtpConfirm < utcTime and true == _ContentsGroup_WorldMarketOTP then
    ...
    if false == isPremiumUser and false == itemSSW:get():isCash() and false == isGameTypeKR2() then
      messageBoxMemo = messageBoxMemo .. [[
