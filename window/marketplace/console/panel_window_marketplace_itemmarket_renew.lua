...
function ItemMarket:setSelectSlot(itemKey)
  ...
  if nil == itemInfo then
    if 0 == itemKey then
      _PA_LOG("ì‹ ì§€ì˜", "itemKeyê°€ 0ì…ë‹ˆë‹¤! ê¸‰ë“±ë½ ìƒí’ˆì´ ì•„ë‹ˆë¼ë©´ ë¬¸ì œê°€ ìˆìŠµë‹ˆë‹¤.")
    else
      _PA_LOG("ì‹ ì§€ì˜", "ì•„ì´í…œ í‚¤ê°’ì´ ìœ íš¨í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤: getWorldMarketListByItemKey(" .. tostring(itemKey) .. ")")
    end
