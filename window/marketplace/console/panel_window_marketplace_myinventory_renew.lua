...
function PaGlobalFunc_MarketPlace_MyInven_Open()
  ...
  PaGlobalFunc_AutoSortFunctionPanel_LinkPanel(InvenSortLinkPanelIndex.MarketPlaceInventory, mainPanel, self._ui.btn_sortOption, self._ui.chk_sort, "Static_SortComponent_Inventory")
  PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.MarketPlaceInventory, false)
  self._ui.scroll_ItemSlot:SetControlPos(0)
  self._startInvenSlotIndex = 0
  self:setUsingTab()
...
function MarketPlace_MyInven:setUsingTab()
  ...
  if paddingCount <= 0 then
    _PA_ASSERT(false, "ê±°ë˜ì†Œ ê°€ë°© íƒ­ì˜ paddingCountê°€ 0ë³´ë‹¤ ì‘ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤. í™•ì¸ì´ í•„ìš”í•©ë‹ˆë‹¤.")
    return
  end
  ...
  if paddingTotalSize < 0 then
    _PA_ASSERT(false, "ì‚¬ìš©í•˜ë ¤ëŠ” ê±°ë˜ì†Œ ê°€ë°© íƒ­ì˜ ê°¯ìˆ˜ê°€ ë„ˆë¬´ ë§ìŠµë‹ˆë‹¤. í™•ì¸ì´ í•„ìš”í•©ë‹ˆë‹¤.")
    return
  end
