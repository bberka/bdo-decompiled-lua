...
function PaGlobalFunc_Wallet_Init()
  ...
  self._isOpenedMileageFunction = true == _ContentsGroup_ChinaOnly
  self:initData()
  self:initControl()
  self:initEvent()
  if isGameTypeKR2() == true and self._ui.txt_MarketTaxBonus:GetShow() == true then
...
function PaGlobalFunc_MarketPlace_Wallet_CreateControlHistroyList(contents, key)
  local self = MarketPlace_Wallet
  if nil == self then
    _PA_ASSERT(false, "íŒ¨ë„ì´ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤!! : MarketPlace_Wallet")
    return
  end
...
function PaGlobalFunc_MarketPlace_GetWalletItemCount(itemEnchantKey)
  ...
  for idx = 0, walletItemListCount - 1 do
    ...
    if nil == itemMyWalletInfo then
      _PA_ASSERT(false, "ì •ë³´ê°€ ì˜¬ë°”ë¥´ì§€ ì•ŠìŠµë‹ˆë‹¤!! : getWorldMarketMyWalletListByIdx(idx)")
      return
    end
...
function PaGlobalFunc_MarketPlace_Wallet_CreateControlMyBiddingList(contents, key)
  ...
  if self._selectedBiddingTabIndex == self._biddingTabIdx.sell then
    ...
    if nil == itemInfo then
      _PA_ASSERT(false, "ì •ë³´ê°€ ì˜¬ë°”ë¥´ì§€ ì•ŠìŠµë‹ˆë‹¤!! : getWorldMarketSellBiddingListByIdx( idx )")
      return
    end
  ...
  elseif self._selectedBiddingTabIndex == self._biddingTabIdx.buy then
    ...
    if nil == itemInfo and false == PaGlobal_TutorialPhase_IsTutorial() then
      _PA_ASSERT(false, "ì •ë³´ê°€ ì˜¬ë°”ë¥´ì§€ ì•ŠìŠµë‹ˆë‹¤!! : getWorldMarketBuyBiddingListByIdx( idx )")
      return
    end
...
function PaGlobalFunc_MarketPlace_Wallet_OpenMySellTab()
  local self = MarketPlace_Wallet
  if nil == self then
    _PA_ASSERT(false, "íŒ¨ë„ì´ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤!! : MarketPlace")
    return
  end
...
function PaGlobalFunc_MarketPlcae_Wallet_RecieveAll()
  ...
  elseif false == isPremiumUser and false == _ContentsGroup_ChinaOnly then
...
function PaGlobalFunc_Wallet_Open()
  ...
  if false == _panel:GetShow() then
    _PA_ASSERT(false, "ë©”ì¸ íŒ¨ë„ì´ ì—´ë ¤ìˆì§€ ì•Šì€ë° ì•„ì´í…œ ë§ˆì¼“ì„ ì—´ë ¤ê³  í–ˆìŠµë‹ˆë‹¤. : íŒ¨ë„ : Panel_Window_MarketPlace_Main")
    return
  end
  self._prevItemIdx = -1
  self._startInvenSlotIndex = 0
  ...
  PaGlobalFunc_AutoSortFunctionPanel_LinkPanel(InvenSortLinkPanelIndex.MarketPlaceWalletMain, _panel, self._ui.btn_sortOption, self._ui.chk_sort)
  PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.MarketPlaceWalletMain, false)
  PaGlobalFunc_MarketPlace_Wallet_ClearFocusSearchText()
