...
function MarketPlace_MyInven:initData()
  ...
  self._useMileageFunction = true == _ContentsGroup_ChinaOnly
end
...
function PaGlobalFunc_MarketPlace_MyInven_Scroll(isUp)
  local self = MarketPlace_MyInven
  if nil == self then
    _PA_ASSERT(false, "íŒ¨ë„ì´ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤!! : Panel_Window_MarketPlace_MyInventory")
    return
  end
...
function MarketPlace_MyInven:marketFilter(slotNo, itemWrapper, invenWhereType)
  ...
  if true == _ContentsGroup_ChinaOnly and true == itemWrapper:get():isOldMoonSmeltableItem() and itemWrapper:get():getCurrentOldMoonSmeltedValue() ~= itemWrapper:get():getMaxOldMoonSmeltedValue() then
    isAble = false
  end
  return not isAble
end
...
function PaGlobalFunc_MarketPlace_MyInven_Open(isWaretype)
  ...
  PaGlobalFunc_AutoSortFunctionPanel_LinkPanel(InvenSortLinkPanelIndex.MarketPlaceInventory, _panel, self._ui.btn_sortOption, self._ui.checkBtn_ItemSort)
