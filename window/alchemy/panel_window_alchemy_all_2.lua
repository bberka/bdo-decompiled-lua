...
function PaGlobal_Alchemy_All_StartAlchemyForConsole()
  ...
  local msgBoxData = {
    title = PAGetString(Defines.StringSheet_GAME, PaGlobal_Alchemy_All._isCook and "LUA_ALCHEMY_ALCHEMY"),
    content = PAGetString(Defines.StringSheet_GAME, PaGlobal_Alchemy_All._isCook and "LUA_ALCHEMY_ASK_MULTIPLE_TIMES"),
...
function PaGlobal_Alchemy_All_StartAlchemy(countProduction)
  ...
  if countProduction > Defines.s64_const.s64_1 and false == ToClient_AlchemySetupMaterialsForMassProduction(countProduction) then
    _PA_ASSERT(false, "setupMaterialsForMassProductionì´ ì‹¤íŒ¨í–ˆìŠµë‹ˆë‹¤. countProductionì´ ToClient_AlchemyGetMaxMassProductionCount()ë³´ë‹¤ í° ê²ƒì€ ì•„ë‹Œì§€ í™•ì¸í•´ë³´ì‹œê¸° ë°”ëë‹ˆë‹¤.")
  end
  ...
  EventProgressBarShow(true, progressBarTimeSec, 9)
