function PaGlobal_Equip_CharacterTag_ItemCopy:init()
  if nil == Panel_Window_Equip_CharacterTag_ItemCopy or true == self._initailize then
    UI.ASSERT_NAME("ë…¸ëŒ€ì˜", false, "PaGlobal_Equip_CharacterTag_ItemCopy:init ì‹¤íŒ¨ Panel_Window_Equip_CharacterTag_ItemCopy íŒ¨ë„ì´ ì—†ìŠµë‹ˆë‹¤")
    return
  end
...
function PaGlobal_Equip_CharacterTag_ItemCopy:validate()
  if nil == Panel_Window_Equip_CharacterTag_ItemCopy then
    UI.ASSERT_NAME(false, "Panel_Window_Equip_CharacterTag_ItemCopy ë¡œë“œë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤.", "ë…¸ëŒ€ì˜")
    return
  end
