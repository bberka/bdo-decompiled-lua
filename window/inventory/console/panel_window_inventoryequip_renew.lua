...
function InventoryInfo:initCostumeUIControls()
  ...
  self._ui.btn_collectBag:SetShow(_ContentsGroup_CollectToolBox)
...
function FromClient_InventoryInfo_ResponseMailGetItem(itemKey, itemCount_s64, immediateItem, isRelay)
  if nil == itemKey or nil == immediateItem then
    UI.ASSERT_NAME(false, "itemKey, immediateItemê°€ ì—†ìŠµë‹ˆë‹¤.", "ê¹€ì˜ì§„")
  end
...
function InventoryInfo:open(openType, isFold)
  ...
  PaGlobalFunc_AutoSortFunctionPanel_LinkPanel(InvenSortLinkPanelIndex.ColsoleInventory, _panel, self._ui.btn_sortOption, self._ui.chk_sort, "Static_SortComponent_Inventory")
...
function InventoryInfo:forceSetSnapToInventory()
  if _panel == nil then
    return
  end
...
function InventoryInfo:close()
  ...
  if _ContentsGroup_NewUI_Dye_All == true and Panel_Window_Palette_All:GetShow() == true then
...
function InventoryInfo:updateEquipment(isMustUpdate)
...
function InventoryInfo:refreshSortKeyguide()
  local RSKeyguideX = 2
  ...
  if nil ~= itemWrapper and slot.icon:IsEnable() then
    ...
    elseif not itemStatic:isUseToVehicle() then
      ...
      if 2 == item_type and true == itemSSW:get():isForJustTrade() then
        ...
        if true == itemSSW:get():isEquipable() and false == PaGlobalFunc_Util_CheckSeasonEquip(itemSSW) then
...
function PaGlobal_FamilyInvnetoryShowToggle()
  ...
  if self._isShowFamilyInven == true then
    PaGlobalFunc_AutoSortFunctionPanel_LinkPanel(InvenSortLinkPanelIndex.ColsoleFamilyInventory, _panel, self._ui.btn_familySortOption, self._ui.chk_familySort, "Static_SortComponent_FamilyInventory")
...
function FromClient_InventoryInfo_PadSnapChangePanel(fromPanel, toPanel)
  if nil ~= toPanel and true == _panel:GetShow() then
    if _panel:GetKey() ~= toPanel:GetKey() then
      ...
      _panel:registerPadEvent(__eConsoleUIPadEvent_LB, "Toggle_InventoryTab_forPadEventFunc(-1)")
      _panel:registerPadEvent(__eConsoleUIPadEvent_RB, "Toggle_InventoryTab_forPadEventFunc(1)")
      InventoryInfo:refreshSortKeyguide()
    end
  end
end
...
function InventoryInfo:getClassAttackType(classType)
  if __eClassType_Warrior == classType or __eClassType_Valkyrie == classType or __eClassType_Giant == classType or __eClassType_Kunoichi == classType or __eClassType_NinjaMan == classType or __eClassType_BladeMaster == classType or __eClassType_BladeMasterWoman == classType or __eClassType_Combattant == classType or __eClassType_Mystic == classType or __eClassType_Lhan == classType or __eClassType_ShyWaman == classType or __eClassType_Guardian == classType or __eClassType_Nova == classType or __eClassType_Corsair == classType or __eClassType_Drakania == classType or __eClassType_Giant_Reserved0 == classType or __eClassType_Giant_Reserved2 == classType or __eClassType_KunoichiOld == classType then
    return 0
  elseif __eClassType_ElfRanger == classType or __eClassType_RangerMan == classType then
    return 1
  elseif __eClassType_Sorcerer == classType or __eClassType_Tamer == classType or __eClassType_WizardMan == classType or __eClassType_WizardWoman == classType or __eClassType_DarkElf == classType or __eClassType_Hashashin == classType or __eClassType_Sage == classType then
    return 2
  end
  return nil
end
...
function PaGlobal_InventoryEquip_ShowSeasonUpgradeBannerEffect(isBGShow, isEffectShow)
  if false == _ContentsGroup_Season_EquipmentUpgrade then
    InventoryInfo._ui.stc_season_Upgrade:SetShow(false)
    InventoryInfo._ui.stc_season_UpgradeBg:SetShow(false)
    InventoryInfo._ui.stc_season_UpgradeEffect:EraseAllEffect()
    return
  end
  local selfPlayer = getSelfPlayer()
  if nil == selfPlayer then
  ...
  if true == isBGShow and (true == equipInven or true == Inventory) then
    ...
    InventoryInfo._ui.stc_season_Upgrade:SetShow(true)
    InventoryInfo._ui.stc_season_UpgradeBg:SetShow(true)
  else
  ...
  if true == isEffectShow and (true == equipInven or true == Inventory) then
    InventoryInfo._ui.stc_season_UpgradeEffect:AddEffect("fUI_Equipment_UP_01A", true, 0, 0)
  end
  InventoryInfo:updateEquipment(true)
end
function PaGlobal_InventoryEquip_ShowSeasonUpgradePossibleEffect(itemWrapper)
  ...
  return itemSSW:get()._isChangeEquipItem
end
