...
function PaGlobal_Inventory_All:setMileageImage()
  self._ui.stc_mileageIcon:ChangeTextureInfoName("Combine/Icon/Combine_Cashshop_Icon_00.dds")
  if isGameTypeEnglish() or isGameTypeID() then
    local x1, y1, x2, y2 = setTextureUV_Func(self._ui.stc_mileageIcon, 24, 1, 46, 23)
    self._ui.stc_mileageIcon:getBaseTexture():setUV(x1, y1, x2, y2)
  elseif isGameTypeTR() then
    local x1, y1, x2, y2 = setTextureUV_Func(self._ui.stc_mileageIcon, 47, 1, 69, 23)
    self._ui.stc_mileageIcon:getBaseTexture():setUV(x1, y1, x2, y2)
  elseif isGameTypeKR2() then
    local x1, y1, x2, y2 = setTextureUV_Func(self._ui.stc_mileageIcon, 70, 1, 91, 23)
    self._ui.stc_mileageIcon:getBaseTexture():setUV(x1, y1, x2, y2)
  else
    local x1, y1, x2, y2 = setTextureUV_Func(self._ui.stc_mileageIcon, 1, 1, 23, 23)
    self._ui.stc_mileageIcon:getBaseTexture():setUV(x1, y1, x2, y2)
  end
end
...
function PaGlobal_Inventory_All:getClassAttackType(classType)
  ...
  elseif __eClassType_ElfRanger == classType or __eClassType_RangerMan == classType then
    return 1
  elseif __eClassType_Sorcerer == classType or __eClassType_Tamer == classType or __eClassType_WizardMan == classType or __eClassType_WizardWoman == classType or __eClassType_DarkElf == classType or __eClassType_Hashashin == classType or __eClassType_Sage == classType or __eClassType_ElfRanger_Reserved1 == classType then
    return 2
  end
  UI.ASSERT_NAME(false, "[ìˆ˜ì • í•„ìš”] classType(" .. tostring(classType) .. ")ì— í•´ë‹¹í•˜ëŠ” attackTypeì´ ì§€ì •ë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤. ì´ê±°ëœ¨ë©´ ê¼­ ì¶”ê°€í•´ì£¼ì„¸ìš”.", "ì´ì£¼")
  return nil
end
