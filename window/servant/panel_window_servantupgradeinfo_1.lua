...
function PaGlobal_ServantUpgradeInfo:makeString(to, from, unitStr, unitValue, dp)
  ...
  if diff > 0 then
    return (string.format("%s%s(<PAColor0xff83a543>â–²%s<PAOldColor>)", toStr, unitStr, diffStr))
  elseif diff < 0 then
    return (string.format("%s%s(<PAColor0xffd05d48>â–¼%s<PAOldColor>)", toStr, unitStr, diffStr))
  else
