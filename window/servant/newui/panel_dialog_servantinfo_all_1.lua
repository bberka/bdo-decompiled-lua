...
function PaGlobal_ServantInfo_All:changeEquipSlotBaseTexture(vehicleType)
  ...
  local vehicleTypeGroup = PaGlobalFunc_Util_getVehicleTypeGroup(vehicleType)
  if nil == vehicleTypeGroup or Util.ENUM_VEHICLE_EQUIP_TYPE_GROUP._COUNT == vehicleTypeGroup then
    _PA_ASSERT(false, "ServantInfo_ALL ëª©ë¡ì— ì—†ëŠ” vehicleType" .. "(" .. tostring(vehicleType) .. ") ì…ë‹ˆë‹¤. Type ì¶”ê°€ê°€ í•„ìš”í•˜ì§€ ì•Šì€ì§€ í™•ì¸ë°”ëë‹ˆë‹¤.")
    return
  end
  if nil == Util.VEHICLE_EQUIP_TYPE_ICON_UV[vehicleTypeGroup] then
    _PA_ASSERT(false, "ServantInfo_ALL ì¥ë¹„ ì•„ì´ì½˜ ëª©ë¡ì— ì—†ëŠ” vehicleType" .. "(" .. tostring(vehicleType) .. ") ì…ë‹ˆë‹¤. ì¶”ê°€ê°€ í•„ìš”í•˜ì§€ ì•Šì€ì§€ í™•ì¸ë°”ëë‹ˆë‹¤..")
    return
  end
  if nil == Util.VEHICLE_PEARLEQUIP_TYPE_ICON_UV[vehicleTypeGroup] then
    _PA_ASSERT(false, "ServantInfo_ALL í„ì¥ë¹„ ì•„ì´ì½˜ ëª©ë¡ì— ì—†ëŠ” vehicleType" .. "(" .. tostring(vehicleType) .. ") ì…ë‹ˆë‹¤. ì¶”ê°€ê°€ í•„ìš”í•˜ì§€ ì•Šì€ì§€ í™•ì¸ë°”ëë‹ˆë‹¤..")
    return
  end
  if nil == Util.VEHICLE_EQUIP_TYPE_TEXTURE_PATH[vehicleTypeGroup] then
    _PA_ASSERT(false, "ServantInfo_ALL í…ìŠ¤ì²˜ ëª©ë¡ì— ì—†ëŠ” vehicleType" .. "(" .. tostring(vehicleType) .. ") ì…ë‹ˆë‹¤. ì¶”ê°€ê°€ í•„ìš”í•˜ì§€ ì•Šì€ì§€ í™•ì¸ë°”ëë‹ˆë‹¤..")
    return
  end
  for slotNo = 1, self._SERVANT_EQUIP_SLOT_COUNT do
