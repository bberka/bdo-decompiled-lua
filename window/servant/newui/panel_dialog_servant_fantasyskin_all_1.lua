...
function PaGlobal_ServantFantasySkin:changeClickControl(buttonIndex)
  ...
  if buttonIndex ~= 0 and buttonIndex ~= 1 then
    UI.ASSERT_NAME(false, "ë²„íŠ¼ì˜ ì¸ë±ìŠ¤ê°€ ë¹„ì •ìƒì…ë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
...
function PaGlobal_ServantFantasySkin:updateTextureSlot()
  ...
  if PaGlobalFunc_ServantFunction_All_Get_NpcType() ~= 0 then
    UI.ASSERT_NAME(false, "ë§ˆêµ¿ê°„ì´ ì•„ë‹Œë° í™˜ìƒë§ˆì˜ ì™¸í˜• ë³€ê²½ì„ í•  ìˆ˜ëŠ” ì—†ìŠµë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
  local servantInfo = stable_getServant(self._currentServantNo)
  if servantInfo == nil then
    UI.ASSERT_NAME(false, "ì„ íƒí•œ í™˜ìƒë§ˆ ì •ë³´ê°€ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
  ...
  if isRollBackFormIndex == true then
    ...
    if changableFormCount == 0 or changableFormCount == nil then
      UI.ASSERT_NAME(false, "í˜„ì¬ ì„ íƒí•œ í™˜ìƒë§ˆì˜ ë³€ê²½ ê°€ëŠ¥í•œ ì™¸í˜• ì •ë³´ê°€ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤.", "ì´ì£¼")
      return
    end
...
function PaGlobal_ServantFantasySkin:setServantModelForm_new(doForce)
  ...
  if self._currentServantNo == nil then
    UI.ASSERT_NAME(false, "í˜„ì¬ ì„ íƒëœ íƒ‘ìŠ¹ë¬¼ì˜ ServantNoê°€ ë¹„ì •ìƒì…ë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
  if PaGlobalFunc_ServantFunction_All_Get_NpcType() ~= 0 then
    UI.ASSERT_NAME(false, "ë§ˆêµ¿ê°„ì´ ì•„ë‹Œë° í™˜ìƒë§ˆì˜ ì™¸í˜• ë³€ê²½ì„ í•  ìˆ˜ëŠ” ì—†ìŠµë‹ˆë‹¤.", "ì´ì£¼")
  ...
  local formManager = getServantFormManager()
  if formManager == nil then
    UI.ASSERT_NAME(false, "formManagerê°€ ì—†ìŠµë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
  local currentServantCharacterKey = servantInfo:getCharacterKey()
  local changableFormCount = formManager:getFormCountByCharacterKey(currentServantCharacterKey)
  if changableFormCount == 0 or changableFormCount == nil then
    UI.ASSERT_NAME(false, "ë³€ê²½ ê°€ëŠ¥í•œ ì™¸í˜•ì •ë³´ê°€ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
  local newFormSSW
  if isRollBackFormIndex == true then
  ...
  if newFormSSW == nil then
    UI.ASSERT_NAME(false, "ì˜¤ë¦¬ì§€ë„ formStaticStatusWrapperê°€ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
...
function PaGlobal_ServantFantasySkin:setServantModelForm(newFormIndexRaw, doForce)
  ...
  if formSSW == nil then
    UI.ASSERT_NAME(false, "FormIndexì— í•´ë‹¹í•˜ëŠ” formStaticStatusWrapperê°€ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
  ...
  if selectedSceneIndex == nil then
    UI.ASSERT_NAME(false, "í˜„ì¬ ì„ íƒëœ SceneIndexê°€ ì—†ìŠµë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
