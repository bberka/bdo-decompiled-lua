...
function HandleClickedGuildMenuButton(index)
  ...
  elseif _UI_Menu_Button.Type_PriceLimit == index then
    ...
    UI.ASSERT(false, "ì‘ì—…í•´ì•¼í•©ë‹ˆë‹¤!")
    return
  end
  GuildListInfoPage._buttonListBG:SetShow(false)
  local messageboxData = {
...
function HandleClicked_GuildListSort(sortType)
  ...
  if 0 == sortType then
    if false == _listSort.grade then
      self.staticText_Grade:SetText(PAGetString(Defines.StringSheet_RESOURCE, "GUILD_TEXT_POSITION") .. "â–²")
  ...
  elseif 1 == sortType then
    if false == _listSort.level then
      self.staticText_Level:SetText(PAGetString(Defines.StringSheet_RESOURCE, "GUILD_TEXT_LEVEL") .. "â–²")
  ...
  elseif 2 == sortType then
    if false == _listSort.class then
      self.staticText_Class:SetText(PAGetString(Defines.StringSheet_RESOURCE, "GUILD_TEXT_CLASS") .. "â–²")
  ...
  elseif 3 == sortType then
    if false == _listSort.name then
      self.staticText_charName:SetText(PAGetString(Defines.StringSheet_RESOURCE, "GUILD_TEXT_CHARNAME") .. "â–²")
      _listSort.name = true
    else
      self.staticText_charName:SetText(PAGetString(Defines.StringSheet_RESOURCE, "GUILD_TEXT_CHARNAME") .. "â–¼")
  ...
  elseif 4 == sortType then
    if false == _listSort.ap then
      self.staticText_activity:SetText(PAGetString(Defines.StringSheet_RESOURCE, "GUILD_TEXT_ACTIVITY") .. "â–²")
      _listSort.ap = true
    else
      self.staticText_activity:SetText(PAGetString(Defines.StringSheet_RESOURCE, "GUILD_TEXT_ACTIVITY") .. "â–¼")
      _listSort.ap = false
  ...
  elseif 5 == sortType then
    if false == _listSort.expiration then
      self.staticText_contract:SetText(PAGetString(Defines.StringSheet_RESOURCE, "GUILD_TEXT_HIRE") .. "â–²")
  ...
  elseif 6 == sortType then
    if false == _listSort.wp then
      self.staticText_contributedTendency:SetText(text_contributedTendency .. "â–²")
      _listSort.wp = true
  ...
  elseif 7 == sortType then
    if true == _ContentsGroup_NewSiegeRule then
      if false == _listSort.siegegrade then
        self.staticText_WarGrade:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILDLIST_WARPARTICIPANT_TITLE") .. "â–²")
    ...
    elseif false == _listSort.siegegrade then
      self.staticText_WarGrade:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILDLIST_WARGRADETITLE") .. "â–²")
      _listSort.siegegrade = true
    else
      self.staticText_WarGrade:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILDLIST_WARGRADETITLE") .. "â–¼")
      _listSort.siegegrade = false
...
function GuildListInfoPage:UpdateData()
  ...
  for index = 0, memberCount - 1 do
    ...
    local myGuildMemberInfo = myGuildListInfo:getMember(dataIdx)
    if nil == myGuildMemberInfo then
      _PA_ASSERT(false, "ë©¤ë²„ ë°ì´í„°ê°€ ì—†ì„ ìˆ˜ ìˆë‚˜")
      return
    end
