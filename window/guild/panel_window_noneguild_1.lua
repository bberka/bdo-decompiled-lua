function PaGlobal_Guild_NoneGuild:init()
  ...
  if isGameTypeKR2() == true then
    self._ui._txt_descTitle:SetShow(false)
    self._ui._stc_descBG:SetShow(false)
    self._ui._txt_desc:SetShow(false)
    self._ui._frame_desc:SetShow(false)
    self._ui._stc_horizonLine:SetShow(false)
  end
  self:clear()
...
function PaGlobal_Guild_NoneGuild:requestGuildApplicant(selectedCategory, index)
  ...
  local function requestUpdateGuildApplicant()
    ...
    elseif false == PaGlobal_Guild_NoneGuild._isSearchList then
      ToClient_requestUpdateGuildApplicant(__eGuildApplicantType_Insert, selectedCategory, index, "ì§€ì›í•˜ê¸°", false)
    else
      ToClient_requestUpdateGuildApplicant(__eGuildApplicantType_Insert, selectedCategory, index, "ì§€ì›í•˜ê¸°", true)
    end
  end
...
function PaGlobal_Guild_NoneGuild:sortName(isChange)
  ...
  if false == self._nameSort then
    self._ui._txt_sort_name:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILD_GUILDNAME") .. "â–¼")
  else
    self._ui._txt_sort_name:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILD_GUILDNAME") .. "â–²")
  end
  self:update()
...
function PaGlobal_Guild_NoneGuild:sortMemberCount(isChange)
  ...
  if false == self._membercountSort then
    self._ui._txt_sort_membercount:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILD_SIZETITLE") .. "â–¼")
  else
    self._ui._txt_sort_membercount:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILD_SIZETITLE") .. "â–²")
  end
  self:update()
