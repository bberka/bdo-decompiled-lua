...
function PaGlobalFunc_AgreementGuild_Master_ContractOpen(memberIdx, requesterMemberGrade, usaAbleActivity)
  ...
  if nil == memberinfo then
    _PA_ASSERT(false, "PaGlobalFunc_AgreementGuild_Master_ContractOpen ì˜ ë©¤ë²„ì¸ë±ìŠ¤ê°€ ë¹„ì •ìƒì…ë‹ˆë‹¤ " .. tostring(memberIdx))
    return
  end
...
function PaGlobalFunc_AgreementGuildMaster_RightCheck(eRightType, memberIdx, requesterMemberGrade)
  ...
  if nil == memberInfo then
    _PA_ASSERT(false, "FGlobal_AgreementGuild_Master_Open ì˜ ë©¤ë²„ì¸ë±ìŠ¤ê°€ ë¹„ì •ìƒì…ë‹ˆë‹¤ " .. tostring(memberIdx))
    return false
  end
