...
function PaGlobal_GuildAllianceList_All:UpdateAliyNotice(myAllianceWrapper)
  ...
  if false == _ContentsGroup_RenewUI then
    ...
    if nil ~= guildCommentsWebUrl then
      ...
      local url = guildCommentsWebUrl .. "/guild" .. tostring(myGuildNo_s64) .. "&userNo=" .. tostring(myUserNo) .. "&certKey=" .. tostring(cryptKey) .. "&isMaster=" .. tostring(isAdmin) .. "&chairGuildNo=" .. tostring(guildNo_s64)
