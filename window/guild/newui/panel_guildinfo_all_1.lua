...
function PaGlobal_GuildInfo_All:swichPlatform(isUsePad)
  self._ui.btn_InfoWeb:SetShow(not isUsePad and not self._isKR2)
  ...
  self._ui.txt_TadayCommentTitle:SetShow(not self._isConsole and not self._isKR2)
...
function PaGlobal_GuildInfo_All:guildTodayCommentUpdate(guildWrapper)
  ...
  if guildCommentsWebUrl ~= nil and self._isConsole == false and self._isKR2 == false then
    if nil == Panel_IME then
      UI.ASSERT_NAME(false, "Panel_IME is nil", "-")
      return
    end
    FGlobal_SetCandidate()
    local url = guildCommentsWebUrl .. "/guild" .. tostring(guildNo_s64) .. "&userNo=" .. tostring(myUserNo) .. "&certKey=" .. tostring(cryptKey) .. "&isMaster=" .. tostring(isAdmin)
    _urlCache = url
    self._ui.stc_webControl:ResetUrl()
