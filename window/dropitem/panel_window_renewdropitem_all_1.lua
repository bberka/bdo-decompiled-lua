...
function PaGlobal_RenewDropItem_All:registEventHandler()
  ...
  if self._isConsole == true then
    ...
    Panel_Window_DropItem_All:registerPadEvent(__eConsoleUIPadEvent_Up_X, "PaGlobalFunc_RenewDropItem_ShowSnappedItemTooltip()")
...
function PaGlobal_RenewDropItem_All:initSortOptionInfo()
  ...
  for index = 0, __eDropUIRegionSortOpion_Name_Count - 1 do
    ...
    elseif index == __eDropUIRegionSortOpion_Atk_Desc then
      ...
      UI.ASSERT_NAME(false, "DropUIRegionSortOptionì´ ì¶”ê°€ë˜ë©´ ì—¬ê¸°ë„ ì¶”ê°€í•´ì£¼ì…”ì•¼í•©ë‹ˆë‹¤.", "ì´ì£¼")
    end
...
function PaGlobal_RenewDropItem_All:selectHuntingGroundListContentByIndex(index)
  ...
  for listIndex = 0, currentHuntingGroundContentListSize - 1 do
    ...
    if content ~= nil then
      ...
      local selectedHuntingGroundIndex
...
function PaGlobal_RenewDropItem_All:updateHuntingGroundDetailInfo(doResetFrameScroll)
  ...
  if self._currentMainCategoryKey == nil then
    _PA_ASSERT(false, "í˜„ì¬ ì„ íƒëœ ë©”ì¸ ì¹´í…Œê³ ë¦¬ ì •ë³´ê°€ ì—†ìŠµë‹ˆë‹¤!")
    return
  end
  if self._currentSubCategoryKey == nil then
    _PA_ASSERT(false, "í˜„ì¬ ì„ íƒëœ ì„œë¸Œ ì¹´í…Œê³ ë¦¬ ì •ë³´ê°€ ì—†ìŠµë‹ˆë‹¤!")
    return
  end
  ...
  if currentHuntingGroundSS == nil then
    _PA_ASSERT(false, "í˜„ì¬ ì„ íƒëœ ì‚¬ëƒ¥í„°ì˜ StaticStatus ì •ë³´ê°€ ì—†ìŠµë‹ˆë‹¤!")
    return
  end
  ...
  elseif currentHuntingGroundSS:isHaveRegionInfo() == true then
    ...
    if regionInfo == nil then
      UI.ASSERT_NAME(false, "regionInfoWrapperê°€ nilì…ë‹ˆë‹¤. ë°œìƒì‹œ ê¼­ ì œë³´ ë¶€íƒë“œë¦½ë‹ˆë‹¤.", "ì´ì£¼")
      return
    end
  ...
  local currnetMyAtk = ToClient_getOffence()
  local currentMyAwakenAtk = ToClient_getAwakenOffence()
  local currentMyDef = ToClient_getDefence()
  ...
  local recommendedAtk = currentHuntingGroundSS:getOffensePoint()
  ...
  local recommendedDef = currentHuntingGroundSS:getDefensePoint()
  ...
  for index = 0, tagCount - 1 do
    ...
    if tagSSW ~= nil then
      if self._tagIconList[index] == nil then
        self._tagIconList[index] = self:makeTagIcon(self._ui._stc_tagArea, "Static_Tag_Icon_" .. index, index)
      end
...
function PaGlobal_RenewDropItem_All:makeRepeatQuestIcon(parent, newIconName, iconIndex)
  ...
  local imageSlotBG = UI.getChildControl(cloneTemplate, "Static_QuestSlotBG")
  local newIconData = {
    boxControl = nil,
    imageControl = nil,
    titleControl = nil,
  ...
  newIconData.boxControl = cloneTemplate
  newIconData.imageControl = UI.getChildControl(imageSlotBG, "Static_QuestSlot")
...
function PaGlobal_RenewDropItem_All:makeSuddenQuestIcon(parent, newIconName, iconIndex)
  ...
  local imageSlotBG = UI.getChildControl(cloneTemplate, "Static_QuestSlotBG")
  local progressBG = UI.getChildControl(cloneTemplate, "Static_ProgressBG")
  local newIconData = {
    boxControl = nil,
    imageControl = nil,
    titleControl = nil,
...
function PaGlobal_RenewDropItem_All:doSearch()
