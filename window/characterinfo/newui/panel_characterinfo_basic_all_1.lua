...
function PaGlobal_CharInfoBasic_All:setUnChangedData()
  ...
  self._asianFlag = isGameTypeTaiwan() or isGameTypeKorea() or isGameTypeJapan()
  for index = 0, 2 do
    local data = {
      _titleUI = nil,
      _prog2 = nil,
    ...
    for groupIndex = 0, __eEfficiencyOfDropItemGroup_Count - 1 do
      ...
      elseif groupIndex == __eEfficiencyOfDropItemGroup_Special then
        ...
        if itemDropGroupPercent == 0 then
          addString = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_ITEMDROPEFFECT_EXTRAEFFECT", "item1", zeroPercentString, "item2", zeroPercentString)
        elseif currentArshaServerBuffPercent > 0 and currentMajorSiegeBuffPercent > 0 then
          addString = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_ITEMDROPEFFECT_EXTRAEFFECT", "item1", currentArshaServerBuffPercent .. "%", "item2", currentMajorSiegeBuffPercent .. "%")
        elseif currentArshaServerBuffPercent > 0 then
          addString = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_ITEMDROPEFFECT_EXTRAEFFECT", "item1", currentArshaServerBuffPercent .. "%", "item2", zeroPercentString)
        elseif currentMajorSiegeBuffPercent > 0 then
          addString = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_ITEMDROPEFFECT_EXTRAEFFECT", "item1", zeroPercentString, "item2", currentMajorSiegeBuffPercent .. "%")
        else
          UI.ASSERT_NAME(false, "ë¹„ì •ìƒì ì¸ ì¼€ì´ìŠ¤ì…ë‹ˆë‹¤. ì•„íš íˆ´íŒ í‘œê¸°ì— ì‹¤íŒ¨ì²˜ë¦¬ë©ë‹ˆë‹¤.", "ì´ì£¼")
        end
      else
        _PA_ASSERT(false, "ì•„ì´í…œ íšë“ í™•ë¥  ê·¸ë£¹ enum(" .. tostring(groupIndex) .. ")ì— í•´ë‹¹í•˜ëŠ” string ì •ì˜ê°€ í•„ìš”í•©ë‹ˆë‹¤.")
      end
      desc = desc .. "\n" .. addString
    end
  end
...
function PaGlobal_CharInfoBasic_All:setClassIcon(control, classType)
  ...
  elseif classType == __eClassType_Warrior_Reserve2 then
    x1, y1, x2, y2 = setTextureUV_Func(control, 343, 286, 399, 342)
  elseif classType == __eClassType_ElfRanger_Reserved1 then
    x1, y1, x2, y2 = setTextureUV_Func(control, 343, 286, 399, 342)
...
function PaGlobal_CharInfoBasic_All:setAtkType()
  ...
  elseif isClassType == __eClassType_Hashashin or isClassType == __eClassType_Sorcerer or isClassType == __eClassType_Tamer or isClassType == __eClassType_DarkElf or isClassType == __eClassType_WizardMan or isClassType == __eClassType_WizardWoman or isClassType == __eClassType_Sage then
