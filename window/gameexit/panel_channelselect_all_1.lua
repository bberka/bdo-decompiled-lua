...
function PaGlobal_ChannelSelect_All:createChannelUI()
  ...
  if nil == curChannelData then
    UI.ASSERT(false, " í˜„ì¬ ì±„ë„ ë°ì´í„°ê°€ ì—†ìŠµë‹ˆë‹¤. ")
    return
  end
  ...
  for idx = 0, totalChannelCount - 1 do
    ...
    cloned_ChannelSlot._stc_PersonalFieldState_Green:SetShow(false)
    cloned_ChannelSlot._stc_PersonalFieldState_Yellow:SetShow(false)
    cloned_ChannelSlot._stc_PersonalFieldState_Red:SetShow(false)
    cloned_ChannelSlot._stc_PersonalFieldState_Locked:SetShow(false)
...
function PaGlobal_ChannelSelect_All:update(isByPersonalField)
  local isGMClient = ToClient_IsGMClient() == true and isGameTypeKR2() == false
  local curChannelData = getCurrentChannelServerData()
  local curWorldData = getGameWorldServerDataByWorldNo(curChannelData._worldNo)
  local channelCount = getGameChannelServerDataCount(curChannelData._worldNo)
  ...
  if false == _ContentsGroup_RenewUI then
    for ch_Index = 0, channelCount - 1 do
      ...
      if nil ~= serverData then
        if true == _ContentsGroup_SeasonContents then
          if (false == serverData._isInstanceChannel or ToClient_SelfPlayerIsGM() == true and isGameTypeKR2() == false) and false == serverData._isSpeedChannel and false == serverData._isSeasonChannel then
            self._serverIndexTable:push_back(ch_Index)
          end
        elseif (false == serverData._isInstanceChannel or ToClient_SelfPlayerIsGM() == true and isGameTypeKR2() == false) and false == serverData._isSpeedChannel then
          self._serverIndexTable:push_back(ch_Index)
        end
      end
    end
      ...
      if nil ~= serverData then
        ...
        if false == isInserted then
          if __ePlatformType_COUNT == serverData._servicePlatformType then
            if false == serverData._isInstanceChannel or ToClient_SelfPlayerIsGM() == true and isGameTypeKR2() == false then
  ...
  for ch_Index = 1, #self._serverIndexTable do
    ...
    if nil == _serverData then
      ...
      if nil ~= self._string_WarName[warTerritoryKey] then
        ...
        _PA_ASSERT(false, "í™•ì¸ë˜ì§€ ì•Šì€ ì „ìŸ ê°€ëŠ¥/ë¶ˆê°€ëŠ¥ ì§€ì—­ê°’ì´ ë“¤ì–´ì™”ë‹¤. í™•ì¸í•˜ê±°ë‚˜ ìƒˆë¡œìš´ ì „ìŸì§€ì—­ì´ ì¶”ê°€ë¬ë‹¤ë©´ ì¶”ê°€í•´ì£¼ì–´ì•¼í•œë‹¤!!!!")
      end
      ...
      elseif _serverData._isPremiumChannel then
        if false == tempWrapper:isPremiumChannelPermission() and false == isGMClient then
      ...
      if true == isByPersonalField then
        ...
        if true == isPrepereToStop or true == isMaintanance or true == isBeingWar or true == _serverData._isSiegeChannel or true == _serverData._isDontPvPTendencyDecrease or true == _serverData._isEventChannel or true == _serverData._isInstanceChannel then
  ...
  if true == _ContentsGroup_InstanceSiege and (nil == isByPersonalField or false == isByPersonalField) then
    ...
    for mode = 0, __eInstanceSiegeMode_Count do
      ...
      if nil == channelSlot then
        UI.ASSERT_NAME(false, "instanceSiegeField ì…ì¥ ë²„íŠ¼ ìƒì„±ì— í•„ìš”í•œ channelSlotì´ nilì…ë‹ˆë‹¤. idx = " .. tostring(ch_idx - 1), "ì¡°ê´‘ë¯¼")
        break
      end
