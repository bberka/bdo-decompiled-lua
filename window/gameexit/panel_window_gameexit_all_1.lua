function PaGlobal_GameExit_All:initialize()
  ...
  self._ui._stc_solareBanner = UI.getChildControl(Panel_Window_GameExit_All, "Static_Solare_Banner")
  local characterDatacount = getCharacterSlotLimit() + 1
  if true == self._isConsole then
    characterDatacount = getCharacterDataCount() + 1
  end
  local rowCount = math.max(math.ceil(math.min(characterDatacount, getCharacterSlotMaxCount(false)) / self._config.slotCols), 3)
...
function PaGlobal_GameExit_All:initShowHide()
  ...
  if true == isGameTypeKR2() then
    self._ui._pc._btn_qeustion:SetShow(false)
    self._ui._pc._chk_listToggle:SetSpanSize(self._ui._pc._btn_qeustion:GetSpanSize().x, self._ui._pc._btn_qeustion:GetSpanSize().y)
  end
end
...
function PaGlobal_GameExit_All:createCopySlot_all()
  ...
  local totalCount = getCharacterSlotMaxCount(false)
  if true == self._isConsole then
...
function PaGlobal_GameExit_All:showToggle(isAttacked)
  ...
  if _ContentsGroup_RenewUI == false and PaGlobal_ExitConfirm_All ~= nil then
  ...
  for idx = startIdx, maxCount - 1 do
    if characterDatacount > idx then
      ...
      if idx <= maxCount - 1 then
        if limitCharacterCount > idx or true == self._isConsole then
...
function PaGlobal_GameExit_All:addInputEventSlotScroll(content, startIdx, endIdx, maxCount)
  if nil == content then
    _PA_LOG("ì„œê¸°ì›", "PaGlobal_GameExit_All:addInputEventSlotScroll í•¨ìˆ˜ì˜ contentê°€ nilì…ë‹ˆë‹¤.")
    return
  end
  if endIdx <= maxCount - 1 then
...
function PaGlobal_GameExit_All:updateCharacterSlot_all()
  ...
  local familyCount = getEnchantInformation():ToClient_getBonusStackCount()
  local maxCharacterCount = getCharacterSlotMaxCount(false)
  local maxIndex = math.min(getCharacterSlotLimit() + 1, maxCharacterCount)
  if true == self._isConsole then
  ...
  for idx = 0, maxIndex - 1 do
    if idx < characterDatacount then
      ...
      elseif maxCharacterCount > getCharacterSlotLimit() then
        self._all._stc_emptyIconPool[idx]:SetShow(false)
        self._all._btn_createCharacterPool[idx]:SetShow(false)
        self._all._btn_buySlotPool[idx]:SetShow(true)
        self._all._btn_buySlotPool[idx]:addInputEvent("Mouse_On", "HandleEventOnOut_GameExit_All_BuySlotText( true," .. idx .. ")")
...
function PaGlobal_GameExit_All:updateLastCompleteQuest()
  ...
  if nil ~= questWrapper then
    self._ui._txt_questList:SetText(ToClient_getReplaceDialog(questWrapper:getTitle()))
  ...
  if nil ~= questWrapper then
    self._ui._txt_questList2:SetText(ToClient_getReplaceDialog(questWrapper:getTitle()))
...
function PaGlobal_GameExit_All:updateAllList()
  local characterDatacount = getCharacterSlotLimit(false) + 1
  if true == self._isConsole then
    characterDatacount = getCharacterDataCount(false) + 1
  end
  local rowCount = math.max(math.ceil(math.min(characterDatacount, getCharacterSlotMaxCount(false)) / self._config.slotCols), 3)
  if rowCount > self._config.slotRows then
    self._addSlotCount = rowCount - self._config.slotRows
...
function PaGlobal_GameExit_All:selectCharacter_all(idx)
  ...
  local char_Level = string.format("%d", characterData._level)
  local char_Name = getCharacterName(characterData)
  local className = getClassName(char_Type)
  local familyName = selfProxy:getUserNickname()
