...
function HandleEventOnOut_GameExit_All_DuelCharInfo(isShow, charDataIdx, startIdx)
  UI.ASSERT_NAME(nil ~= isShow, "HandleEventOnOut_GameExit_All_DuelCharInfo isShow nil", "ì²œë§Œê¸°")
  if false == isShow then
    TooltipSimple_Hide()
    return
  end
...
function HandleEventOnOut_GameExit_All_ServantInfoTooltip(isShow, charDataIdx, uiIdx, servantIdx)
  UI.ASSERT_NAME(nil ~= isShow, "HandleEventOnOut_GameExit_All_ServantInfoTooltip isShow nil", "ì²œë§Œê¸°")
  if false == isShow then
    TooltipSimple_Hide()
    return
  else
    UI.ASSERT_NAME(nil ~= charDataIdx, "HandleEventOnOut_GameExit_All_ServantInfoTooltip charDataIdx nil", "ì²œë§Œê¸°")
    UI.ASSERT_NAME(nil ~= uiIdx, "HandleEventOnOut_GameExit_All_ServantInfoTooltip uiIdx nil", "ì²œë§Œê¸°")
    UI.ASSERT_NAME(nil ~= servantIdx, "HandleEventOnOut_GameExit_All_ServantInfoTooltip servantIdx nil", "ì²œë§Œê¸°")
  end
  local characterData = getCharacterDataByIndex(charDataIdx)
  if nil == characterData then
    return
...
function HandleEventOn_GameExit_All_ShowCharacterCountTooltip()
  local haveCharacterCount = getCharacterDataCount(false)
  local limitCharacterCount = getCharacterSlotLimit(false)
  local maxCharacterCount = getCharacterSlotMaxCount(false)
  local name = "ê°€ë¬¸ ë‚´ ìºë¦­í„°ìˆ˜"
  local desc = ""
  if limitCharacterCount == maxCharacterCount then
    if haveCharacterCount == limitCharacterCount then
      desc = "í˜„ì¬ ë³´ìœ ì¤‘ì¸ ìºë¦­í„°ëŠ” <PAColor0xFFFFEEA0>" .. haveCharacterCount .. "ëª…<PAOldColor> ì…ë‹ˆë‹¤. ë” ì´ìƒ ìºë¦­í„°ë¥¼ í™•ì¥í•  ìˆ˜ ì—†ìŠµë‹ˆë‹¤."
    else
      desc = "í˜„ì¬ ë³´ìœ ì¤‘ì¸ ìºë¦­í„°ëŠ” <PAColor0xFFFFEEA0>" .. haveCharacterCount .. "ëª…<PAOldColor> ì…ë‹ˆë‹¤.\n<PAColor0xFFFFEEA0>" .. limitCharacterCount - haveCharacterCount .. "ëª…<PAOldColor>ì˜ ìºë¦­í„°ë¥¼ ì¶”ê°€ë¡œ ìƒì„±í•  ìˆ˜ ìˆìŠµë‹ˆë‹¤."
    end
  elseif haveCharacterCount == limitCharacterCount then
    desc = "í˜„ì¬ ë³´ìœ ì¤‘ì¸ ìºë¦­í„°ëŠ” <PAColor0xFFFFEEA0>" .. haveCharacterCount .. "ëª…<PAOldColor> ì…ë‹ˆë‹¤.\n<PAColor0xFFFFEEA0>" .. maxCharacterCount - limitCharacterCount .. "ê°œ<PAOldColor>ì˜ ìºë¦­í„° ìŠ¬ë¡¯ì„ í™•ì¥í•  ìˆ˜ ìˆìŠµë‹ˆë‹¤."
  else
    desc = "í˜„ì¬ ë³´ìœ ì¤‘ì¸ ìºë¦­í„°ëŠ” <PAColor0xFFFFEEA0>" .. haveCharacterCount .. "ëª…<PAOldColor> ì…ë‹ˆë‹¤.\n<PAColor0xFFFFEEA0>" .. limitCharacterCount - haveCharacterCount .. "ëª…<PAOldColor>ì˜ ìºë¦­í„°ë¥¼ ì¶”ê°€ë¡œ ìƒì„±í•  ìˆ˜ ìˆìŠµë‹ˆë‹¤.\në˜í•œ <PAColor0xFFFFEEA0>" .. maxCharacterCount - limitCharacterCount .. "ê°œ<PAOldColor>ì˜ ìºë¦­í„° ìŠ¬ë¡¯ì„ í™•ì¥í•  ìˆ˜ ìˆìŠµë‹ˆë‹¤."
  end
...
function PaGlobal_GameExit_All_GetWorkTypeString(workingType)
  ...
  elseif CppEnums.PcWorkType.ePcWorkType_ReadBook == workingType then
    ...
    _PA_ASSERT(false, "ìºë¦­í„° ì‘ì—… íƒ€ì…ì´ ì¶”ê°€ ë˜ì—ˆìŠµë‹ˆë‹¤. Panel_Window_GameExit_All_2.lua ë„ ì¶”ê°€í•´ ì£¼ì–´ì•¼ í•©ë‹ˆë‹¤.")
    workingText = "unKnown"
  end
  return workingText
end
function PaGlobal_GameExit_All_CharChangeConfirm()
  if nil ~= Panel_Window_Solrare_Matching and Panel_Window_Solrare_Matching:GetShow() == true or nil ~= Panel_Popup_Solare_MatchConfirm and Panel_Popup_Solare_MatchConfirm:GetShow() == true or ToClient_IsMatchingStateByMatchServer() == true then
...
function PaGlobalFunc_GameExit_All_ServantInfoText(briefServantInfo)
  UI.ASSERT_NAME(nil ~= briefServantInfo, "PaGlobalFunc_GameExit_All_ServantInfoText briefServantInfo nil", "ì²œë§Œê¸°")
