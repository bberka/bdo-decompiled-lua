...
function PaGlobal_GuildHouse_Auction_All:prepareOpen()
  ...
  if true == self._isConsole and Defines.UIMode.eUIMode_NpcDialog ~= GetUIMode() then
    UI.ASSERT(false, "UIMode ê°€ Dialog ê°€ ì•„ë‹™ë‹ˆë‹¤.  Current Mode = " .. tostring(GetUIMode()))
    return
  end
  self:setNoticeDesc()
...
function PaGlobal_GuildHouse_Auction_All:createAuctionListControl(idx, tabType)
  ...
  UI.ASSERT_NAME(nil ~= idx, "PaGlobal_GuildHouse_Auction_All:createAuctionListControlì˜ idx nilì´ë©´ ì•ˆëœë‹¤.", "ê¹€ìœ¤ì§€")
  UI.ASSERT_NAME(nil ~= tabType, "PaGlobal_GuildHouse_Auction_All:createAuctionListControlì˜ tabType nilì´ë©´ ì•ˆëœë‹¤.", "ê¹€ìœ¤ì§€")
  local control
...
function PaGlobal_GuildHouse_Auction_All:auctionDisplayTime(timeValue)
  UI.ASSERT_NAME(nil ~= timeValue, "PaGlobal_GuildHouse_Auction_All:auctionDisplayTimeì˜ timeValue nilì´ë©´ ì•ˆëœë‹¤.", "ê¹€ìœ¤ì§€")
