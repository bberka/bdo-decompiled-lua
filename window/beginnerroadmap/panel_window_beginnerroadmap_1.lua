...
function PaGlobal_BeginnerRoadMap:processRoadMapEvent(index)
  ...
  if paramList[1] == nil or paramList[1] == undefinedLoadMapEventParamValue then
    UI.ASSERT_NAME(false, "ì´ë²¤íŠ¸ ì´ë¦„ì€ ì •ì˜ë˜ì–´ ìˆëŠ”ë°, ì´ë²¤íŠ¸ í–‰ìœ„ëŠ” ì •ì˜ë˜ì–´ ìˆì§€ ì•ŠìŠµë‹ˆë‹¤!", "ì´ì£¼")
    return
  end
  ...
  if eventCategory == 1 then
    ...
    if targetUIIndex == undefinedLoadMapEventParamValue then
      UI.ASSERT_NAME(false, "ì–´ëŠ UIë¥¼ ì—´ì–´ì•¼ í• ê¹Œìš”", "ì´ì£¼")
      return
    end
    ...
    elseif targetUIIndex == 2 then
      ...
      UI.ASSERT_NAME(false, "ë¡œë“œë§µ UI ë°ì´í„°ì—ì„œ íŠ¹ì • UIë¥¼ ì—´ì–´ì£¼ëŠ” ì´ë²¤íŠ¸ê°€ ì¶”ê°€ë˜ì—ˆìŠµë‹ˆë‹¤. í”„ë¡œê·¸ë¨íŒ€ì—ì„œë„ ì‘ì—…ì´ í•„ìš”í•©ë‹ˆë‹¤.", "ì´ì£¼")
      return
    end
  elseif eventCategory == 2 then
  else
    UI.ASSERT_NAME(false, "ë¡œë“œë§µ UI Event Parameterì˜ ì²˜ë¦¬ ê³¼ì •ì„ ì •ì˜í•´ì£¼ì„¸ìš”. ë°ì´í„°ë§Œ ì¶”ê°€í•˜ë©´ ì•ˆë©ë‹ˆë‹¤. í”„ë¡œê·¸ë¨íŒ€ì—ì„œë„ ì‘ì—…ì´ í•„ìš”í•©ë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
end
