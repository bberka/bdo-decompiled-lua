...
function HandleEventLUp_SailorManagerAll_SelectSailor(index)
  ...
  control.txt_speedValue:SetText(string.format("%.1f", sailorInfo._speed * 0,0001) .. "%")
  control.txt_turningValue:SetText(string.format("%.1f", sailorInfo._turning * 0,0001) .. "%")
  control.txt_feedValue:SetText(sailorInfo._foodConsume)
  control.txt_excelValue:SetText(string.format("%.1f", sailorInfo._accel * 0,0001) .. "%")
  control.txt_breakValue:SetText(string.format("%.1f", sailorInfo._break * 0,0001) .. "%")
  control.txt_costValue:SetText(sailorInfo._cost)
  control.txt_cannonCoolTime:SetText(string.format("%.1f", sailorInfo._cannonCoolTime * 0,0001) .. "%")
  control.txt_cannonAccuracy:SetText(string.format("%.1f", sailorInfo._cannonAccuracy * 0,0001) .. "%")
  control.txt_cannonMaxLength:SetText(string.format("%.1f", sailorInfo._cannonMaxLength * 0,0001) .. "%")
  control.txt_cannonMaxAngle:SetText(string.format("%.1f", sailorInfo._cannonMaxAngle * 0,0001) .. "%")
  ...
  for index = 1, recoveryItemSize do
    ...
    if nil ~= recoveryItem then
      ...
      if nil == restoreSlot then
        _PA_SVC_LOG("ìŠ¬ë¡¯ì´ ì—†ìŠµë‹ˆë‹¤. ì„ ì› ì¶©ì„±ë„ íšŒë³µ UIê°€ ì¼œì§€ì§€ ì•Šì„ê²ë‹ˆë‹¤.")
        return
      end
      ...
      if nil == itemStatic then
        _PA_SVC_LOG("itemStatic ì •ë³´ê°€ ì—†ìŠµë‹ˆë‹¤. ì„ ì› ì¶©ì„±ë„ íšŒë³µ UIê°€ ì¼œì§€ì§€ ì•Šì„ê²ë‹ˆë‹¤.")
        return
      end
