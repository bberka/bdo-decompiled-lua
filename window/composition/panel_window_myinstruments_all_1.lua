...
function PaGlobal_MyInstruments_All:update()
  ...
  if true == _ContentsGroup_AdvancedMusic then
    for index = __eInstrumentAdvancedType_Novice, __eInstrumentAdvancedType_Count - 1 do
      if index == __eeInstrumentAdvancedType_Synth then
        if true == _ContentsGroup_MarnisInstrument then
  ...
  for index = 0, instrumentCount - 1 do
    ...
    if -1 ~= midiType then
      ...
      if advancedTypeClient == advancedType and true == isOpen then
        ...
        if false == addedInstrument then
          insertInstrumentTable[checkIndex] = instrumnetName
          checkIndex = checkIndex + 1
          if nil == self._controlTable[index] then
