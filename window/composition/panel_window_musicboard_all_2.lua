...
function PaGlobal_MusicBoard_All_ListTooltipShow(key, index, temp)
  if nil == Panel_Window_MusicBoard_All then
    return
  end
  ...
  local tooltipIndex = index + PaGlobal_MusicBoard_All._current_InstrumentType * count
  if PaGlobal_MusicBoard_All._current_InstrumentType == __eeInstrumentAdvancedType_Synth then
    if index % PaGlobal_MusicBoard_All._controlCount + 1 == 15 then
      temp = 0
    end
    tooltipIndex = index + temp * PaGlobal_MusicBoard_All._controlCount
  end
  if _ContentsGroup_ContraInstrument == true and _ContentsGroup_ContraInstrument == true and PaGlobal_MusicBoard_All._current_InstrumentType == __eInstrumentAdvancedType_Advanced then
    if 15 == index then
      tooltipIndex = 15
    end
    tooltipIndex = index + (temp - 1) * PaGlobal_MusicBoard_All._controlCount
  end
  TooltipSimple_Show(control, PAGetString(Defines.StringSheet_GAME, "AUDIO_MIDI_INSTRUMENT_" .. tooltipIndex))
end
