function PaGlobal_MusicBoard_All_ListControlCreate(control, key)
  ...
  for index = 0, self._instrumentTrackListSize - 1 do
    ...
    if -1 ~= midiType then
      ...
      trackList[index]._filter:addInputEvent("Mouse_On", "PaGlobal_MusicBoard_All_ListTooltipShow( " .. _key .. ", " .. midiType % self._controlCount .. ", " .. math.floor(midiType / self._controlCount) .. ")")
      trackList[index]._filter:addInputEvent("Mouse_Out", "TooltipSimple_Hide()")
      trackList[index]._cover:addInputEvent("Mouse_On", "PaGlobal_MusicBoard_All_ListTooltipShow( " .. _key .. ", " .. midiType % self._controlCount .. ", " .. math.floor(midiType / self._controlCount) .. ")")
...
function PaGlobal_MusicBoard_All:initialize()
  ...
  for index = self._controlCount, 15 do
    local temp = UI.getChildControl(self._ui._stc_FilterBg, "CheckButton_Instrument_" .. index + 1)
    temp:SetShow(false)
  end
  local checkIndex = 1
  local insertInstrumentTable = {}
