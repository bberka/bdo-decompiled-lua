...
function PaGlobal_Quest_All:ControlAll_Init()
  ...
  if true == isGameTypeKR2() then
    ...
    self._ui.radio_progressing:SetSpanSize(-110, 6)
    self._ui.radio_main:SetSpanSize(110, 6)
    self._ui.radio_recommend:SetShow(false)
    self._ui.radio_repeat:SetShow(false)
    self._ui.radio_new:SetShow(false)
    self._ui.stc_favorTypeBg:SetShow(false)
    Panel_Window_Quest_All:SetSize(Panel_Window_Quest_All:GetSizeX(), Panel_Window_Quest_All:GetSizeY() - self._ui.stc_favorTypeBg:GetSizeY())
    self._ui.stc_DoneQuestBg:SetSpanSize(self._ui.stc_DoneQuestBg:GetSpanSize().x, 5)
    Panel_Window_Quest_All:ComputePosAllChild()
  else
    self._radioTabs[self._TABTYPE.PROGRESS] = self._ui.radio_progressing
    self._radioTabs[self._TABTYPE.MAIN] = self._ui.radio_main
...
function PaGlobal_Quest_All:ControlSetShow()
  ...
  if true == isGameTypeKR2() then
    self._ui_pc.btn_question:SetShow(false)
    self._ui_pc.chk_popUp:SetSpanSize(self._ui_pc.btn_question:GetSpanSize().x, self._ui_pc.btn_question:GetSpanSize().y)
  end
end
