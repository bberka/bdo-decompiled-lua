...
function PaGlobal_TradeMyBasket_All:getMyItem()
  ...
  for itemIdx = 0, myItemCount - 1 do
    ...
    if nil == tradeItemWrapper:get() then
      _PA_ASSERT(false, "npcShop_getTradeItem ê°€ nil ì…ë‹ˆë‹¤!! : PaGlobal_TradeMarketGoods_CreateList")
      return
    end
...
function PaGlobal_TradeMyBasket_All:updateMyItemData(content, key)
  ...
  if 100 < tonumber(tostring(str_sellRate)) then
    str_sellRate_Value = "<PAColor0xff76b24d>" .. str_sellRate_Value .. "â–²<PAOldColor>"
  else
    str_sellRate_Value = "<PAColor0xff3b8bbe>" .. str_sellRate_Value .. "â–¼<PAOldColor>"
  end
  local itemIcon = {}
