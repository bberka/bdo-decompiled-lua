...
function PaGlobal_ContentsTutorial_All:prepareOpen()
  ...
  if self._tutorialKey == -1 or self._tutorialDataCount == 0 then
    if isGameServiceTypeDev() == true then
      _PA_LOG("ì¡°ê´‘ë¯¼", "[ContentsTutorial OpenFailed] prepareOpení˜¸ì¶œ ì „ openCheckAndLoadFile ë¥¼ ë¨¼ì € í˜¸ì¶œí•´ì£¼ì„¸ìš”.")
    end
    return
  end
  ...
  if isGameServiceTypeDev() == true then
    _PA_LOG("ì¡°ê´‘ë¯¼", "[ContentsTutorial Start] tutorialKey(" .. tostring(self._tutorialKey) .. ")")
  end
...
function PaGlobal_ContentsTutorial_All:prepareClose()
  ...
  if self._tutorialKey ~= -1 and isGameServiceTypeDev() == true then
    _PA_LOG("ì¡°ê´‘ë¯¼", "[ContentsTutorial End] tutorialKey(" .. tostring(self._tutorialKey) .. ")")
  end
...
function PaGlobal_ContentsTutorial_All:setInputTypeFunction(UIInputType)
  ...
  if UIInputType == __eUiInputType_Inventory then
    self._uiInputTypeFunc = Process_UIMode_CommonWindow_Inventory
  end
...
function PaGlobal_ContentsTutorial_All:executeTutorialData(func, funcParam, subCategoriIndex)
  ...
  if isSuccess == true then
    _PA_LOG("ì¡°ê´‘ë¯¼", "[SUCCESS] [" .. tostring(self._nextDataIndex - 1) .. "], subCategori(" .. tostring(subCategoriIndex) .. "), " .. tostring(func) .. "(" .. tostring(funcParam) .. ")")
  else
    _PA_LOG("ì¡°ê´‘ë¯¼", "[FAILED] [" .. tostring(self._nextDataIndex - 1) .. "], subCategori(" .. tostring(subCategoriIndex) .. "), " .. tostring(func) .. "(" .. tostring(funcParam) .. ")")
  end
  return isSuccess
end
...
function PaGlobal_ContentsTutorial_All:executeTargetControl(funcParam)
  ...
  if paramList[1] == "Item" then
    ...
    if self._isConsole == false then
      ...
      if self:targetControl(PaGlobal_Inventory_All_FindItemSlot(itemKeyRaw)) == false then
        return false
      end
      isTargetSuccess = self:focusControl()
    else
...
function PaGlobalFunc_ContentsTutorial_All_Open(fileName, tutorialKey)
  ...
  if PaGlobalFunc_DialogMain_All_Close ~= nil then
  ...
  local messageboxData = {
    ...
    content = PAGetString(Defines.StringSheet_GAME, "LUA_TUTORIAL_OBSIDIAN_TEXT_NEW_KR_29"),
...
function PaGlobalFunc_ContentsTutorial_All_CancleTutorial()
  local messageboxData = {
    title = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE"),
    content = PAGetString(Defines.StringSheet_GAME, "LUA_MARKETPLACE_TUTORIAL_EXIT_DESC"),
    functionYes = PaGlobalFunc_ContentsTutorial_All_Close,
    functionCancel = MessageBox_Empty_function,
    priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
  }
  MessageBox.showMessageBox(messageboxData)
