...
function PaGlobal_Equipment_All:controlInit()
  ...
  local stc_circle_original = UI.getChildControl(Panel_Window_Equipment_All, "Static_MainCircle")
  local stc_circle_artifact = UI.getChildControl(Panel_Window_Equipment_All, "Static_MainCircle_1")
  if true == _ContentsGroup_ArtifactItem then
    self._ui.stc_circle = stc_circle_artifact
    stc_circle_original:SetShow(false)
    Panel_Window_Equipment_All:SetSize(Panel_Window_Equipment_All:GetSizeX(), Panel_Window_Equipment_All:GetSizeY() + 62)
  else
    self._ui.stc_circle = stc_circle_original
    stc_circle_artifact:SetShow(false)
  end
...
function PaGlobal_Equipment_All:checkUsableSlot(index)
  ...
  if false == _ContentsGroup_ArtifactItem and (index == CppEnums.EquipSlotNo.artifact1 or index == CppEnums.EquipSlotNo.artifact2) then
    return false
  end
  return true
end
function PaGlobal_Equipment_All:setSlot()
  ...
  if true == _ContentsGroup_ArtifactItem then
    local txt_subTitle = UI.getChildControl(Panel_Window_Equipment_All, "Static_Text_SubTitle")
...
function PaGlobal_Equipment_All:registerEventHandler()
  ...
  if true == _ContentsGroup_SeasonContents then
    self._ui.stc_seasonBannerPass:addInputEvent("Mouse_LUp", "PaGlobalFunc_SeasonPassAndRoadMap_Open()")
