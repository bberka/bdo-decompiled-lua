function PaGlobal_Equipment_All:prepareOpen()
  ...
  if true == _ContentsGroup_Season_EquipmentUpgrade then
    if true == selfPlayer:get():isSeasonCharacter() and 10 <= selfPlayer:get():getLevel() then
      ...
      if true == equipInven or true == Inventory then
        if true == showTransferIcon then
          PaGlobal_Equipment_All._ui.stc_seasonBannerUpgrade:SetSpanSize(PaGlobal_Equipment_All._ui.stc_seasonBannerPass:GetSpanSize().x, 90)
          PaGlobal_Equipment_All._ui.stc_seasonBannerPass:SetSpanSize(PaGlobal_Equipment_All._ui.stc_seasonBannerPass:GetSpanSize().x, 180)
          PaGlobal_Equipment_All._ui.stc_seasonBannerReward:SetSpanSize(PaGlobal_Equipment_All._ui.stc_seasonBannerReward:GetSpanSize().x, 270)
        else
          PaGlobal_Equipment_All._ui.stc_seasonBannerUpgrade:SetSpanSize(PaGlobal_Equipment_All._ui.stc_seasonBannerPass:GetSpanSize().x, 0)
          PaGlobal_Equipment_All._ui.stc_seasonBannerPass:SetSpanSize(PaGlobal_Equipment_All._ui.stc_seasonBannerPass:GetSpanSize().x, 90)
          PaGlobal_Equipment_All._ui.stc_seasonBannerReward:SetSpanSize(PaGlobal_Equipment_All._ui.stc_seasonBannerReward:GetSpanSize().x, 180)
        end
  ...
  if isGameTypeGT() == false and isGameTypeKorea() == false then
      ...
      if nil ~= itemWrapper then
        ...
        if true == _ContentsGroup_Season_EquipmentUpgrade then
          if true == self._seasonUpgradeEffect then
            if true == itemSSW:get()._isChangeEquipItem then
        ...
        if true == isGameTypeKR2() and CppEnums.EquipSlotNo.avatarUnderWear == slotNo then
          slot.icon:SetShow(false)
          slotBG:SetShow(false)
