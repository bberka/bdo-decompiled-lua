...
local ArtifactsTooltip = {
  ...
  _equipmentSize = 0,
  _txt_emptySlot = nil,
  _equipNoMin = CppEnums.EquipSlotNo.artifact1,
  ...
  for ii = 1, 2 do
    ...
    if itemSSW ~= nil then
      ...
      if socketCount ~= 0 then
        ...
        local equipItemGrade = itemSSW:getGradeType()
        self:ChangeTextColor(equip.txt_equipName, equipItemGrade)
        ...
        for socketNo = 0, socketCount - 1 do
          ...
          if pushedItemSSW ~= nil then
            local nameColorGrade = pushedItemSSW:getGradeType()
            self:ChangeTextColor(equip.stc_lightStone[socketNo + 1].txt_lightStoneName, nameColorGrade)
