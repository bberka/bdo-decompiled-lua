...
function PaGlobal_DamageLog:open()
  ...
  if false == ToClient_IsDevelopment() then
    UI.ASSERT_NAME(false, "ë°ë¯¸ì§€ ë¡œê·¸ëŠ” Devì—ì„œë§Œ ì‚¬ìš© ê°€ëŠ¥ í•©ë‹ˆë‹¤.", "ë°•ê·œë‚˜")
    return
  end
  self:clear()
...
function PaGlobal_DamageLog:getAttackeeInfo(logMgrWrapper)
  ...
  if nil ~= logWrapper then
    ...
    if nil ~= className then
      txt_var = string.format("í”¼ê²©ì: %s(%d)", className, tmpVar)
      return txt_var
    end
  end
  ...
  if nil ~= logWrapper then
    ...
    if nil ~= characterSSW then
      txt_var = string.format("í”¼ê²©ì: %s(%d)", characterSSW:getName(), tmpVar)
      return txt_var
    end
  end
  return ""
...
function PaGlobal_DamageLog:updateBottomControls()
  ...
  local txt_desc = "ë°ë¯¸ì§€ í‰ê·  = " .. tostring(avgDamage) .. ", " .. tostring(totalHitCount) .. " ë²ˆ ì‹¤í–‰"
