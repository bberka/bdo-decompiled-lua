...
function HandleEventRUp_DeliveryRequest_SlotRClick(slotNo)
  UI.ASSERT_NAME(nil ~= slotNo, "HandleEventRUp_DeliveryRequest_SlotRClick slotNo nil", "ì´ìˆ˜ì¢…")
  if nil == Panel_Window_Delivery_Request then
    return
  end
...
function FromClient_DeliveryItemState(state)
  UI.ASSERT_NAME(nil ~= state, "FromClient_DeliveryItemState state nil", "ê¹€ì˜ì§„")
  if nil == Panel_Window_Delivery_Request then
    return
  end
  if 0 == state then
