...
function PaGlobal_Delivery_All:registerEventHandler()
  ...
  if true == isGameTypeKR2() then
    self._ui._pc.btn_question:SetShow(false)
  end
...
function PaGlobal_Delivery_All:sendRequest_Update()
  ...
  if -1 ~= self._selectCarriageKey[self._selectDeliverer] then
    ...
    self._ui.txt_weightValue:SetText(tostring(math.floor(remainWeight * 0,0001)) .. "/" .. tostring(maxCarriageWeight * 0,0001) .. PAGetString(Defines.StringSheet_RESOURCE, "TRADEMARKET_STD_TXT_LT"))
  else
    self._ui.txt_weightValue:SetText("0/" .. tostring(maxCarriageWeight * 0,0001) .. PAGetString(Defines.StringSheet_RESOURCE, "TRADEMARKET_STD_TXT_LT"))
  end
