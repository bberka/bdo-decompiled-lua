...
function PaGlobal_GuildPartyList_All:checkBlocked(index)
  if nil == index then
    _PA_ASSERT(false, "PaGlobal_GuildPartyList_All:checkBlocked .. íŒŒí‹° ëª¨ì§‘ì •ë³´ì˜ indexê°€ ì˜¬ë°”ë¥´ì§€ ì•ŠìŠµë‹ˆë‹¤.")
    return false
  end
...
function PaGlobal_GuildPartyList_All:checkAddAlready(index, isPartyLeader)
  if nil == index or nil == isPartyLeader then
    _PA_ASSERT(false, "PaGlobal_GuildPartyList_All:checkAddAlready .. index ë˜ëŠ” isPartyLeader ê°’ì´ ë“¤ì–´ì˜¤ì§€ ì•Šì•„ _canInvite ì„¤ì •ì´ ë˜ì§€ ì•ŠìŠµë‹ˆë‹¤.")
    return
  end
