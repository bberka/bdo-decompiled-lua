...
function pearlShop:initialize()
  ...
  local btn_class = UI.getChildControl(self._classFilter, "Button_Class")
  local stc_classSelectTitle = UI.getChildControl(self._classFilter, "StaticText_ClassSelectTitle")
  btn_class:SetShow(false)
  stc_classSelectTitle:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_INGAMESHOP_CLASSBASE"))
  local menuYSize = (btn_class:GetSizeY() + 8) * self._classCount + 20
  for i = 0, self._classCount do
    local productControl = UI.cloneControl(btn_class, self._classFilter, "Button_Class_" .. i)
    self._classButton[i] = productControl
    self._classButton[i]:SetPosY(btn_class:GetPosY() + i * (self._classButton[i]:GetSizeY() + 3))
  end
...
function pearlShop:getClassButtonData(classType)
  ...
  buttonData.className = CppEnums.ClassType2String[classType]
  if nil == buttonData.className or "" == buttonData.className then
    return nil
  end
  return buttonData
end
...
function pearlShop:getRealIndexByControlIndex(index)
  ...
  if nil == self._list[infoIndex] then
    UI.ASSERT_NAME(false, "infoIndexì— ëŒ€ì‘ë˜ëŠ” ì¸ë±ìŠ¤ê°€ ì—†ìŠµë‹ˆë‹¤.", "ì´ì£¼")
    return nil
  end
...
function pearlShop:updateControlInfo(index)
  local control = self:getProductControlByIndex(index)
  if control == nil then
    UI.ASSERT_NAME(false, "control ì •ë³´ë¥¼ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤.", "ì´ì£¼")
    return false
  end
  local info
  if self._isBuyHistroy == true then
    ...
    if realIndex == nil then
      control:SetShow(false)
      UI.ASSERT_NAME(false, "realIndex ê°’ì´ ë¹„ì •ìƒì…ë‹ˆë‹¤.", "ì´ì£¼")
      return false
    end
    local productNoRaw = ToClient_GetPurchaseItemCashProdcutNoByIndex(realIndex)
    if productNoRaw == nil then
      control:SetShow(false)
      UI.ASSERT_NAME(false, "productNoRaw ê°’ì´ ë¹„ì •ìƒì…ë‹ˆë‹¤.", "ì´ì£¼")
      return false
    end
