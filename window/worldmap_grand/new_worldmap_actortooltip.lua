...
local function showDropGroupControlInTooltip(itemExchangeSourceSS)
  if nil == itemExchangeSourceSS then
    _PA_ASSERT(false, "ItemExchangeSourceStaticStatusê°€ nil ì…ë‹ˆë‹¤.")
    return
  end
  local dropItemGroupCount = itemExchangeSourceSS:getDropGroupCount()
  if -1 == dropItemGroupCount then
    _PA_ASSERT(false, "getDropGroupCount()ê°€ -1 ì…ë‹ˆë‹¤.")
    return
  end
  if dropItemGroupCount > nodeResultDropGroupMaxCount then
    dropItemGroupCount = nodeResultDropGroupMaxCount
    _PA_ASSERT(false, "ìƒì‚° ë…¸ë“œì—ì„œ ìƒì‚°ë˜ëŠ” ì•„ì´í…œì˜ ì¢…ë¥˜ê°€ í‘œì‹œ ìµœëŒ€ì¹˜(" .. tostring(nodeResultDropGroupMaxCount) .. "ì¢…ë¥˜)ë¥¼ ë„˜ì—ˆìŠµë‹ˆë‹¤. ë” í‘œì‹œí•˜ë ¤ë©´ ë£¨ì•„ì½”ë“œë¥¼ ìˆ˜ì •í•´ì£¼ì„¸ìš”.")
  end
  for ii = 1, nodeResultDropGroupMaxCount do
  ...
  if -1 ~= dropItemGroupCount then
    for ii = 1, dropItemGroupCount do
      ...
      if nil == itemGroupValue then
        _PA_ASSERT(false, "getDropGroupByIndex()ì˜ ì¸ìê°€ ë²”ìœ„ë¥¼ ë²—ì–´ë‚¬ê±°ë‚˜ ItemExchangeSourceStaticStatusì˜ _productDropItemì •ë³´ê°€ ì—†ìŠµë‹ˆë‹¤.")
      end
...
local function setAdjustSizeDropGroupTooltip(dropGroupCount)
  if -1 == dropGroupCount then
    _PA_ASSERT(false, "getDropGroupCount()ì˜ ê°’ì´ ì˜ëª»ë˜ì—ˆìŠµë‹ˆë‹¤.")
    return
  end
...
function FromClient_OnWorldMapNode(nodeBtn)
  ...
  if nil ~= regionInfo then
    ...
    if nil ~= minorSiegeWrapper then
      ...
      elseif true == siegeWrapper:doOccupantExist() then
        ...
        villageWar.nodeWarJoinCount:SetShow(true)
  ...
  if nil ~= siegeNode then
    ...
    local txtSize = villageWar.nodeWarJoinCount:GetTextSizeY()
    local txtSizeDiff = txtSize - joinCountBaseSize
...
function FromClient_PartyIcon(partyMemberIcon, partyMemberProxy, index)
  ...
  elseif partyMemeberClassType == __eClassType_Nova then
    ...
    partyMember[partyCount].ClassIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    partyMember[partyCount].ClassIcon:setRenderTexture(partyMember[partyCount].ClassIcon:getBaseTexture())
  elseif partyMemeberClassType == __eClassType_Sage then
    partyMember[partyCount].ClassIcon:ChangeTextureInfoName(classTypeTexture)
    local x1, y1, x2, y2 = setTextureUV_Func(partyMember[partyCount].ClassIcon, 33, 422, 62, 451)
    partyMember[partyCount].ClassIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    partyMember[partyCount].ClassIcon:setRenderTexture(partyMember[partyCount].ClassIcon:getBaseTexture())
  elseif partyMemeberClassType == __eClassType_Corsair then
    partyMember[partyCount].ClassIcon:ChangeTextureInfoName(classTypeTexture)
    local x1, y1, x2, y2 = setTextureUV_Func(partyMember[partyCount].ClassIcon, 33, 453, 62, 482)
    partyMember[partyCount].ClassIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    partyMember[partyCount].ClassIcon:setRenderTexture(partyMember[partyCount].ClassIcon:getBaseTexture())
  elseif partyMemeberClassType == __eClassType_Warrior_Reserve2 then
    partyMember[partyCount].ClassIcon:ChangeTextureInfoName(classTypeTexture)
    local x1, y1, x2, y2 = setTextureUV_Func(partyMember[partyCount].ClassIcon, 33, 453, 62, 482)
    partyMember[partyCount].ClassIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    partyMember[partyCount].ClassIcon:setRenderTexture(partyMember[partyCount].ClassIcon:getBaseTexture())
  elseif partyMemeberClassType == __eClassType_ElfRanger_Reserved1 then
    partyMember[partyCount].ClassIcon:ChangeTextureInfoName(classTypeTexture)
    local x1, y1, x2, y2 = setTextureUV_Func(partyMember[partyCount].ClassIcon, 33, 453, 62, 482)
    partyMember[partyCount].ClassIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    partyMember[partyCount].ClassIcon:setRenderTexture(partyMember[partyCount].ClassIcon:getBaseTexture())
  elseif partyMemeberClassType == __eClassType_Drakania then
    partyMember[partyCount].ClassIcon:ChangeTextureInfoName(classTypeTexture)
    local x1, y1, x2, y2 = setTextureUV_Func(partyMember[partyCount].ClassIcon, 64, 174, 93, 203)
    partyMember[partyCount].ClassIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    partyMember[partyCount].ClassIcon:setRenderTexture(partyMember[partyCount].ClassIcon:getBaseTexture())
  elseif partyMemeberClassType == __eClassType_Giant_Reserved0 then
    partyMember[partyCount].ClassIcon:ChangeTextureInfoName(classTypeTexture)
    local x1, y1, x2, y2 = setTextureUV_Func(partyMember[partyCount].ClassIcon, 64, 174, 93, 203)
    partyMember[partyCount].ClassIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    partyMember[partyCount].ClassIcon:setRenderTexture(partyMember[partyCount].ClassIcon:getBaseTexture())
  elseif partyMemeberClassType == __eClassType_Giant_Reserved2 then
    partyMember[partyCount].ClassIcon:ChangeTextureInfoName(classTypeTexture)
    local x1, y1, x2, y2 = setTextureUV_Func(partyMember[partyCount].ClassIcon, 64, 174, 93, 203)
    partyMember[partyCount].ClassIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    partyMember[partyCount].ClassIcon:setRenderTexture(partyMember[partyCount].ClassIcon:getBaseTexture())
  elseif partyMemeberClassType == __eClassType_KunoichiOld then
    partyMember[partyCount].ClassIcon:ChangeTextureInfoName(classTypeTexture)
    local x1, y1, x2, y2 = setTextureUV_Func(partyMember[partyCount].ClassIcon, 64, 174, 93, 203)
    partyMember[partyCount].ClassIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    partyMember[partyCount].ClassIcon:setRenderTexture(partyMember[partyCount].ClassIcon:getBaseTexture())
