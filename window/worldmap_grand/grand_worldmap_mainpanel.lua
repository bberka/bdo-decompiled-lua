...
local worldmapGrand = {
  ui = {
    ...
    checkBtn_cameraSaveSlot = UI.getChildControl(Panel_WorldMap_Main, "CheckButton_WorldMapCamera"),
    btn_cameraSaveSlot_Template = UI.getChildControl(Panel_WorldMap_Main, "Button_Camera_SaveSlot_Templete"),
    btn_cameraSaveSlot = {},
    stc_cameraSaveSlotBG = UI.getChildControl(Panel_WorldMap_Main, "Static_Camera_SaveSlot_BG")
  },
  searchResultUiPool = {},
  template = {
...
worldmapGrand.ui.checkBtn_cameraSaveSlot:SetCheck(false)
worldmapGrand.ui.checkBtn_cameraSaveSlot:addInputEvent("Mouse_On", "CheckBtn_CameraSaveSlotTooltip( true )")
worldmapGrand.ui.checkBtn_cameraSaveSlot:addInputEvent("Mouse_Out", "CheckBtn_CameraSaveSlotTooltip( false )")
worldmapGrand.ui.checkBtn_cameraSaveSlot:addInputEvent("Mouse_LUp", "HandleEventLUp_CheckBtn_CameraSaveSlot()")
worldmapGrand.ui.stc_cameraSaveSlotBG:SetShow(false)
local dayString = {
...
function worldmapGrand:territoryFiltterSetPosWithMajorSiege()
  ...
  self.ui.stc_TerritoryInfo:SetPosX(self.ui.stc_SiegeTerritoryFilterBG:GetPosX())
  self.ui.stc_TerritoryInfo:SetPosY(self.ui.stc_SiegeTerritoryFilterBG:GetPosY() + self.ui.btn_SiegeTerritoryFilter:GetSizeX() + buttonPosY + padding * 2)
...
function CreateBtn_CameraSaveSlots()
  if worldmapGrand.ui.btn_cameraSaveSlot[0] ~= nil then
    return
  end
  local initPosY = worldmapGrand.ui.checkBtn_cameraSaveSlot:GetPosY()
  for i = 0, worldmapGrand._cameraSaveSlotCount - 1 do
    worldmapGrand.ui.btn_cameraSaveSlot[i] = UI.cloneControl(worldmapGrand.ui.btn_cameraSaveSlot_Template, Panel_WorldMap_Main, "Button_Camera_SaveSlot_" .. i)
    worldmapGrand.ui.btn_cameraSaveSlot[i]:SetPosY(initPosY)
    worldmapGrand.ui.btn_cameraSaveSlot[i]:addInputEvent("Mouse_LUp", "HandleEventLUp_Btn_CameraSaveSlot(" .. i .. ")")
    worldmapGrand.ui.btn_cameraSaveSlot[i]:addInputEvent("Mouse_On", "Btn_CameraSaveSlotTooltip( true, " .. i .. ")")
    worldmapGrand.ui.btn_cameraSaveSlot[i]:addInputEvent("Mouse_Out", "Btn_CameraSaveSlotTooltip( false, " .. i .. ")")
    worldmapGrand.ui.btn_cameraSaveSlot[i]:SetText(i)
  end
end
function CheckBtn_CameraSaveSlotTooltip(isShow)
  if isShow then
    local btn = worldmapGrand.ui.checkBtn_cameraSaveSlot
...
function Btn_CameraSaveSlotTooltip(isShow, index)
  ...
  if isShow then
    local btn = worldmapGrand.ui.btn_cameraSaveSlot[index]
...
function UpdateBtn_WorldMapCameraSaveSlotMonoTone(index)
  if ToClient_IsSetWorldMapQuickScreenPosition(index) == true then
    worldmapGrand.ui.btn_cameraSaveSlot[index]:SetMonoTone(false)
  else
    worldmapGrand.ui.btn_cameraSaveSlot[index]:SetMonoTone(true)
  end
end
function HandleEventLUp_CheckBtn_CameraSaveSlot()
  if worldmapGrand.ui.stc_cameraSaveSlotBG:GetShow() == true then
    worldmapGrand.ui.stc_cameraSaveSlotBG:SetShow(false)
    local initPosY = worldmapGrand.ui.checkBtn_cameraSaveSlot:GetPosY()
    for i = 0, worldmapGrand._cameraSaveSlotCount - 1 do
      local btn = worldmapGrand.ui.btn_cameraSaveSlot[i]
      local ani = btn:addMoveAnimation(0, 0,15, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
      ani:SetStartPosition(btn:GetPosX(), btn:GetPosY())
      ani:SetEndPosition(btn:GetPosX(), initPosY)
      ani:SetDisableWhileAni(true)
      ani:SetHideAtEnd(true)
    end
  else
    worldmapGrand.ui.stc_cameraSaveSlotBG:SetShow(true)
    local startY = worldmapGrand.ui.btn_cameraSaveSlot_Template:GetPosY()
    local sizeY = worldmapGrand.ui.btn_cameraSaveSlot_Template:GetSizeY()
    local offsetY = sizeY + 3
    local posIndex = 0
    for i = 0, worldmapGrand._cameraSaveSlotCount - 1 do
      local btn = worldmapGrand.ui.btn_cameraSaveSlot[i]
      btn:SetShow(true)
      ...
      if i == 0 then
        posIndex = worldmapGrand._cameraSaveSlotCount - 1
      else
        posIndex = i - 1
      end
      local ani = btn:addMoveAnimation(0, 0,15, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
      ani:SetStartPosition(btn:GetPosX(), btn:GetPosY())
      ani:SetEndPosition(btn:GetPosX(), startY + offsetY * posIndex)
    end
  end
end
