function PaGlobal_WorldMapNodeTooltip_VillageWar:initialize()
  ...
  self._ui._txt_specialAttackLimit = UI.getChildControl(limitStatBG, "StaticText_Reg_Special")
  self._ui._txt_mySpecialAttackLimit = UI.getChildControl(limitStatBG, "StaticText_Reg_Special_Mystat")
  ...
  self._limitSpecialAttackPos = self._ui._txt_specialAttackLimit:GetPosY()
  self._mySpecialAttackPos = self._ui._txt_mySpecialAttackLimit:GetPosY()
  ...
  self._ui._txt_specialAttackLimit:SetTextMode(__eTextMode_AutoWrap)
  self._ui._txt_mySpecialAttackLimit:SetTextMode(__eTextMode_AutoWrap)
  ...
  if nil ~= siegeNode then
    ...
    if true == _ContentsGroup_SeigeSeason5 then
      ...
      if true == _ContentsGroup_NewCharacterStatInfo then
        ...
        local limitresistanceAddedAttack = ToClient_getLimitSiegeResistanceAddedAttack(siegeNode._regionKey:get())
        local myResistanceAddedAttack = ToClient_getBonusBuffStat(__eSendToClientBuffStatType_ResistanceAddedAttack, 0)
        if limitresistanceAddedAttack >= 0 and limitresistanceAddedAttack < CppDefine.e100Percent then
          local statString = PaGlobal_Window_Stat_All:getRateText(limitresistanceAddedAttack)
          self._ui._txt_specialAttackLimit:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_NODETOOLTIP_STATLIMIT_REG_SPECIAL", "special", statString .. "%"))
        else
          self._ui._txt_specialAttackLimit:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_NODETOOLTIP_STATLIMIT_REG_SPECIAL", "special", "-"))
        end
        self._ui._txt_mySpecialAttackLimit:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_NODETOOLTIP_STATLIMIT_REG_SPECIAL_MYSTAT", "special", GetRateLimitFontColor(limitresistanceAddedAttack, myResistanceAddedAttack)))
        ...
        if true == _ContentsGroup_SiegeHitAndResistLimit then
          ...
          if true == _ContentsGroup_SiegeResistanceAddedAttackLimit then
            self._ui._txt_specialAttackLimit:SetPosY(self._limitSpecialAttackPos + wrapCount * padding)
            if self._defaultTextSizeY < self._ui._txt_specialAttackLimit:GetTextSizeY() then
              wrapCount = wrapCount + 1
            end
            self._ui._txt_mySpecialAttackLimit:SetPosY(self._mySpecialAttackPos + wrapCount * padding)
            if self._defaultTextSizeY < self._ui._txt_mySpecialAttackLimit:GetTextSizeY() then
              wrapCount = wrapCount + 1
            end
            self._ui._txt_siegeLimitDesc:SetPosY(self._siegeLimitDescBasePos + wrapCount * padding)
          else
            self._ui._txt_specialAttackLimit:SetShow(false)
            self._ui._txt_mySpecialAttackLimit:SetShow(false)
            self._ui._txt_siegeLimitDesc:SetPosY(self._limitSpecialAttackPos + wrapCount * padding)
          end
        else
          ...
          if true == _ContentsGroup_SiegeResistanceAddedAttackLimit then
            self._ui._txt_specialAttackLimit:SetPosY(self._hitLimitNewBasePos + wrapCount * padding)
            if self._defaultTextSizeY < self._ui._txt_specialAttackLimit:GetTextSizeY() then
              wrapCount = wrapCount + 1
            end
            self._ui._txt_mySpecialAttackLimit:SetPosY(self._myHitStatBasePos + wrapCount * padding)
            if self._defaultTextSizeY < self._ui._txt_mySpecialAttackLimit:GetTextSizeY() then
              wrapCount = wrapCount + 1
            end
            self._ui._txt_siegeLimitDesc:SetPosY(self._hitRateLimitNewBasePos + wrapCount * padding)
          else
            self._ui._txt_specialAttackLimit:SetShow(false)
            self._ui._txt_mySpecialAttackLimit:SetShow(false)
