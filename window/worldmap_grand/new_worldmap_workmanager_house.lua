...
function Work_Estimated_Update(isWorkOver)
  _PA_LOG("ì •íƒœê³¤", "111isWorkOver : " .. tostring(isWorkOver))
  local rowIndex = Work_List._selected_Data_Row
  local collumIndex = Work_List._selected_Data_Collum
  if nil == rowIndex or nil == collumIndex then
    return
  end
  _PA_LOG("ì •íƒœê³¤", "222ë°–ë°–ë°–ë°–ë°–ë°–ë°–ë°–ë°–rowIndex : " .. tostring(rowIndex) .. " / collumIndex : " .. tostring(collumIndex))
  if isWorkOver then
    _PA_LOG("ì •íƒœê³¤", "333ì•ˆì•ˆì•ˆì•ˆì•ˆì•ˆì•ˆì•ˆrowIndex : " .. tostring(rowIndex) .. " / collumIndex : " .. tostring(collumIndex))
    _PA_LOG("ì •íƒœê³¤", "444Work_List._over_Index : " .. tostring(Work_List._over_Index))
    rowIndex = Work_List._dataIndex[Work_List._over_Index]._rowIndex
    collumIndex = Work_List._dataIndex[Work_List._over_Index]._collumIndex
  end
  local distance = 0
...
function HandleClicked_WorkCount_House()
  ...
  if s64_MaxWorkableCount <= toInt64(0, 0) then
    _PA_LOG("ì´ë¬¸ì¢…", "ì¼ê¾¼ì´ ì‘ì—…í•  ìˆ˜ ì—†ìŠµë‹ˆë‹¤.")
  else
