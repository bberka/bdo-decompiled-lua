...
function commonInfoSection:SetInfo(territoryInfo, territoryKeyRaw)
  ...
  elseif true == occupyBenefit.sectionBG:GetShow() then
    ...
    self.warInfo:SetSpanSize(30, 340 + addSize)
  end
  if true == _ContentsGroup_BattleFieldVolunteer then
...
function StatLimitOpen(territoryInfo)
  ...
  if limitResistanceAddedAttack >= 0 and limitResistanceAddedAttack < CppDefine.e100Percent then
    ...
    limitStat._limitResistanceAddedAttack:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_NODETOOLTIP_STATLIMIT_REG_SPECIAL", "special", "-"))
  end
  ...
  if true == _ContentsGroup_SiegeHitAndResistLimit then
    ...
    if true == _ContentsGroup_SiegeResistanceAddedAttackLimit then
      limitStat._limitResistanceAddedAttack:SetPosY(limitStat._limitResistanceAddedAttackPosY + wrapCount * padding)
      if limitStat._defaultTextSizeY < limitStat._limitResistanceAddedAttack:GetTextSizeY() then
        wrapCount = wrapCount + 1
      end
      limitStat._limitMyResistanceAddedAttack:SetPosY(limitStat._limitMyResistanceAddedAttackPosY + wrapCount * padding)
      if limitStat._defaultTextSizeY < limitStat._limitMyResistanceAddedAttack:GetTextSizeY() then
        wrapCount = wrapCount + 1
      end
      ...
      limitStat._desc:SetPosY(limitStat._limitResistanceAddedAttackPosY + wrapCount * padding)
    end
  else
    ...
    if true == _ContentsGroup_SiegeResistanceAddedAttackLimit then
      limitStat._limitResistanceAddedAttack:SetPosY(limitStat._limitHitPosY + wrapCount * padding)
      if limitStat._defaultTextSizeY < limitStat._limitResistanceAddedAttack:GetTextSizeY() then
        wrapCount = wrapCount + 1
      end
      limitStat._limitMyResistanceAddedAttack:SetPosY(limitStat._limitMyHitPosY + wrapCount * padding)
      if limitStat._defaultTextSizeY < limitStat._limitMyResistanceAddedAttack:GetTextSizeY() then
        wrapCount = wrapCount + 1
      end
      limitStat._desc:SetPosY(limitStat._limitHitRatePosY + wrapCount * padding)
    else
      limitStat._limitResistanceAddedAttack:SetShow(false)
      limitStat._limitMyResistanceAddedAttack:SetShow(false)
