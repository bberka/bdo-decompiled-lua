...
function PaGlobal_GrowthPass:initialize_mainArea()
  ...
  _PA_ASSERT(nil ~= originalQuestButtonTemplate, "ë¹„ì •ìƒì…ë‹ˆë‹¤.")
  ...
  for key, value in pairs(self._ui._categoryButtonList) do
    ...
    if nil ~= layoutWrapper then
        ...
        for col = 0, totalColCount - 1 do
          ...
          if nil ~= cellWrapper then
            ...
            elseif __eGrowthPass_CellType_MiniVertical == cellData.type then
              ...
              _PA_ASSERT(false, "cellTypeì´ ë¹„ì •ìƒ ì…ë‹ˆë‹¤. key:" .. tostring(categoryKeyRaw) .. "/row:" .. tostring(row) .. "/col:" .. tostring(col) .. "/type:" .. tostring(cellData.type))
            end
      ...
      _PA_ASSERT(false, "ë ˆì´ì•„ì›ƒ ì •ë³´ê°€ nullptrì…ë‹ˆë‹¤. í™•ì¸ë¶€íƒë“œë¦½ë‹ˆë‹¤. [key:" .. tostring(categoryKeyRaw) .. "]")
    end
  end
  ...
  for totalRewardIndex = 0, totalRewardCount - 1 do
    ...
    if nil ~= totalRewardSSW then
      ...
      if nil ~= totalRewardBaseItemSSW then
        ...
        _PA_ASSERT(false, tostring(totalRewardIndex + 1) .. "ë²ˆ ì§¸ ì§„í–‰ë„ ë³´ìƒì˜ BaseReward ì •ë³´ê°€ ë¹„ì •ìƒì…ë‹ˆë‹¤. (Key:" .. tostring(totalRewardData.totalRewardKey:get()) .. ")")
      end
    else
      _PA_ASSERT(false, tostring(totalRewardIndex + 1) .. "ë²ˆ ì§¸ ì§„í–‰ë„ ë³´ìƒ ì•„ì´í…œ ì •ë³´ê°€ ë¹„ì •ìƒì…ë‹ˆë‹¤. (Key:" .. tostring(totalRewardData.totalRewardKey:get()) .. ")")
    end
...
function PaGlobalFunc_GrowthPass_Animation_Update(deltaTime)
  ...
  if self._animationFrame > 0,0666666666666667 and self._animationFrame < 1 then
...
function PaGlobal_GrowthPass:updateCategoryLayout()
  ...
  if self._categoryCount <= self._currentSelectedCategoryIndex or self._currentSelectedCategoryIndex < 0 then
    _PA_ASSERT(false, "ì¹´í…Œê³ ë¦¬ ì¸ë±ìŠ¤ ê°’ì´ ë¹„ì •ìƒì…ë‹ˆë‹¤.")
    return
  end
  self._ui._frame_mainBG:UpdateContentScroll()
...
function PaGlobal_GrowthPass:showLayout(index, isShow)
  ...
  if nil == categoryData then
    _PA_ASSERT(false, "ì¹´í…Œê³ ë¦¬ ë°ì´í„°ë¥¼ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤.")
    return
  end
  ...
  if nil == layoutWrapper then
    _PA_ASSERT(false, "ì¹´í…Œê³ ë¦¬ wrapperë¥¼ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤.")
    return
  end
  local layoutList = self._ui._categoryLayoutCellList[keyRaw]
  if nil == layoutList then
    _PA_ASSERT(false, "ì¹´í…Œê³ ë¦¬ ë ˆì´ì•„ì›ƒì„ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤.")
    return
  end
  local totalRowCount = layoutWrapper:getRowCount()
  local totalColCount = layoutWrapper:getColCount()
  ...
  for row = 0, totalRowCount - 1 do
    for col = 0, totalColCount - 1 do
      ...
      if nil ~= cellRef then
        ...
        _PA_ASSERT(false, "ì¹´í…Œê³ ë¦¬ ì…€ ë°ì´í„°ë¥¼ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤.[" .. tostring(keyRaw) .. "][" .. tostring(row) .. "][" .. tostring(col) .. "]")
      end
    end
  end
  ...
  for key, value in pairs(self._ui._categoryButtonList) do
    if self._currentSelectedCategoryIndex == key then
      ...
      if nil == currentCategorySSW then
        _PA_ASSERT(false, "[Lua] ì„±ì¥íŒ¨ìŠ¤ ì¹´í…Œê³ ë¦¬ ê³ ì •ì •ë³´ë¥¼ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤. í™•ì¸ ë¶€íƒë“œë¦½ë‹ˆë‹¤. (CategoryKey:" .. tostring(categoryKeyRaw) .. ")")
      end
      local layoutWrapper = ToClient_getGrowthPassLayoutWrapper(value.categoryKey)
      local categoryKeyRaw = value.categoryKey:get()
      if nil ~= layoutWrapper and nil ~= currentCategorySSW then
        ...
        for rewardIndex = 0, self._categoryRewardCount - 1 do
          ...
          if nil == rewardItemWrapper then
            _PA_ASSERT(false, "[Lua] ì„±ì¥íŒ¨ìŠ¤ ì¹´í…Œê³ ë¦¬ì˜ ë³´ìƒ ì•„ì´í…œ ê³ ì •ì •ë³´ë¥¼ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤. í™•ì¸ ë¶€íƒë“œë¦½ë‹ˆë‹¤. (CategoryKey:" .. tostring(categoryKeyRaw) .. ")")
            return
          end
