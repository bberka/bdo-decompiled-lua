...
function PaGlobal_Login_All:initLoginImage()
  ...
  if true == isGameTypeKR2() then
    local CHTable = {
      ...
      [4] = "New_UI_Common_forLua/Window/Lobby/bgCalpeon_6.dds"
    }
    for index = 1, 4 do
...
function PaGlobal_Login_All:shuffleOrder(table)
  _PA_LOG("ì •íƒœê³¤", "table : " .. tostring(table))
...
function PaGlobal_Login_All:onScreenResize()
  ...
  if isGameTypeKorea() or isGameTypeTaiwan() or isGameTypeGT() or isGameTypeTR() or isGameTypeTH() or isGameTypeID() or true == self._isConsole or isGameTypeRussia() or isGameTypeJapan() or isGameTypeEnglish() or isGameTypeSA() then
...
function PaGlobal_Login_All:enterLogin()
  connectToGame(self._ui._edit_id:GetEditText(), "ê²€ì€ì „ì‚¬ë¹„ë²ˆ")
end
