function PaGlobal_ServerSelect_All:initialize()
  ...
  if true == _ContentsGroup_DracaniaAwaken then
    ...
    self._movieURL = {
      ...
      "coui://UI_Movie/CS/Loading/Login/login_08.webm",
      "coui://UI_Movie/CS/Loading/Login/login_09.webm",
      "coui://UI_Movie/CS/Loading/Login/login_10.webm"
    }
    self._movieOrder = {
      1,
      2,
  ...
  if true == _ContentsGroup_DracaniaAwaken then
    self._currentMovieIndex = getRandomValue(1, 10)
  end
  if false == _ContentsGroup_MountainOfEternalWinter then
  ...
  if true == isGameTypeKR2() then
    ...
    self:setResizeResolution()
  elseif true == _ContentsGroup_UseVideoContent then
...
function PaGlobal_ServerSelect_All:initDesriptionData()
  ...
  if _ContentsGroup_HadumCalpheon == true then
    self._channelDescriptionData[self._CHANNEL_TYPE.HADUM].desc = PAGetString(Defines.StringSheet_GAME, "LUA_SERVERSELECT_HADUM_DESC_CAL")
  end
  if true == self._isConsole then
  ...
  for ii = 1, worldServerCount do
    ...
    for jj = 1, channelCount do
      ...
      if 0 ~= restrictedServerNo and restrictedServerNo == channelServerData._serverNo then
        ...
        self._restrictedServerData[ii].guildServerGroupNo = channelServerData._guildServerGroupNo
      end
...
function PaGlobal_ServerSelect_All:createChannelBranch(content, key)
  ...
  _PA_ASSERT(nil ~= channelName, "ì„œë²„ ì´ë¦„ì€ ì¡´ì¬í•´ì•¼í•©ë‹ˆë‹¤.")
  local stateStr = ""
  ...
  if key32 == self._currentHoveredKey32 then
    ...
    btn_enterance:SetShow(enteranceButtonShow or isGameServiceTypeDev())
    if true == self._ani._startAnimationFinished then
...
function PaGlobal_ServerSelect_All:setMainServer(worldIndex, serverIndex)
  ...
  if nil == serverData then
    Proc_ShowMessage_Ack("ì˜ëª»ëœ ì„œë²„ ì •ë³´ì…ë‹ˆë‹¤. ë‹¤ì‹œ ì‹œë„í•´ì£¼ì„¸ìš”.")
    return
  end
...
function PaGlobal_ServerSelect_All:initBanners()
  ...
  if nil == domainURL or "" == domainURL then
    domainURL = "https://dev-game-portal.xbox.playblackdesert.com/Banner"
  else
    domainURL = "https://" .. domainURL .. "/Ingame/Banner"
  end
