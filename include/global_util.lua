...
function utcTimestampToDate(timestamp, isTimeZone)
  ...
  datetime._second = secondInDay % 60
  secondInDay = getQuotient(60, secondInDay)
  datetime._minute = secondInDay % 60
  secondInDay = getQuotient(60, secondInDay)
  datetime._hour = secondInDay % 24
  ...
  datetime._year = year
  datetime._weekday = timestampToWeekday(timestamp)
  datetime._week = getQuotient(7, days + 11 - datetime._weekday)
  if true == isLeapYear(datetime._year) then
    monthDays[2] = 29
  else
    monthDays[2] = 28
  end
  ...
  datetime._month = month
  datetime._day = days + 1
  return datetime
end
...
function PaGlobalFunc_Util_IsSuccessionContentsOpen(classType)
  ...
  local successionWeapon = {
    ...
    [__eClassType_ElfRanger_Reserved1] = false,
    [__eClassType_ShyMan] = false,
    [__eClassType_BladeMaster_Reserved1] = false,
...
function PaGlobalFunc_Util_CheckSeasonEquip(itemStaticWrapper)
  ...
  if nil == selfPlayer or nil == itemStaticWrapper then
    UI.ASSERT_NAME(nil ~= selfPlayer, "selfPlayerê°€ nilì´ë©´ ì•ˆ ë©ë‹ˆë‹¤.", "ìš°ì •ë¬´")
    UI.ASSERT_NAME(nil ~= itemStaticWrapper, "itemStaticWrapperê°€ nilì´ë©´ ì•ˆ ë©ë‹ˆë‹¤.", "ìš°ì •ë¬´")
  end
...
function PaGlobalFunc_Util_VehicleEquipNoToSlotNo(vehicleEquipType, slotNo, isPearl)
  ...
  if nil == equipNoList then
    _PA_ASSERT(false, "EquipNoë¥¼ ê°€ì ¸ì˜¤ë˜ ì¤‘ vehicleEquipType ì´ ë¹„ì •ìƒì…ë‹ˆë‹¤." .. tostring(vehicleEquipType))
    return -1
  end
...
function PaGlobalFunc_Util_getVehicleSlotNoToEquipNo(vehicleEquipType, equipNo, isPearl)
  ...
  if nil == equipNoList then
    _PA_ASSERT(false, "íƒ‘ìŠ¹ë¬¼ EquipNoë¥¼ êµ¬ì„±í•˜ì§€ ëª»í–ˆìŠµë‹ˆë‹¤. í™•ì¸ë°”ëë‹ˆë‹¤." .. tostring(vehicleEquipType))
    return -1
  end
  if nil == equipNoList[vehicleEquipType] then
    _PA_ASSERT(false, "íƒ‘ìŠ¹ë¬¼ EquipNoë¥¼ ê°€ì ¸ì˜¤ë˜ ì¤‘ vehicleEquipType ì´ ë¹„ì •ìƒì…ë‹ˆë‹¤." .. tostring(vehicleEquipType))
    return -1
  end
  ...
  if false then
    if price_s64 >= number1 then
      ...
      if "0" ~= string.sub(tostring(price_s64), length - 7, length - 7) then
        replaceNumber = replaceNumber .. string.sub(tostring(price_s64), length - 7, length - 4) .. "ë§Œ"
      elseif "0" ~= string.sub(tostring(price_s64), length - 6, length - 6) then
        replaceNumber = replaceNumber .. string.sub(tostring(price_s64), length - 6, length - 4) .. "ë§Œ"
      elseif "0" ~= string.sub(tostring(price_s64), length - 5, length - 5) then
        replaceNumber = replaceNumber .. string.sub(tostring(price_s64), length - 5, length - 4) .. "ë§Œ"
      elseif "0" ~= string.sub(tostring(price_s64), length - 4, length - 4) then
        replaceNumber = replaceNumber .. string.sub(tostring(price_s64), length - 4, length - 4) .. "ë§Œ"
      end
    end
