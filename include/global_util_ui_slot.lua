...
function SlotItem:setItemByStaticStatus(itemStaticWrapper, s64_stackCount, expirationIndex, isBroken, isCash, isSoulCollecTor, soulCount, soulMax, isWidget, isWorldMarket, isSealed, isFamilyInven, isOpenedByLightStoneBag, isForceShowStackCount)
  ...
  if nil ~= self.remoteEnchantText then
    ...
    elseif itemStatic:isEquipable() and 16 == enchantLevel then
      ...
      if true == _ContentsGroup_KoreanEnchantGrade then
        self.remoteEnchantText:SetText("ì¥")
      elseif isKR2 then
        self.remoteEnchantText:SetText("é•¿")
      end
      self.remoteEnchantText:SetShow(true)
    elseif itemStatic:isEquipable() and 17 == enchantLevel then
      self.remoteEnchantText:SetText("II")
      if true == _ContentsGroup_KoreanEnchantGrade then
        self.remoteEnchantText:SetText("ê´‘")
      elseif isKR2 then
        self.remoteEnchantText:SetText("å¹¿")
      end
      self.remoteEnchantText:SetShow(true)
    elseif itemStatic:isEquipable() and 18 == enchantLevel then
      ...
      if true == _ContentsGroup_KoreanEnchantGrade then
        self.remoteEnchantText:SetText("ê³ ")
      elseif isKR2 then
        self.remoteEnchantText:SetText("æ•…")
      end
      self.remoteEnchantText:SetShow(true)
    elseif itemStatic:isEquipable() and 19 == enchantLevel then
      self.remoteEnchantText:SetText("IV")
      if true == _ContentsGroup_KoreanEnchantGrade then
        self.remoteEnchantText:SetText("ìœ ")
      elseif isKR2 then
        self.remoteEnchantText:SetText("ç‰")
      end
      self.remoteEnchantText:SetShow(true)
    elseif itemStatic:isEquipable() and 20 == enchantLevel then
      self.remoteEnchantText:SetText("V")
      if true == _ContentsGroup_KoreanEnchantGrade then
        self.remoteEnchantText:SetText("ë™")
      elseif isKR2 then
        self.remoteEnchantText:SetText("ä¸œ")
      end
      self.remoteEnchantText:SetShow(true)
    else
...
function SlotItem:getCountStringByCount64_pc(count64)
  ...
  if true == self.param.isTranslation then
    ...
    elseif "number" == type(count64) then
      if count64 >= 1000000 then
        strCount = tostring(math.floor(count64 * 1E-06)) .. "." .. tostring(math.floor(count64 * 1E-05 - math.floor(count64 * 1E-06) * 10)) .. "M"
