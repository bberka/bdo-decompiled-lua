...
function registerClosePanelList()
  ...
  if true == _ContentsGroup_OldMoonSmelting then
    registerCloseLuaEvent(Panel_Window_OldMoonSmelting, closeTypeBitSet.default, "PaGlobalFunc_OldMoonSmelting_Close()")
  end
...
function initCloseFunction()
  _PA_LOG("ì´ë‹¤í˜œ", "initCloseFunction Start")
...
function checkAllPanelSetCloseFunction()
  ...
  if true ~= result then
    UI.ASSERT(false, " íŒ¨ë„ì— ,close í•¨ìˆ˜ê°€ ì…‹íŒ…ë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤ !! close ê°€ ì •ìƒì‘ë™í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤ ")
    UI.ASSERT(false, " í•´ë‹¹ íŒ¨ë„ì„ globalCloseManager_Renew.lua ì˜ˆì™¸ ë¦¬ìŠ¤íŠ¸ì— ì¶”ê°€í•˜ê±°ë‚˜ close í•¨ìˆ˜ë¥¼ ë“±ë¡í•´ì£¼ì„¸ìš” ")
  end
  _PA_LOG("ì´ë‹¤í˜œ", "ìœ íš¨ì„± ê²€ì‚¬ ë")
end
