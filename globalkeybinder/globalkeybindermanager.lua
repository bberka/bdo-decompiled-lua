...
function PaGlobal_GlobalKeyBinder:Update(deltaTime)
  ...
  local isSolare = nil ~= Panel_Window_Solrare_Matching and Panel_Window_Solrare_Matching:GetShow() == true or nil ~= Panel_Popup_Solare_MatchConfirm and Panel_Popup_Solare_MatchConfirm:GetShow() == true or ToClient_IsMatchingStateByMatchServer() == true
  if UIMode.eUIMode_KeyCustom_ButtonShortcuts ~= GetUIMode() and UIMode.eUIMode_DeadMessage ~= GetUIMode() and false == ToClient_IsRemoteInstallMode() and false == isSolare then
...
function SetUIMode(uiMode)
  if uiMode >= 0 and uiMode < UIMode.eUIMode_Count then
    if true == ToClient_IsDevelopment() then
      _PA_LOG("ë°•ê´‘ìš´", "======================================")
      _PA_LOG("ë°•ê´‘ìš´", "prev UIMode = " .. tostring(_uiMode))
      _PA_LOG("ë°•ê´‘ìš´", "cur UIMode = " .. tostring(uiMode))
    end
    _uiMode = uiMode
  end
end
