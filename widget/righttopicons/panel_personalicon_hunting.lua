  ...
  if griffonCount > 0 then
    for index = 0, griffonCount - 1 do
      ...
      if nil ~= areaName then
        if 0 == index then
          if "" == msg.desc then
            msg.desc = msg.desc .. "[" .. tostring(areaName) .. "]ì— ì œì™• ê·¸ë¦¬í°ì´ " .. count .. " ë§ˆë¦¬ ì¶œí˜„í•˜ì˜€ìŠµë‹ˆë‹¤."
          else
            msg.desc = msg.desc .. "\n" .. "[" .. tostring(areaName) .. "]ì— ì•¼ìƒì˜ ì œì™• ê·¸ë¦¬í°ì´ " .. count .. " ë§ˆë¦¬ ì¶œí˜„í•˜ì˜€ìŠµë‹ˆë‹¤."
          end
        else
          msg.desc = msg.desc .. "\n" .. "[" .. tostring(areaName) .. "]ì— ì œì™• ê·¸ë¦¬í°ì´ " .. count .. " ë§ˆë¦¬ ì¶œí˜„í•˜ì˜€ìŠµë‹ˆë‹¤."
        end
      end
    end
    msg.name = PAGetString(Defines.StringSheet_GAME, "LUA_MOVIEGUIDE_WHALE_FIND_NAME")
