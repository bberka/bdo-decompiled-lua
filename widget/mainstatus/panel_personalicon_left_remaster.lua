...
function PersonalIcon:InitControl()
  ...
  self._iconControl[self._iconType.TransferAccountBuff] = UI.getChildControl(_panel, "Static_TransferEventBuff")
...
function PersonalIcon:unRenderPanel()
  if nil == Panel_PersonalIcon_Left then
    _PA_ASSERT_NAME(false, "Panel_PersonalIcon_Left nil ì…ë‹ˆë‹¤.", "ê¹€ê·¼ìš°")
    return
  end
...
function PersonalIcon:registEventHandler()
  ...
  if ToClient_GetIsTransferAccount() == false then
    self._iconControl[self._iconType.TransferAccountBuff]:addInputEvent("Mouse_LUp", "PaGlobal_SA_Common_TransferLink()")
  end
...
function PersonalIcon:update()
  ...
  local applyTransferAccountBuff = ToClient_GetIsTransferAccount()
  ...
  if _ContentsGroup_TransferEventBuff == true then
    self._iconControl[self._iconType.TransferAccountBuff]:SetShow(true)
    self._iconControl[self._iconType.TransferAccountBuff]:SetMonoTone(not applyTransferAccountBuff)
  end
  self:updatePos()
...
function InputMO_PersonalIcon_ShowTooltip(isShow, iconType)
  ...
  elseif iconType == self._iconType.TransferAccountBuff then
    name = PAGetString(Defines.StringSheet_GAME, "LUA_TRANSFER_NA_BUFF_TOOLTIP_NAME")
    if ToClient_GetIsTransferAccount() == true then
      desc = PAGetString(Defines.StringSheet_GAME, "LUA_TRANSFER_NA_BUFF_TOOLTIP_DESC_AFTER")
    else
      desc = PAGetString(Defines.StringSheet_GAME, "LUA_TRANSFER_NA_BUFF_TOOLTIP_DESC_BEFORE")
    end
    uiControl = self._iconControl[self._iconType.TransferAccountBuff]
  end
