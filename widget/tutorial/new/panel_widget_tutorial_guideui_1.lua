...
function PaGlobal_Widget_Tutorial_GuideUI:startEvent(tutorialSSW)
  _PA_ASSERT(nil ~= tutorialSSW, "ë¹„ì •ìƒì…ë‹ˆë‹¤.")
  ...
  if true == useBlackBG then
    ...
    if nil ~= panel then
      ...
      if nil ~= control then
        ...
        _PA_ASSERT(false, "BlackBGì—ì„œ ì°¾ì„ ChildControlì´ ì—†ìŠµë‹ˆë‹¤.")
      end
    else
      _PA_ASSERT(false, "BlackBGì— ì ìš©í•  Panelì„ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤.")
    end
    ...
    if nil ~= findControl then
      ...
      if nil ~= panel then
        ...
        if nil ~= control then
          ...
          _PA_ASSERT(false, "WhiteBGì—ì„œ ì°¾ì„ ChildControlì´ ì—†ìŠµë‹ˆë‹¤.")
        end
      else
        _PA_ASSERT(false, "WhiteBGì— ì ìš©í•  Panelì„ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤.")
      end
    end
    ...
    if nil ~= panel then
      ...
      for ii = 0, maxCount - 1 do
        ...
        if nil ~= control then
          ...
          _PA_ASSERT(0 == childID, "SelectedBGì—ì„œ ì°¾ì„ ChildControlì´ ì—†ìŠµë‹ˆë‹¤. Index = " .. tostring(ii))
        end
      end
      self._selectedPanel = panel
    else
      _PA_ASSERT(false, "SelectedBGì— ì ìš©í•  Panelì„ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤.")
    end
  end
end
