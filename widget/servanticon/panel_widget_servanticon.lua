  ...
  elseif iconType == self._iconType.rentInstanceField then
    ...
    if iconState > 0 then
      ...
      elseif 2 == iconState then
        ...
        isShow = false
        _PA_ASSERT(false, "ë¹„ì •ìƒì…ë‹ˆë‹¤.")
      end
    end
...
function PaGlobalFunc_ServantIcon_Init()
  local self = ServantIcon
  if nil == self then
    _PA_ASSERT(false, "íŒ¨ë„ì´ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤!! : ServantIcon")
    return
  end
  self:init()
end
...
function Servant_Call(idx)
  ...
  if 0 == idx then
    ...
    _PA_LOG("ì¢…í˜„", "ì˜¬ë°”ë¥´ì§€ ì•Šì€ idx ì…ë‹ˆë‹¤! : Servant_Call(" .. idx .. ")")
  end
end
...
function InputLUp_ServantIcon_LeftMouseFunction(index)
  ...
  if index == self._iconType.servant_Land then
    ...
    if true == _ContentsGroup_PersonalField and true == ToClient_IsEnterMirrorField() then
...
function InputRUp_ServantIcon_RightMouseFunction(index)
  ...
  if self._iconType.tag == index and (nil ~= Panel_Window_Solrare_Matching and Panel_Window_Solrare_Matching:GetShow() == true or nil ~= Panel_Popup_Solare_MatchConfirm and Panel_Popup_Solare_MatchConfirm:GetShow() == true or ToClient_IsMatchingStateByMatchServer() == true) then
...
function FromClient_ServantIcon_StartAutoUseBuffItemTimer(itemKeyRaw)
  ...
  if itemKeyRaw == nil then
    UI.ASSERT_NAME(false, "ìë™ ì‚¬ìš© ëŒ€ìƒ ì•„ì´í…œ Keyê°€ ë¹„ì •ìƒì…ë‹ˆë‹¤!!! ë°œìƒì‹œ ê¼­ ì œë³´ ë¶€íƒë“œë¦½ë‹ˆë‹¤!!!!", "ì´ì£¼")
    return
  end
  local itemEnchantKey = ItemEnchantKey(itemKeyRaw, 0)
  local itemSSW = getItemEnchantStaticStatus(itemEnchantKey)
  if itemSSW == nil then
    UI.ASSERT_NAME(false, "ìë™ ì‚¬ìš© ëŒ€ìƒ ItemSSWê°€ ë¹„ì •ìƒì…ë‹ˆë‹¤!!! ë°œìƒì‹œ ê¼­ ì œë³´ ë¶€íƒë“œë¦½ë‹ˆë‹¤!!!!", "ì´ì£¼")
    return
  end
  ...
  if icon == nil then
    UI.ASSERT_NAME(false, "ìë™ ì‚¬ìš© Iconì´ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤!!! ë°œìƒì‹œ ê¼­ ì œë³´ ë¶€íƒë“œë¦½ë‹ˆë‹¤!!!!", "ì´ì£¼")
    return
  end
