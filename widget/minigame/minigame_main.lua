...
local minigame = {
  _config = {
    _limitKeyFrame = 60,
    _updateTerm = 0,0166666666666667,
    _keyInputType = {
      _downOnce = 0,
      _up = 1,
      _press = 2
...
function minigame:checkUpdateCondition(inputType, keyType)
  if true == self._isShowDebugMessage then
    _PA_LOG("ì´ë‹¤í˜œ", "Frame : " .. self._tempFrame .. " CurrentDelta : " .. self._deltaTime .. " UpdateTerm : " .. self._config._updateTerm .. "  isUpdate" .. tostring(self._isUpdate))
    _PA_LOG("ì´ë‹¤í˜œ", "inputType : " .. inputType .. " keyType : " .. keyType)
  end
  ...
  if true == self._isShowDebugMessage then
    _PA_LOG("ì´ë‹¤í˜œ", "can calling Function")
  end
  return true
end
