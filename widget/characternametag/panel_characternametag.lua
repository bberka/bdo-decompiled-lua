...
if true == isGameTypeKR2() then
  _stackHpBarColor[0] = Defines.Color.C_FFF96047
end
...
if true == isGameTypeKR2() then
  _normalHpBarColor.defaultColor = Defines.Color.C_FFF96047
end
local _mpBarColor = {
  ...
  for key, value in pairs(_characterHpBarContainer) do
    if nil ~= key and nil ~= _characterHpBarContainer[key] and true == value.calcFlag then
      if nil == value.uiPanel then
        UI.ASSERT_NAME(nil ~= value.uiPanel, "stackHpBarAnimation()  panel nil", "ë°•ê´‘ìš´")
      end
...
local function settingBadPlayerColor(actorKeyRaw, actorProxyWrapper, targetPanel)
  ...
  if true == ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_LocalWar) or true == ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_Solare) then
    return
  end
...
local function settingName(actorKeyRaw, targetPanel, actorProxyWrapper)
  ...
  elseif actorProxy:isSelfPlayer() == true then
    ...
    if _ContentsGroup_EditGuildColor == true then
      ...
      if fontColor ~= nil and fontColor ~= "" and selfplayerActorProxyWrapper:checkAppliableGuildColor() == true then
  ...
  elseif actorProxy:isPlayer() then
    ...
    if _ContentsGroup_EditGuildColor == true then
      ...
      local fontColor = ToClient_GetGuildColorStringByGuildNo(guildNoOrAllianceNo)
      if fontColor ~= nil and fontColor ~= "" and playerActorProxyWrapper:checkAppliableGuildColor() == true then
        nameTag:SetFontColor(fontColor)
        nameTag:useGlowFont(false)
        nameTag:useGlowFont(true, "BaseFont_10_Glow", UI_color.C_FF000000)
        nameTag:SetMonoTone(false)
...
local function settingAlias(actorKeyRaw, targetPanel, actorProxyWrapper)
  ...
  if actorProxy:isPlayer() then
    ...
    if nil ~= characterActorProxyWrapper then
      ...
      local fontColor = ToClient_GetGuildColorStringByGuildNo(guildNoOrAllianceNo)
      if playerActorProxyWrapper:checkAppliableGuildColor() == false then
        fontColor = nil
      end
...
local function settingHpBar(actorKeyRaw, targetPanel, actorProxyWrapper)
  ...
  if false == (actorProxy:isInstanceObject() or actorProxy:isVehicle() or actorProxy:isHouseHold() or actorProxy:isNpc() or actorProxy:isPlayer() or actorProxy:isSelfPlayer() or actorProxy:isMonster() or actorProxy:isInstallationActor() or actorProxy:isReplayActor()) then
    return
  end
  ...
  elseif actorProxy:isPlayer() then
    if false == _ContentsGroup_StackingHpBar then
      ...
      local isShow = false == actorProxy:isHideTimeOver(hideTimeType.overHeadUI) or isParty == true or isArenaAreaOrZone == true or isSolare == true
      local hpPointer = UI.getChildControl(progressGroup, "StaticText_HP")
      local hpRate = actorProxy:getHp() * 100 / actorProxy:getMaxHp()
      ...
      if GameOption_GetShowStackHp ~= nil and GameOption_GetShowStackHp() == true and true == _ContentsGroup_StackingHpBar then
        ...
        if true == isParty or true == isSolare and solareMyTeamNo == solarePlayerTeamNo then
  ...
  elseif actorProxy:isHouseHold() then
    ...
    if false == (houseHoldActorWrapper:getStaticStatusWrapper():getObjectStaticStatus():isKingOrLordTent() or houseHoldActorWrapper:getStaticStatusWrapper():getObjectStaticStatus():isBarricade() or houseHoldActorWrapper:getStaticStatusWrapper():getObjectStaticStatus():isAdvancedBase()) then
      return
    end
    local prevRate = hpMain:GetProgressRate()
    local hpRate = actorProxy:getHp() * 100 / actorProxy:getMaxHp()
    ...
    if houseHoldActorWrapper:getStaticStatusWrapper():getObjectStaticStatus():isKingOrLordTent() and _ContetnsGroup_SiegeResource then
      ...
      if isMyGuild then
        local resourceBack = UI.getChildControl(progressGroup, "Static_SiegeResourcesStackBg")
        local resourceMain = UI.getChildControl(progressGroup, "Progress2_SiegeResourcesStack")
  ...
  if true == furnitureCheck and nil ~= furnitureActorKeyRaw then
    ...
    if nil ~= actorProxyWrapper then
      ...
      if actorProxy ~= nil then
        ...
        _PA_LOG("LUA", "called function Funiture_Endurance_Hide() / actorProxyWrapper:get()ì—ì„œ nullpointerê°€ ì˜¨ë‹¤!!!!")
        return
      end
    else
      _PA_LOG("LUA", "called function Funiture_Endurance_Hide() / getActor( furnitureActorKeyRaw )ì—ì„œ nullpointerê°€ ì˜¨ë‹¤!!!!")
      return
    end
  end
