...
function FromClient_FameChanged(actorKeyRaw, beforeFameValue, afterFameValue)
  ...
  if selfActorKeyRaw == actorKeyRaw then
    ...
    if 0 == effectNumber then
      UI.ASSERT(false, "Fameì˜ ë³€í™”ëŸ‰ì´ 0 ì…ë‹ˆë‹¤! 0ì€ ì²˜ë¦¬í•  í•„ìš”ê°€ ì—†ìŠµë‹ˆë‹¤. ìˆ˜ì •ì´ í•„ìš”í•©ë‹ˆë‹¤.")
    end
...
function DamageOutputFunction_OnDamage(attakeeKeyRaw, effectNumber, effectType, additionalDamageType, posFloat3, isTribeAttack, attackerActorKeyRaw, isNotRandom)
  ...
  if effectType == 5 and isOnRenderMissEffect == false then
    return
  end
  ...
  elseif effectType == 101 then
    ...
    UI.ASSERT(false, "ì´ìƒí•œ íš¨ê³¼ë²ˆí˜¸ê°€ ë“¤ì–´ì˜´.")
  end
  local targetPanel = target._panel
  local isShowTargetPanel = isShowAttackEffect
  if isShowTargetPanel == false and isOnRenderMissEffect == true and effectType == 5 then
  ...
  elseif effectType == 5 then
    ...
    baseX = baseX - effectControlSetting[effectType]._sizeX / 2
    SetAnimationControl(target.effectList[effectType], baseX, baseY - missEffectOffsetY, timeRate, isShowTargetPanel)
  ...
  if isDamageShow then
    if 0 == additionalDamageType and 1 == effectType then
      ...
      SetAnimationControl(target.effectList[7], baseX, baseY, 10, isShowTargetPanel)
