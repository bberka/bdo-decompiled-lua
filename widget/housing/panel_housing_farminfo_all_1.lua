...
function PaGlobal_FarmInfo_All:updateData(isRePosition, ownerHouseHoldNo, cultivationWeatherSSW, progressingInfo, serverUtc64Time)
  ...
  if true == isRePosition then
    ...
    self:setSliderAndStatic("_slider_temperature", "_stc_temperatureArrow", "_txt_temperatureValue", self._tempPercent, cultivationWeatherSSW:getRateByWeather(CppEnums.WeatherKind.WeatherKind_Temperature, self._tempPercent) * 0,0001, true)
    self:setSliderAndStatic("_slider_humidity", "_stc_humidityArrow", "_txt_humidityValue", self._humidityPercent, cultivationWeatherSSW:getRateByWeather(CppEnums.WeatherKind.WeatherKind_Humidity, self._humidityPercent) * 0,0001, false)
  else
    self:setSliderAndStatic("_slider_temperature", "_stc_temperatureArrow", "_txt_temperatureValue", nil, cultivationWeatherSSW:getRateByWeather(CppEnums.WeatherKind.WeatherKind_Temperature, self._tempPercent) * 0,0001, true)
    self:setSliderAndStatic("_slider_humidity", "_stc_humidityArrow", "_txt_humidityValue", nil, cultivationWeatherSSW:getRateByWeather(CppEnums.WeatherKind.WeatherKind_Humidity, self._humidityPercent) * 0,0001, false)
  end
  local groundWaterRate = housing_getInstallationMenuHarvestWaterRate(ownerHouseHoldNo)
  local farmWaterRate = housing_getGrowingRateValue(ownerHouseHoldNo, CppEnums.HarvestGrowRateKind.HarvestGrowRateKind_Water) * 0,0001
  ...
  self:setProgressBarOnly("_progress_compost", housing_getGrowingRateValue(ownerHouseHoldNo, CppEnums.HarvestGrowRateKind.HarvestGrowRateKind_Nutrient) * 0,0001)
  local plusValue = cultivationWeatherSSW:getRateByWeather(CppEnums.WeatherKind.WeatherKind_Water, totalWaterRate) * 0,0001
  if 0 ~= plusValue then
    plusValue = -plusValue
  end
  ...
  if nil ~= progressingInfo then
    self:setProgressBar("_progress_growing", "_txt_growingValue", math.floor(progressingInfo:getCurrentProgress(serverUtc64Time) * 0,0001))
    self:setProgressBar("_progress_helth", "_txt_helthValue", 100 - progressingInfo:getDecreaseYieldsRate() * 0,0001)
    self:setProgressBar("_progress_needWater", "_txt_needWaterValue", 100 - progressingInfo:getNeedWater() * 0,0001)
