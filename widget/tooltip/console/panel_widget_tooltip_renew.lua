...
function TooltipInfo:registEventHandler()
  ...
  registerEvent("FromClient_ItemLocalUpdated", "FromClient_TooltipInfo_ItemLocalUpdated")
end
...
function TooltipInfo:open(tooltipDataType, data, tooltipTargetType, targetX, compareX, isMyWallet, enchantKey, slotType)
  ...
  if CppEnums.ItemType.Equip ~= itemType or Defines.TooltipTargetType.ItemWithoutCompare == tooltipTargetType or true == itemSSW:isVehicleItem() then
    ...
    if nil ~= equipItemWrapper then
      ...
      equipItemSSW:updateLocalDataAll()
...
function TooltipInfo_updateDESCRIPTION(itemWrapper, itemSSW, tooltipTargetType, isMyWallet, enchantKey, slotType)
  ...
  desc = desc .. PAGetString(Defines.StringSheet_GAME, "LUA_TOOLTIP_DESC_TITLE") .. " " .. itemdesc
  local item_type = itemSSW:getItemType()
...
function FromClient_TooltipInfo_ItemLocalUpdated(itemKey)
  if false == PaGlobalFunc_TooltipInfo_GetShow() then
    return
  end
  PaGlobalFunc_TooltipInfo_Close()
  local data
  if nil ~= tempDataList.itemKey then
    data = getItemEnchantStaticStatus(tempDataList.itemKey)
  else
    data = tempDataList.data
  end
  TooltipInfo:open(tempDataList.tooltipDataType, data, tempDataList.tooltipTargetType, tempDataList.targetX, tempDataList.compareX, tempDataList.isMyWallet, tempDataList.enchantKey, tempDataList.slotType)
end
