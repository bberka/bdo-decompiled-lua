...
local normalTooltip = {
  ...
  smelting_title = UI.getChildControl(Panel_Tooltip_Item, "Tooltip_Item_OldMoonSmelting_Title"),
  smelting_area = UI.getChildControl(Panel_Tooltip_Item, "Tooltip_Item_OldMoonSmelting_Gauge_Area"),
  smelting_gauge_maxValue = UI.getChildControl(Panel_Tooltip_Item, "Progress2_OldMoonSmelting_MaxValue"),
  smelting_gauge_currentValue = UI.getChildControl(Panel_Tooltip_Item, "Progress_OldMoonSmelting_CurrentValue"),
  smelting_textValue = UI.getChildControl(Panel_Tooltip_Item, "Tooltip_Item_OldMoonSmelting_ValueString"),
...
local equippedTooltip = {
  ...
  smelting_title = UI.getChildControl(Panel_Tooltip_Item_equipped, "Tooltip_Item_OldMoonSmelting_Title"),
  smelting_area = UI.getChildControl(Panel_Tooltip_Item_equipped, "Tooltip_Item_OldMoonSmelting_Gauge_Area"),
  smelting_gauge_maxValue = UI.getChildControl(Panel_Tooltip_Item_equipped, "Progress2_OldMoonSmelting_MaxValue"),
  smelting_gauge_currentValue = UI.getChildControl(Panel_Tooltip_Item_equipped, "Progress_OldMoonSmelting_CurrentValue"),
  smelting_textValue = UI.getChildControl(Panel_Tooltip_Item_equipped, "Tooltip_Item_OldMoonSmelting_ValueString"),
...
local chattingLinkedItemTooltip = {
  ...
  smelting_title = UI.getChildControl(Panel_Tooltip_Item_chattingLinkedItem, "Tooltip_Item_OldMoonSmelting_Title"),
  smelting_area = UI.getChildControl(Panel_Tooltip_Item_chattingLinkedItem, "Tooltip_Item_OldMoonSmelting_Gauge_Area"),
  smelting_gauge_maxValue = UI.getChildControl(Panel_Tooltip_Item_chattingLinkedItem, "Progress2_OldMoonSmelting_MaxValue"),
  smelting_gauge_currentValue = UI.getChildControl(Panel_Tooltip_Item_chattingLinkedItem, "Progress_OldMoonSmelting_CurrentValue"),
  smelting_textValue = UI.getChildControl(Panel_Tooltip_Item_chattingLinkedItem, "Tooltip_Item_OldMoonSmelting_ValueString"),
...
local chattingLinkedItemClickTooltip = {
  ...
  smelting_title = UI.getChildControl(Panel_Tooltip_Item_chattingLinkedItemClick, "Tooltip_Item_OldMoonSmelting_Title"),
  smelting_area = UI.getChildControl(Panel_Tooltip_Item_chattingLinkedItemClick, "Tooltip_Item_OldMoonSmelting_Gauge_Area"),
  smelting_gauge_maxValue = UI.getChildControl(Panel_Tooltip_Item_chattingLinkedItemClick, "Progress2_OldMoonSmelting_MaxValue"),
  smelting_gauge_currentValue = UI.getChildControl(Panel_Tooltip_Item_chattingLinkedItemClick, "Progress_OldMoonSmelting_CurrentValue"),
  smelting_textValue = UI.getChildControl(Panel_Tooltip_Item_chattingLinkedItemClick, "Tooltip_Item_OldMoonSmelting_ValueString"),
...
function Panel_Tooltip_Item_Initialize()
  ...
  local function InitControls(target)
    ...
    SetTextWrap(target.smelting_title)
    SetTextWrap(target.smelting_textValue)
...
function Panel_Tooltip_Item_Show(itemStaticStatus, uiBase, isSSW, isItemWrapper, chattingLinkedItem, index, isItemMarket, invenSlotNo, TooltipType, indexForNaming, openedUIType)
...
function Panel_Tooltip_Item_Show_GeneralNormal(slotNo, slotType, isOn, index, targetX, targetY, uiBase, addPosX, addPosY)
  ...
  elseif "inventory" == slotType then
    ...
    parent = true
    inven = true
  elseif "FamilyInventory" == slotType then
  ...
  elseif "ShipEquipManagementInventory" == slotType then
    ...
    UI.ASSERT(false, "showTooltip(normal)ìœ¼ë¡œ ì •ì˜ë˜ì§€ ì•Šì€ slotíƒ€ì…(" .. slotType .. ")ì´ ë“¤ì–´ì™”ìŠµë‹ˆë‹¤. í™•ì¸í•´ì£¼ì„¸ìš”.")
    return
  end
  if nil == itemWrapper then
    return
...
function Panel_Tooltip_Item_Show_GeneralStatic(slotNo, slotType, isOn, index, posX, posY)
  ...
  elseif "masterpiecdAuction" == slotType then
    ...
    UI.ASSERT(false, "showTooltip(static)ìœ¼ë¡œ ì •ì˜ë˜ì§€ ì•Šì€ slotíƒ€ì…(" .. slotType .. ")ì´ ë“¤ì–´ì™”ìŠµë‹ˆë‹¤. í™•ì¸í•´ì£¼ì„¸ìš”.")
    return
  end
  if nil == itemSSW then
    return
...
function Panel_Tooltip_Item_ShowInfo(target, inputValue, isSSW, isItemWrapper, chattingLinkedItem, index, isNextEnchantInfo, invenSlotNo, itemNamingStr, openedUIType)
  ...
  if equippedTooltip == target then
    ...
    if CppEnums.ContentsEventType.ContentsType_InventoryBag == itemWrapper:getStaticStatus():get():getContentsEventType() then
      ...
      equippedTooltip.bagSize:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_TOOLTIP_ITEM_BAGSIZE", "size", bagSize))
      local itemNameinBag = ""
      for index = 0, bagSize - 1 do
        bagItemWrapper = getEquipInventoryBagItem(index)
        if nil ~= bagItemWrapper then
          if "" == itemNameinBag then
            itemNameinBag = tostring(bagItemWrapper:getStaticStatus():getName())
          else
  ...
  elseif 0 == nameColorGrade then
    target.itemName:SetFontColor(4292925656)
  elseif 1 == nameColorGrade then
    target.itemName:SetFontColor(4290110555)
  elseif 2 == nameColorGrade then
    target.itemName:SetFontColor(4284724988)
  elseif 3 == nameColorGrade then
    target.itemName:SetFontColor(4294951744)
  elseif 4 == nameColorGrade then
    target.itemName:SetFontColor(4293815895)
  else
  ...
  if Panel_Tooltip_Item_equipped:GetShow() then
    ...
    if _weightPoint > 0 then
      _weightPoint = diffRedColorStr .. "â–²" .. string.format("%.2f", _weightPoint) .. "<PAOldColor>"
    elseif _weightPoint < 0 then
      _weightPoint = "<PAColor0xFFFFCE22>â–¼" .. string.format("%.2f", _weightPoint * -1) .. "<PAOldColor>"
    end
    if 0 ~= _offencePoint and 0 == _defencePoint then
      if _offencePoint > 0 then
        _offencePoint = "<PAColor0xFFFFCE22>â–²" .. _offencePoint .. "<PAOldColor>"
      elseif _offencePoint < 0 then
        _offencePoint = diffRedColorStr .. "â–¼" .. _offencePoint * -1 .. "<PAOldColor>"
      end
    ...
    elseif 0 ~= _offencePoint and 0 ~= _defencePoint then
      ...
      if _defencePoint > 0 then
        _defencePoint = "<PAColor0xFFFFCE22>â–²" .. _defencePoint .. "<PAOldColor>"
      elseif _defencePoint < 0 then
        _defencePoint = diffRedColorStr .. "â–¼" .. _defencePoint * -1 .. "<PAOldColor>"
      end
      attackString = "(" .. _offencePoint .. " )"
      normalTooltip.attack_diffValue:SetText(attackString)
  ...
  target.smelting_title:SetShow(false)
  target.smelting_area:SetShow(false)
  target.smelting_gauge_maxValue:SetShow(false)
  target.smelting_gauge_currentValue:SetShow(false)
  target.smelting_textValue:SetShow(false)
  if true == _ContentsGroup_ChinaOnly and true == isItemWrapper and nil ~= item then
    local isShowSmeltingValue = true == item:isOldMoonSmeltableItem()
    if true == isShowSmeltingValue then
      local smelting_currentValue = item:getCurrentOldMoonSmeltedValue()
      local smelting_maxValue = item:getMaxOldMoonSmeltedValue()
      local smelting_percent = smelting_currentValue / smelting_maxValue * 100
      target.smelting_title:SetShow(true)
      target.smelting_area:SetShow(true)
      target.smelting_gauge_maxValue:SetShow(true)
      target.smelting_gauge_maxValue:SetProgressRate(100)
      target.smelting_gauge_maxValue:SetAniSpeed(0)
      target.smelting_gauge_currentValue:SetShow(true)
      target.smelting_gauge_currentValue:SetProgressRate(smelting_percent)
      target.smelting_gauge_currentValue:SetAniSpeed(0)
      target.smelting_textValue:SetShow(true)
      target.smelting_textValue:SetText(tostring(smelting_currentValue) .. " / " .. tostring(smelting_maxValue))
    end
  end
...
- ]] .. itemSSW:getEnchantDescription()
  ...
  if true == _ContentsGroup_ChinaOnly and true == isItemWrapper and nil ~= item and true == itemWrapper:isOldMoonFullSatck() then
    local oldMoonBuffCount = itemWrapper:getOldMoonBuffCount()
    if oldMoonBuffCount > 0 then
      _desc = _desc .. [[


]] .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_OLDMOON_TOOLTIP_TITLE")
      for oldMoonBuffIndex = 0, oldMoonBuffCount - 1 do
        _desc = _desc .. [[

<PAColor0xffe9bd23>   ]] .. itemWrapper:getOldMoonBuffDesc(oldMoonBuffIndex) .. "<PAOldColor>"
      end
    end
  end
  ...
  if true == _ContentsGroup_ChinaOnly and true == target.smelting_title:GetShow() then
    target.smelting_title:SetPosY(TooltipYPos + elementgap)
    target.smelting_area:SetPosY(TooltipYPos + 15)
    target.smelting_textValue:SetPosY(TooltipYPos + 15)
    target.smelting_gauge_maxValue:SetPosY(TooltipYPos + 15 + (target.smelting_area:GetSizeY() - target.smelting_gauge_maxValue:GetSizeY()) / 2)
    target.smelting_gauge_currentValue:SetPosY(TooltipYPos + 15 + (target.smelting_area:GetSizeY() - target.smelting_gauge_currentValue:GetSizeY()) / 2)
    TooltipYPos = GetBottomPos(target.smelting_title) + elementBiggap
  end
