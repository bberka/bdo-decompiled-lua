...
function ShowCommandFunc(respawnTime, isMast)
  ...
  if true == isSolare then
    ...
    WatingModeSetting_Default(respawnTime, isMast)
  end
...
function WatingModeSetting_Default(respawnTime, isMast)
  local self = watchingMode
...
function WatingModeSetting_Solare()
  ...
  self.exit:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WATCHINGCOMMAND_EXIT"))
