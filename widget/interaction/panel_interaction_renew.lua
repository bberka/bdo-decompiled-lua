...
function PanelInteraction:show(actor)
  ...
  if nil == actorName then
    UI.ASSERT(false, "panel_interaction_renew : actorName ì´ nil ì…ë‹ˆë‹¤!")
  end
  ...
  for ii, value in pairs(_interactionTargetUIList) do
    ...
    if CppEnums.InteractionType.InteractionType_Collect == ii and isShow == true and _ContentsGroup_CollectToolBox == true and ToClient_CheckEquipUnionBelt() == true then
      isShow = false
      ...
      if collectWrapper == nil then
        ...
        if collectWrapper == nil then
          ...
          if collectWrapper == nil then
            _PA_ASSERT(false, "ì±„ì§‘ì¸í„°ë™ì…˜ì„ ìˆ˜í–‰ì¤‘ì¸ë° ëŒ€ìƒì´ ì±„ì§‘ë¬¼ì´ ì•„ë‹™ë‹ˆë‹¤. ë¹„ì •ìƒì…ë‹ˆë‹¤.")
          end
        end
      end
      ...
      for ii, value in pairs(_collectInteractionTargetUIList) do
        local isAble = collectWrapper:getCharacterStaticStatusWrapper():isCollectableToolType(_collect_Button_ID[ii]) and ToClient_CheckHaveToolTypeInUnionBelt(_collect_Button_ID[ii])
        if actor:get():isHoeCharacter() == true then
          if _collect_Button_ID[ii] == __eCollectToolType_Hoe and ToClient_CheckHaveToolTypeInUnionBelt(__eCollectToolType_Hoe) then
            isAble = true
          else
            isAble = false
          end
...
function PanelInteraction:getButtonIcon(control, index)
  if nil == _button_TextureUV[index] then
    UI.ASSERT(false, "panel_interaction_renew : í•´ë‹¹ ì¸ë±ìŠ¤ì˜ í…ìŠ¤ì³ê°€ ì—†ìŠµë‹ˆë‹¤! " .. index)
    return
  end
...
function PanelInteraction:updatePerFrame(deltaTime)
  ...
  if selfPlayer == nil or selfPlayer:isDead() == true then
    self:close()
    return
  elseif Panel_Window_Exchange:GetShow() == true then
    self:close()
    return
...
function PanelInteraction:updateDesc(indteractionType)
    ...
    if actor:get():isCollect() or actor:get():isDeadBody() then
      ...
      if nil == collectWrapper then
        ...
        if nil == collectWrapper then
          collectWrapper = getDeadBodyActor(actor:getActorKey())
          if nil == collectWrapper then
            _PA_ASSERT(false, "ì±„ì§‘ì¸í„°ë™ì…˜ì„ ìˆ˜í–‰ì¤‘ì¸ë° ëŒ€ìƒì´ ì±„ì§‘ë¬¼ì´ ì•„ë‹™ë‹ˆë‹¤. ë¹„ì •ìƒì„")
            return
          end
        end
      end
  ...
  if indteractionType == CppEnums.InteractionType.InteractionType_Collect and _ContentsGroup_CollectToolBox == true and ToClient_CheckEquipUnionBelt() == true and self._ui.needCollectTool:GetShow() == false then
...
function PaGlobalFunc_PanelInteraction_ButtonPosUpdate(deltaTime)
  local self = PanelInteraction
  if nil == self then
    _PA_ASSERT(false, "íŒ¨ë„ì´ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤!! : PanelInteraction")
    return
  end
  ...
  if ANIMATION_TIME < self._buttonTimeAcc then
    ...
    local actor = interaction_getInteractable()
    if _ContentsGroup_CollectToolBox == true and ToClient_CheckEquipUnionBelt() == true then
      local jj = 0
      for ii, value in pairs(_collectInteractionTargetUIList) do
...
function PaGlobalFunc_PanelInteraction_ButtonPushed(interactionType)
  ...
  elseif CppEnums.InteractionType.InteractionType_Collect == interactionType then
    ...
    if _ContentsGroup_CollectToolBox == true and ToClient_CheckEquipUnionBelt() == true then
      ToClient_SetCollectToolTypeUnionBelt(self._currentSelectTool)
    else
      ToClient_SetCollectToolTypeUnionBelt(__eCollectToolType_Count)
    end
  ...
  elseif CppEnums.InteractionType.InteractionType_Ride == interactionType then
    ...
    if true == _ContentsGroup_PersonalField then
      if false == ToClient_CheckPersonalFieldDelayTime(true) then
        return
      end
      if true == ToClient_IsEnterMirrorField() then
        local instanceSpawnWrapper
        if nil ~= vehicleWrapper and nil ~= vehicleWrapper:get() then
          instanceSpawnWrapper = ToClient_getInstanceSpawnInfoWrapperByPosition(vehicleWrapper:get():getPosition())
...
function FromClient_PanelInteraction_BuildingUpgrade(actorKeyRaw)
  ...
  if nil == upgradeStaticWrapper then
    _PA_ASSERT(false, "FromClient_PanelInteraction_BuildingUpgrade : upgradeStaticWrapperì´ ë¹„ì–´ìˆìŠµë‹ˆë‹¤. ")
    return
  end
  if nil == upgradeStaticWrapper:getTargetCharacter() then
    _PA_ASSERT(false, "FromClient_PanelInteraction_BuildingUpgrade : upgradeStaticWrapper:getTargetCharacter()ì´ ë¹„ì–´ìˆìŠµë‹ˆë‹¤. ")
    return
  end
...
function FromClient_InteractionRenew_ElephantBarnTalk()
  if PaGlobalFunc_RecoveryElephant_Open == nil then
    UI.ASSERT_NAME(false, "Open í•˜ë ¤ëŠ” UIê°€ Loadë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤.", "ì´ì£¼")
    return
  end
