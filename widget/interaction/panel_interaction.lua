...
function Interaction_Update(deltaTime)
  ...
  elseif Defines.UIMode.eUIMode_Default ~= uiMode then
    Interaction_Close()
    return
  end
...
function FromClient_InteractionBuildingUpgrade(actorKeyRaw)
  ...
  if nil == upgradeStaticWrapper then
    _PA_ASSERT(false, "FromClient_InteractionBuildingUpgrade --- upgradeStaticWrapperì´ ë¹„ì–´ìˆìŠµë‹ˆë‹¤. ")
    return
  end
  if nil == upgradeStaticWrapper:getTargetCharacter() then
    _PA_ASSERT(false, "FromClient_InteractionBuildingUpgrade --- upgradeStaticWrapper:getTargetCharacter()ì´ ë¹„ì–´ìˆìŠµë‹ˆë‹¤. ")
    return
  end
