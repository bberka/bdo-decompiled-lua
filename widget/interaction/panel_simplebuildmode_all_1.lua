...
function PaGlobal_SimpleBuildMode_All:prepareClose()
  ...
  if _ContentsGroup_SiegeInstallation == true and PaGlobal_House_InstallationSiege_All_EndBuildTent ~= nil and Panel_House_InstallationMode_Siege_All ~= nil then
...
function PaGlobal_SimpleBuildMode_All:update(deltaTime)
  ...
  if isKeyUpFor(CppEnums.VirtualKeyCode.KeyCode_R) == true then
  ...
  if isKeyUpFor(CppEnums.VirtualKeyCode.KeyCode_F) == true then
    if ToClient_isSimpleBuildFixState() == true then
      PaGlobal_SimpleBuildMode_All_SetCameraMode(not self._isChangeCameraMode, 1)
    end
    return
  end
end
