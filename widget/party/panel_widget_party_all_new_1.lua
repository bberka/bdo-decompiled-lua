function PaGlobal_Party_All:initialize()
  ...
  if true == isGameTypeKR2() then
    self._ui_pc.btn_marketOption_Mini:SetShow(false)
    self._ui_pc.btn_marketOption:SetShow(false)
    self._ui_pc.btn_changeType:SetPosX(self._ui_pc.btn_changeType:GetPosX() - 35)
    self._ui_pc.btn_changeType_Mini:SetPosX(self._ui_pc.btn_changeType_Mini:GetPosX() - 35)
  end
  self._initialize = true
end
function PaGlobal_Party_All:hidePanelConditionCheck()
  if nil == Panel_Widget_Party_All then
    _PA_ASSERT_NAME(false, "Panel_Widget_Party_Allì´ nil ì…ë‹ˆë‹¤.", "ì„œê¸°ì›")
    return
  end
...
function PaGlobal_Party_All:registEventHandler()
  ...
  for index = 1, self._config.maxPartyCnt do
    ...
    control.control:addInputEvent("Mouse_On", "PaGlobal_Party_All:overPartyLeave(true, " .. index .. ")")
    control.control:addInputEvent("Mouse_Out", "PaGlobal_Party_All:overPartyLeave(false, " .. index .. ")")
  ...
  self._ui_pc.radio_onlyLeader:addInputEvent("Mouse_LUp", "PaGlobal_Party_All:selectLootingType(" .. CppEnums.PartyLootType.LootType_Master .. ")")
  ...
  self._ui_pc.radio_onlyLeader_Mini:addInputEvent("Mouse_LUp", "PaGlobal_Party_All:selectLootingType(" .. CppEnums.PartyLootType.LootType_Master .. ")")
  for index = 1, self._config.maxPartyCnt do
...
function PaGlobal_Party_All:setPartyMember()
  local memberCount = RequestParty_getPartyMemberCount()
  UI.ASSERT_NAME(memberCount > 0, "íŒŒí‹°ì› ìˆ˜ê°€ ë¹„ì •ìƒì…ë‹ˆë‹¤.", "ë°•ê·œë‚˜")
  for index = 0, memberCount - 1 do
    ...
    if memberInfo._index == 0 and false == memberInfo._isSelf then
      UI.ASSERT_NAME(false, "_index = 0ëŠ” ë¬´ì¡°ê±´ Selfì—¬ì•¼ í•©ë‹ˆë‹¤.", "ë°•ê·œë‚˜")
      return
    end
...
function PaGlobal_Party_All:setClassIcon(control, class)
  ...
  control:ChangeTextureInfoNameDefault(self._textureClassSymbol.path)
...
function PaGlobal_Party_All:closePartyOption(index)
  if nil ~= index then
    ...
    control._btn_selfLeave:SetShow(false)
...
function PaGlobal_Party_All:overPartyLeave(isShow, index)
  if false == self._partyData[index]._isSelf then
    return
  end
  local control = self._ui.stc_members[index]
  local control_mini = self._ui.stc_members_Mini[index]
  local isMouseOnPanel = self:Common_WidgetMouseOn(index)
  if false == isShow then
    control._btn_selfLeave:SetShow(isMouseOnPanel)
    control_mini._btn_selfLeave:SetShow(isMouseOnPanel)
    return
  end
  if true == isShow and true == self._partyData[index]._isSelf then
    control._btn_selfLeave:SetShow(true)
    control_mini._btn_selfLeave:SetShow(true)
  end
end
...
function PaGlobal_Party_All:setPartyMemberXXX(partyData, isMini)
  UI.ASSERT_NAME(nil ~= partyData, "PartyDataê°€ nil ì…ë‹ˆë‹¤.", "ë°•ê·œë‚˜")
  ...
  if nil == member then
    UI.ASSERT_NAME(nil ~= partyData, "member ì •ë³´ê°€ ì—†ìŠµë‹ˆë‹¤.", "ë°•ê·œë‚˜")
    return
  end
...
function PaGlobal_Party_All:showPartyData()
  UI.ASSERT_NAME(false, "*** PaGlobal_Party_All:showPartyData(ë²„ê·¸ ì¬í˜„ì‹œ í™•ì¸ìš©) ***", "ë°•ê·œë‚˜")
  _PA_LOG("ë°•ê·œë‚˜", "self._isMemberDataLoad = " .. tostring(self._isMemberDataLoad))
  for key, partyData in pairs(self._partyData) do
    _PA_LOG("ë°•ê·œë‚˜", "key = " .. tostring(key))
    _PA_LOG("ë°•ê·œë‚˜", "isMaster = " .. tostring(partyData._isMaster))
    _PA_LOG("ë°•ê·œë‚˜", "name = " .. tostring(partyData._name))
    _PA_LOG("ë°•ê·œë‚˜", "class = " .. tostring(partyData._class))
    _PA_LOG("ë°•ê·œë‚˜", "level = " .. tostring(partyData._level))
    _PA_LOG("ë°•ê·œë‚˜", "actorKeyRaw = " .. tostring(partyData._actorKeyRaw))
    _PA_LOG("ë°•ê·œë‚˜", "")
  end
end
