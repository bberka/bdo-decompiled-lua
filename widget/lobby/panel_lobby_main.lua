...
Panel_Lobby_ClassUI.ClassButtons[__eClassType_Giant_Reserved2] = UI.getChildControl(_frameContents, "RadioButton_Select_Unknown9")
Panel_Lobby_ClassUI.ClassButtons[__eClassType_KunoichiOld] = UI.getChildControl(_frameContents, "RadioButton_Select_Unknown10")
...
local isPremiumCharacter = false
function FGlobal_SetPremiumCharacter(isSC)
  isPremiumCharacter = isSC
end
function FGlobal_getIsPremiumCharacter()
  return isPremiumCharacter
end
...
local function Panel_Lobby_Function_Initialize()
  ...
  Panel_Lobby_ClassUI.ClassNames[__eClassType_Sage]:SetTextMode(__eTextMode_AutoWrap)
  Panel_Lobby_ClassUI.ClassNames[__eClassType_Corsair]:SetTextMode(__eTextMode_AutoWrap)
  ...
  Panel_Lobby_ClassUI.ClassNames[__eClassType_Drakania]:SetTextMode(__eTextMode_AutoWrap)
  Panel_Lobby_ClassUI.ClassNames[__eClassType_Giant_Reserved0]:SetTextMode(__eTextMode_AutoWrap)
  Panel_Lobby_ClassUI.ClassNames[__eClassType_Giant_Reserved2]:SetTextMode(__eTextMode_AutoWrap)
  Panel_Lobby_ClassUI.ClassNames[__eClassType_KunoichiOld]:SetTextMode(__eTextMode_AutoWrap)
  ...
  Panel_Lobby_ClassUI.ClassNames[__eClassType_ShyMan]:SetText("ìƒ¤ì´ ë‚¨ì")
  Panel_Lobby_ClassUI.ClassNames[__eClassType_BladeMaster_Reserved1]:SetText("ì„ì‹œ")
  ...
  Panel_Lobby_ClassUI.ClassNames[__eClassType_Corsair]:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CLASSTYPE_10"))
  ...
  Panel_Lobby_ClassUI.ClassNames[__eClassType_Sage]:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CLASSTYPE_SAGE"))
  ...
  Panel_Lobby_ClassUI.ClassNames[__eClassType_Drakania]:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CLASSTYPE_7"))
  Panel_Lobby_ClassUI.ClassNames[__eClassType_Giant_Reserved0]:SetText("ìºë¦­í„°5")
  Panel_Lobby_ClassUI.ClassNames[__eClassType_Giant_Reserved2]:SetText("ìºë¦­í„°6")
  Panel_Lobby_ClassUI.ClassNames[__eClassType_KunoichiOld]:SetText("ìºë¦­í„°7")
...
function Panel_Lobby_ClassCountCheck()
  local characterCount = getCharacterDataCount(false)
  classCount = {}
  for index = 0, characterCount - 1 do
    local charData = getCharacterDataByIndex(index, false)
...
function Panel_Lobby_function_SelectClassType(index, isOn)
  if index < __eClassType_Count then
    if false == ToClient_checkCreatePossibleClass(index, isPremiumCharacter) then
      return
    end
    ...
    if true == _ContentsGroup_UsePadSnapping then
      ...
      Panel_Lobby_UI.CCSC_ClassName:SetText(getClassName(index, isPremiumCharacter))
    else
      if true == isGameTypeKR2() then
        Panel_Lobby_UI.CCSC_ClassMovie:SetShow(false)
        Panel_Lobby_UI.CCSC_RadioNormalMovie:SetShow(false)
        ...
        local movieName = getClassMovie(index, isPremiumCharacter)
      ...
      Panel_Lobby_UI.CCSC_ClassDesc:SetText(getClassDescription(index, isPremiumCharacter))
      Panel_Lobby_UI.CCSC_ClassDesc:SetShow(false)
      txt_BottomDesc:SetTextMode(__eTextMode_AutoWrap)
      txt_BottomDesc:SetText(getClassDescription(index, isPremiumCharacter))
    end
...
function Panel_Lobby_SetMovie()
  local movieName = getClassMovie(_index, isPremiumCharacter)
...
function Panel_CharacterCreateOk()
  chracterCreate(Panel_Lobby_Global_Variable.characterSelectType, Panel_Lobby_UI.CC_CharacterNameEdit:GetEditText(), isPremiumCharacter)
end
...
function Panel_CharacterCreateOK_Renew(characterName)
  ...
  local function createCharacterFunc()
    chracterCreate(Panel_Lobby_Global_Variable.characterSelectType, _edit_CharacterName, isPremiumCharacter, false, false)
...
function Panel_CharacterCreateOK_Confirm(isNewRoute)
  ...
  if false == _ContentsGroup_SelectStartPosition then
    isNewRoute = false
  end
  local function createCharacterFunc()
    if true == _ContentsGroup_SeasonContents or true == _ContentsGroup_PreSeason then
      if true == isPremiumCharacter then
        ...
        chracterCreate(Panel_Lobby_Global_Variable.characterSelectType, _edit_CharacterName, isPremiumCharacter, PaGlobalFunc_Customization_IsSeasonBtnClick(), isNewRoute)
      end
    else
      chracterCreate(Panel_Lobby_Global_Variable.characterSelectType, _edit_CharacterName, isPremiumCharacter, false, isNewRoute)
    end
  end
  local messageBoxTitle = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE")
  local messageBoxMemo = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_LOBBY_MAIN_CHARACTERCREATE_RECONFIRM_CHARACTERNAME", "name", _edit_CharacterName)
...
<PAColor0xFFF26A6A>]] .. PAGetString(Defines.StringSheet_GAME, "LUA_MAKENAME_WARNING") .. "<PAOldColor>"
  ...
  MessageBox.showMessageBox(messageBoxData)
  if true == _ContentsGroup_SelectStartPosition and false == FGlobal_getIsPremiumCharacter() then
...
function Panel_CharacterCreateOK_NewCustomization()
  ...
  if false == _valid_isValid or _edit_CharacterName ~= _valid_inputString then
    ...
    MessageBox.showMessageBox(messageBoxData)
    PaGlobal_CheckCharacterCreationArea()
    _valid_isValid = false
    _valid_inputString = ""
    return
  end
  if false == _ContentsGroup_NewSequenceTutorial and true == _ContentsGroup_SelectStartPosition and false == FGlobal_getIsPremiumCharacter() and nil ~= Panel_Window_StartBranch then
    PaGlobalFunc_StartBranch_All_CreateOpen()
    return
  end
  Panel_CharacterCreateOK_Confirm(false)
end
function Panel_CharacterCreateCancel()
  ...
  characterCreateCancel(FGlobal_getIsPremiumCharacter())
  TooltipSimple_Hide()
end
...
function PaGlobalFunc_CharacterSelect_FirstClass()
  if true == ToClient_IsContentsGroupOpen("1600") then
    return __eClassType_Drakania
  end
  return nil
end
