...
function PaGlobalFunc_CharacterSelect_All_OnScreenResize()
  ...
  if true == PaGlobal_CharacterSelect_All._isUsePadSnapping then
    ...
    btnSize = getScreenSizeY() - (PaGlobal_CharacterSelect_All._ui.btn_CreateCharacter:GetPosY() - PaGlobal_CharacterSelect_All._ui.stc_RightBg:GetPosY())
    btnSize = btnSize + (PaGlobal_CharacterSelect_All._ui.list2_Character:GetPosY() - PaGlobal_CharacterSelect_All._ui.stc_RightBg:GetPosY()) + padding
    PaGlobal_CharacterSelect_All._ui.list2_Character:SetSize(PaGlobal_CharacterSelect_All._ui.list2_Character:GetSizeX(), getScreenSizeY() - btnSize)
  end
...
function HandleEventOnOut_CharacterSelect_All_CharacterCountGuide(isShow)
  if true == isShow then
    local isPremiumCharacer = PaGlobal_CharacterSelect_All._ui.rdo_Special:IsCheck()
    local maxSlot = getCharacterSlotMaxCount(isPremiumCharacer)
    local limitCount = getCharacterSlotLimit(isPremiumCharacer)
    local haveCount = getCharacterDataCount(isPremiumCharacer)
    local title = "ìºë¦­í„° ìŠ¬ë¡¯ ìˆ˜"
    local desc = ""
    local desc1 = "ìºë¦­í„° ìŠ¬ë¡¯ ìˆ˜ë§Œí¼ ìºë¦­í„°ë¥¼ ìƒì„±í•  ìˆ˜ ìˆìŠµë‹ˆë‹¤."
    local desc2 = "ìºë¦­í„° ìŠ¬ë¡¯ì€ í„ ìƒì ì—ì„œ íŒë§¤í•˜ëŠ” <PAColor0xFFFFC832>[ìºë¦­í„° ìŠ¬ë¡¯ í™•ì¥ê¶Œ]<PAOldColor>ìœ¼ë¡œ í™•ì¥í•  ìˆ˜ ìˆìŠµë‹ˆë‹¤(í˜„ì¬ <PAColor0xFFFFC832>" .. maxSlot - limitCount .. "<PAOldColor>ê°œì˜ ìŠ¬ë¡¯ <PAColor0xFF96D4FC>í™•ì¥ ê°€ëŠ¥<PAOldColor>)."
    if limitCount == maxSlot or true == isPremiumCharacer then
      desc = desc1
    else
      desc = desc1 .. " " .. desc2
    end
...
function HandleEventLUp_CharacterSelect_All_CreateNewCharacter()
  ...
  local do_Create = function()
    ...
    if false == PaGlobal_Season_Character_Create:prepareOpen(isPrimiumCharacter) then
      changeCreateCharacterMode_SelectClass(isPrimiumCharacter)
      FGlobal_SetPremiumCharacter(isPrimiumCharacter)
    end
  end
  local MessageData = {
    title = nil,
  ...
  if true == PaGlobal_CharacterSelect_All._isPrimiumCharacter then
...
function HandleEventLUp_CharacterSelect_All_Deleting()
  ...
  if -1 ~= currentSelectIdx then
    ...
    local removeTime
    if removeTimeCheckLevel > charData._level or true == isPrimiumCharacter then
...
function HandleEventLUp_CharacterSelect_All_PrepareEnterToField(charIdx)
  ...
  if true == ToClient_isEventChannel() and false == ToClient_SelfPlayerIsGM() then
    ...
    if true == ToClient_isDiffServerNo(lastEnterServerNo) then
    ...
    elseif __eLobby_EnterToField == enterFieldType then
      ...
      local function tutorialSkipEnterGame()
        if _ContentsGroup_SelectStartPosition == true then
          ...
          ToClient_NewSequenceTutorialSkipChangeStartPosition(true, false, charIdx)
        end
      end
      local function enterTutorial()
        ToClient_NewSequenceTutorialSkipChangeStartPosition(false, false, charIdx)
      end
      if true == _ContentsGroup_NewSequenceTutorial then
        ...
        if 1 == characterData._level and false == PaGlobal_CharacterSelect_All._isPrimiumCharacter then
        ...
        elseif characterData._level >= 2 and lastLogoutTime == lastLogInTime and PaGlobal_CharacterSelect_All._isPrimiumCharacter == false then
          tutorialSkipEnterGame()
        else
          normalEnterGame()
        end
...
function HandleEventLUp_CharacterSelect_All_TabClick(idx)
  ...
  if idx ~= PaGlobal_CharacterSelect_All._currentTab then
    ...
    FGlobal_SetPremiumCharacter(PaGlobal_CharacterSelect_All._isPrimiumCharacter)
    local characterData = getCharacterDataByIndex(PaGlobal_CharacterSelect_All._selectedCharIdx, PaGlobal_CharacterSelect_All._isPrimiumCharacter)
    if nil ~= characterData then
      local classType = getCharacterClassType(characterData)
...
function FromClient_ChracterSelect_All_ReceiveWaiting()
  ...
  local characterData = getCharacterDataByIndex(PaGlobal_CharacterSelect_All._selectedCharIdx, isPrimiumCharacter)
  if nil == characterData then
    UI.ASSERT(false, "ìºë¦­í„° ì„ íƒ ì •ë³´ê°€ ì—†ìŠµë‹ˆë‹¤!")
    return
  end
  PaGlobal_CharacterSelect_All._ui.txt_CharacterStatus:SetFontColor(Defines.Color.C_FFD05D48)
...
function PaGlobalFunc_CharacterSelect_All_MakeWaitingUserMsg(receiveTicketNoMyRegion)
  ...
  if isGameTypeTaiwan() then
    taiwanMsg = "\n\né»‘è‰²æ²™æ¼ ç‚ºå–®ä¸€ä¸–ç•Œå…¨ä¼ºæœå™¨å…±é€šï¼Œåœ¨éŠæˆ²æ™‚å¯åœ¨å„é »é“é–“è‡ªç”±ç§»å‹•ï¼Œè«‹é¸æ“‡é€²è¡Œè¼ƒé †åˆ©çš„é »é“ç™»å…¥éŠæˆ²å³å¯"
  end
