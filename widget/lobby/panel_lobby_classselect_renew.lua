...
local _isPremiumCharacter = false
function FGlobal_SetPremiumCharacter(isSpecial)
  _isPremiumCharacter = isSpecial
end
function FGlobal_getIsPremiumCharacter()
  return _isPremiumCharacter
end
...
function ClassSelect:open()
  ...
  local classType = -1
  local topClassType
  local isPremiumCharacter = FGlobal_getIsPremiumCharacter()
  local count = getPossibleClassCount()
  local newClassType = __eClassType_Corsair
  if true == ToClient_IsContentsGroupOpen("1600") then
    newClassType = __eClassType_Drakania
  end
  for classIdx = 0, count - 1 do
    classType = getPossibleClassTypeFromIndex(classIdx)
    if false == isPremiumCharacter and classType == newClassType and nil ~= self:getClassButtonData(classType) then
      self._ui.list_ClassSelect:getElementManager():pushKey(toInt64(0, classType))
  ...
  for classIdx = 0, count - 1 do
    classType = getPossibleClassTypeFromIndex(classIdx)
    if nil ~= self:getClassButtonData(classType) and (true == isPremiumCharacter or classType ~= newClassType) then
      self._ui.list_ClassSelect:getElementManager():pushKey(toInt64(0, classType))
      if nil == topClassType then
        topClassType = classType
      end
...
function ClassSelect:getClassButtonData(classType)
  ...
  elseif classType == __eClassType_Drakania then
    ...
    buttonData.className = PAGetString(Defines.StringSheet_GAME, "LUA_CLASSTYPE_7")
    buttonData.mainWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SLAYER")
    buttonData.subWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SHARD")
    buttonData.manaType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_MPTYPE_WP")
  elseif classType == __eClassType_Giant_Reserved0 then
    ...
    buttonData.className = "ì„ì‹œìºë¦­í„°5"
    buttonData.mainWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SLAYER")
    buttonData.subWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SHARD")
    buttonData.manaType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_MPTYPE_WP")
  elseif classType == __eClassType_Giant_Reserved2 then
    ...
    buttonData.className = "ì„ì‹œìºë¦­í„°6"
    buttonData.mainWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SLAYER")
    buttonData.subWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SHARD")
    buttonData.manaType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_MPTYPE_WP")
  elseif classType == __eClassType_KunoichiOld then
    ...
    buttonData.className = "ì„ì‹œìºë¦­í„°7"
    buttonData.mainWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SLAYER")
    buttonData.subWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SHARD")
    buttonData.manaType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_MPTYPE_WP")
  else
...
function PaGlobalFunc_ClassSelect_Init()
  ...
  if nil == self then
    _PA_ASSERT(false, "íŒ¨ë„ì´ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤!! : ClassSelect")
    return
  end
  self:init()
end
...
function PaGlobalFunc_ClassSelect_CharacterCreate(characterName, isNewRoute)
  local self = ClassSelect
  if nil == self then
    _PA_ASSERT(false, "íŒ¨ë„ì´ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤!! : ClassSelect")
    return
  ...
  local function createCharacterFunc()
    ...
    chracterCreate(self._currentSelectType, _edit_CharacterName, _isPremiumCharacter, isSeasonCharacter, isNewRoute)
...
<PAColor0xFFF26A6A>]] .. PAGetString(Defines.StringSheet_GAME, "LUA_MAKENAME_WARNING") .. "<PAOldColor>"
  ...
  if true == _ContentsGroup_SelectStartPosition and false == _isPremiumCharacter then
...
function InputMLUp_ClassSelect_SelectClassType(classType)
  ...
  if classType >= __eClassType_Count then
    _PA_ASSERT(false, "ì˜¬ë°”ë¥´ì§€ ì•Šì€ class type ì…ë‹ˆë‹¤!!")
    return
  end
  if false == ToClient_checkCreatePossibleClass(classType, _isPremiumCharacter) then
    _PA_ASSERT(false, "ì„ íƒí•  ìˆ˜ ì—†ëŠ” í´ë˜ìŠ¤ ì…ë‹ˆë‹¤!!")
    return
  end
  if classType == self._currentSelectType then
