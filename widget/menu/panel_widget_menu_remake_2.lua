function HandleEventLUp_MenuRemake_ReportBadMan()
  ...
  local actorKey = selfPlayer:getActorKey()
  local playerWrapper = getPlayerActor(actorKey)
  if nil == playerWrapper then
    return
  end
  local fame = playerWrapper:get():getFame()
  if fame < 0 then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_UNABLE_TO_REPORT2"))
    return
  end
  ...
  PaGlobal_Knave_Report:open()
end
function HandleEventLUp_MenuRemake_SheriffRank()
  if false == _ContentsGroup_Sheriff then
    return
  end
  PaGlobal_Cop_Rank_Open()
end
