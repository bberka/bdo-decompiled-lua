...
function varify()
  for type, index in pairs(Type) do
    ...
    _PA_ASSERT(func, "ë§ë©”ë‰´ì— ë©”ë‰´íƒ€ì… Function ì´ ì—†ìŠµë‹ˆë‹¤. type : " .. tostring(type))
    _PA_ASSERT(icon, "ë§ë©”ë‰´ì— ë©”ë‰´íƒ€ì… icon ì´ ì—†ìŠµë‹ˆë‹¤. type : " .. tostring(type))
    _PA_ASSERT(name, "ë§ë©”ë‰´ì— ë©”ë‰´íƒ€ì… name ì´ ì—†ìŠµë‹ˆë‹¤. type : " .. tostring(type))
  end
end
function FromClient_ConsoleQuickMenu_ExecuteFunctionType(datatype, functionType)
  ...
  if nil == executeFunc then
    _PA_LOG("í›„ì§„", "[ConsoleQuickMenu] QuickMenu.functionType ì— íƒ€ì…ì´ ì¶”ê°€ë˜ì—ˆìœ¼ë©´ PaGlobal_ConsoleQuickMenu._functionTypeList._ExecuteFunction ì— ì¶”ê°€í•´ ì£¼ì„¸ìš”  ")
    return
  end
  executeFunc()
end
