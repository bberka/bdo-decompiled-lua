  ...
  if 4 ~= targetActor:getCharacterGradeType() then
    if checkActiveCondition(targetActor:getCharacterKey()) == true then
      ...
      elseif nowHP > 10 then
        ...
        if true == isChina then
          animationName = "CN_Ani_Color_10"
        else
...
function panel_Update_Monster_Info(actorKey)
  ...
  if Defines.UIMode.eUIMode_NpcDialog ~= modecheck then
    if true == _ContentsGroup_UseBattleFocusUIModeVer2 then
      local isShowMonsterHP = ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eIsUseMonsterHPPanel)
      if true == isShowMonsterHP then
