...
local messageTexture = {
  ...
  [messageType.fantasyMix_Doom] = "New_UI_Common_forLua/Widget/NakMessage/Nak_9genHorse_00.dds",
    ...
    for ii = 1, enCommerceType.enCommerceType_Max - 1 do
      if "" == commerceMsg then
        ...
        if 0 < varyTradeInfo:getPercentByCommerceType(ii) then
          commerceMsg = _commerceType[ii] .. " <PAColor0xFFFFCE22>â–²" .. varyTradeInfo:getPercentByCommerceType(ii) / 10000 .. "<PAOldColor>%"
        ...
        elseif 0 > varyTradeInfo:getPercentByCommerceType(ii) then
          ...
          commerceMsg = _commerceType[ii] .. " <PAColor0xFFF26A6A>â–¼" .. varyTradeInfo:getPercentByCommerceType(ii) / 10000 .. "<PAOldColor>%"
      ...
      elseif 0 < varyTradeInfo:getPercentByCommerceType(ii) then
        commerceMsg = commerceMsg .. " | " .. _commerceType[ii] .. " <PAColor0xFFFFCE22>â–²" .. varyTradeInfo:getPercentByCommerceType(ii) / 10000 .. "<PAOldColor>%"
      ...
      elseif 0 > varyTradeInfo:getPercentByCommerceType(ii) then
        commerceMsg = commerceMsg .. " | " .. _commerceType[ii] .. " <PAColor0xFFF26A6A>â–¼" .. varyTradeInfo:getPercentByCommerceType(ii) / 10000 .. "<PAOldColor>%"
    ...
    if processIndex < curIndex then
      ...
      elseif messageType.combatArea == MessageData._Msg[processIndex].type then
        if true == _ContentsGroup_ChinaFontColor then
          _text_Msg:SetFontColor(UI_color.C_FFFFEDD4)
          _text_MsgSub:SetFontColor(UI_color.C_FFFFEDD4)
        else
...
function FromClient_SolareStartEndMessage(isSolareEnter)
  ...
  if isSolareEnter == true then
    ...
    msg.sub = PAGetString(Defines.StringSheet_GAME, "LUA_SOLARE_MATCHSTART_NAK_DESC_CLOSE")
...
function PaGlobal_RewardSelectNak_HidePanel()
  if nil == Panel_RewardSelect_NakMessage then
    _PA_ASSERT_NAME(false, "Panel_RewardSelect_NakMessage nil ì…ë‹ˆë‹¤.", "ê¹€ê·¼ìš°")
    return
  end
  local selfPlayer = getSelfPlayer()
  if nil == selfPlayer then
