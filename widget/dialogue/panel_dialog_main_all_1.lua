...
function PaGlobal_DialogMain_All:prepareOpen()
  ...
  if false == self._isDialogStarting and Panel_Interest_Knowledge_All ~= nil and nil ~= Panel_Dialog_Exchange_All and false == Panel_Dialog_Exchange_All:GetShow() then
...
function PaGlobal_DialogMain_All:prepareClose(isSetWait, isTest, isAttacked, calledByQuestBranch)
  ...
  if 1 == selfPlayer:getLevel() and 10 == currentRegionKeyRaw and nil == dialog_getTalker() and false == questList_isClearQuest(21117, 1) and false == questList_hasProgressQuest(21117, 1) and false == _ContentsGroup_NewSequenceTutorial then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_TUTIRIAL_DIALOGCLOSEALERT"))
    return
  elseif 1 == selfPlayer:getLevel() and 890 == currentRegionKeyRaw and nil == dialog_getTalker() and false == questList_isClearQuest(14001, 1) and false == questList_hasProgressQuest(14001, 1) then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_TUTIRIAL_DIALOGCLOSEALERT"))
    return
  elseif 1 == selfPlayer:getLevel() and 890 == currentRegionKeyRaw and nil == dialog_getTalker() and true == questList_isClearQuest(14001, 1) and false == questList_isClearQuest(14001, 2) and false == questList_hasProgressQuest(14001, 2) then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_TUTIRIAL_DIALOGCLOSEALERT"))
    return
  elseif 1 == selfPlayer:getLevel() and 10 == currentRegionKeyRaw and nil == dialog_getTalker() and false == questList_isClearQuest(21125, 1) and false == questList_hasProgressQuest(21125, 1) and true == _ContentsGroup_NewSequenceTutorial then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_TUTIRIAL_DIALOGCLOSEALERT"))
    return
  end
  ...
  if true == _ContentsGroup_MiniGame_YachtDice and PaGlobal_MiniGame_YachtDice ~= nil and true == Panel_Window_MiniGame_YachtDice:GetShow() then
    PaGlobal_MiniGame_YachtDice:prepareClose()
    return
  end
  if true == _ContentsGroup_OldMoonSmelting and nil ~= Panel_Window_OldMoonSmelting and true == Panel_Window_OldMoonSmelting:GetShow() then
  ...
  if nil == talker and true == _ContentsGroup_RemasterUI_Main_Alert and PaGlobal_Widget_Alert_BlackSpiritIcon_Effect ~= nil then
  ...
  if FGlobal_QuestWidget_UpdateList ~= nil then
...
function PaGlobal_DialogMain_All:funcBtnIconEffetc(index, funcButtonType)
  ...
  elseif funcButtonType == CppEnums.ContentsType.Contents_PetUpgrade then
    self._funcBtnList[index]:AddEffect("fUI_Dialog_01F", true, 0, 0)
  end
end
...
function PaGlobal_DialogMain_All:registEventHandler()
  ...
  registerEvent("FromClient_DialogFunctionClick_Contents_OldMoonSmelting", "FromClient_DialogFunctionClick_Contents_OldMoonSmelting")
  ...
  if true == self._isConsole then
    ...
    if _ContentsGroup_MasterlessHorse == true then
      Panel_Npc_Dialog_All:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandleEventLUp_HorseWithoutOwner_Summon(31560)")
    end
  else
