    ...
    if dataIndex < self._value._itemListSize then
      ...
      if nil ~= shopItemWrapper then
        ...
        if self._ENUMTABINDEXBUY == self._value._currentTabIndex then
          ...
          _PA_LOG("êµ¬ìˆ˜í˜„", "buyableMaxCount : " .. tostring(buyableMaxCount))
...
function PaGlobal_NPCShop_ALL:updateMoney()
  ...
  if true == isGuildFlag then
    ...
    if dialog_isEventWarehouseNpc() == true then
      local eventWarehouseMoney = warehouse_moneyFromRegionKey_s64(warehouse_getEventWarehouseRegionKey())
      self._ui._txt_Silver_Storage:SetText(makeDotMoney(eventWarehouseMoney))
      hasWareHouseFlag = true
    else
...
function PaGlobal_NPCShop_ALL:buySome()
  ...
  if self._ui._btn_Check_Warehouse:IsCheck() then
    if dialog_isEventWarehouseNpc() == true then
      money_s64 = warehouse_moneyFromRegionKey_s64(warehouse_getEventWarehouseRegionKey())
    else
