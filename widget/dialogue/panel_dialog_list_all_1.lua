...
function PaGlobal_DialogList_All:dialogTalkControlSet(control, key)
  if nil == key then
    _PA_ASSERT_NAME(false, "PaGlobal_DialogList_All:dialogTalkControlSet keyê°€ nilì…ë‹ˆë‹¤", "ì •ì§€í˜œ")
    return
  end
...
function PaGlobal_DialogList_All:dialogQuestControlSet(control, key)
  if nil == key then
    _PA_ASSERT_NAME(false, "PaGlobal_DialogList_All:dialogQuestControlSet keyê°€ nilì…ë‹ˆë‹¤", "ì •ì§€í˜œ")
    return
  end
  local index = Int64toInt32(key) - 1
  local dialogData = ToClient_GetCurrentDialogData()
