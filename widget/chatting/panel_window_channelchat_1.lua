...
function PaGlobal_ChannelChat:controlInit_All()
  ...
  for index = 0, self._serverListCount - 1 do
    ...
    if nil == baseTextureUV then
      _PA_ASSERT(false, "ì¡´ì¬ í•˜ì§€ ì•ŠëŠ” ëŒ€í™”ê·¸ë£¹ Textureì…ë‹ˆë‹¤")
...
function PaGlobal_ChannelChat:setTextureControl(control, stringHashKey)
  if nil == control then
    _PA_ASSERT(false, "ì¡´ì¬í•˜ì§€ ì•ŠëŠ” ì»¨íŠ¸ë¡¤ì´ë‹¤")
  end
  if nil == stringHashKey then
    _PA_ASSERT(false, "ìœ íš¨í•˜ì§€ ì•ŠëŠ” ìŠ¤íŠ¸ë§ í‚¤ë‹¤")
    stringHashKey = "LUA_CHANNELCHAT_ROOM_DESC_01"
  end
  local x1, y1, x2, y2 = 0, 0, 0, 0
  local baseTextureUV = self._bg_UV[stringHashKey]
  if nil == baseTextureUV then
    _PA_ASSERT(false, "ì¡´ì¬ í•˜ì§€ ì•ŠëŠ” ëŒ€í™”ê·¸ë£¹ Textureì…ë‹ˆë‹¤" .. tostring(stringHashKey))
  ...
  if "LUA_CHANNELCHAT_ROOM_DESC_29" == stringHashKey or "LUA_CHANNELCHAT_ROOM_DESC_30" == stringHashKey or "LUA_CHANNELCHAT_ROOM_DESC_31" == stringHashKey then
