...
function contentOpen:update(contentType)
  ...
  _PA_LOG("ë¬¸ì¥í™˜", "contentType = " .. tostring(contentType))
...
function contentOpen:open(contentType)
  if "" == contentOpen._image[contentType] or "" == contentOpen._title[contentType] or "" == contentOpen._desc[contentType] then
    _PA_LOG("ë¬¸ì¥í™˜", tostring(contentType) .. "ë²ˆ ì •ë³´ê°€ ì—†ìŠµë‹ˆë‹¤. í•´ë‹¹ ì´ë¯¸ì§€ì™€ stringì„ í™•ì¸í•´ì£¼ì„¸ìš”. string : " .. contentOpen._title[contentType] .. " / desc : " .. contentOpen._desc[contentType])
    return
  end
  _panel:SetShow(true)
