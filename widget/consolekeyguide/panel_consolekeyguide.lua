...
function PaGlobalFunc_ConsoleKeyGuide_Init()
  ...
  if nil == self then
    _PA_ASSERT(false, "íŒ¨ë„ì´ ì¡´ì¬í•˜ì§€ ì•ŠìŠµë‹ˆë‹¤!! : ConsoleKeyGuide")
    return
  end
  self:init()
end
  ...
  if vehicleProxy == nil then
    ...
    elseif CppEnums.VehicleType.Type_ThrowStone == vehicleType then
      ...
      elseif vehicleSubType == 1 then
        ...
        UI.ASSERT_NAME(false, "ë¹„ì •ìƒì…ë‹ˆë‹¤. ì‹ ê¸°ì „ì˜ vehicleSubTypeì´ 0ë„ ì•„ë‹ˆê³  1ë„ ì•„ë‹™ë‹ˆë‹¤.", "ì´ì£¼")
        guideState = nil
      end
    elseif CppEnums.VehicleType.Type_ThrowFire == vehicleType then
      local isUpgrade = 9088 == characterKey or 31073 == characterKey or 31074 == characterKey or 31075 == characterKey
