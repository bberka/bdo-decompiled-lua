...
function PaGlobalFunc_Customization_InputName_Confirm(str)
  ...
  if true == _ContentsGroup_SelectStartPosition and false == FGlobal_getIsPremiumCharacter() and nil ~= Panel_Window_StartBranch then
  ...
  PaGlobalFunc_ClassSelect_CharacterCreate(nameStr, false)
end
function PaGlobalFunc_Customization_InputName_SelectStartBranch(isNewRoute)
  local self = Customization_InputNameInfo
  if false == self._createSync then
    return
  end
  ...
  _AudioPostEvent_SystemUiForXBOX(50, 1)
  PaGlobalFunc_ClassSelect_CharacterCreate(nameStr, isNewRoute)
end
