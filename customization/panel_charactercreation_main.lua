...
if true == isDevelopment and nil ~= Button_ProfileScreenShot and nil ~= Button_ProfileScreenShot_Title then
  ...
  if nil == Button_RandFaceBoneOpen_DEV and nil ~= HandleClicked_RandomFaceBoneUI_Open_DEV then
    ...
    if nil == Button_RandFaceBoneOpen_DEV_Title then
      ...
      Button_RandFaceBoneOpen_DEV_Title:SetText("ëœë¤ ì–¼êµ´ìƒì„± ì—´ê¸°(Devìš©)")
    end
  ...
  if nil == Button_CaptureFaceBoneByBeautyAlbum_DEV and nil ~= HandleClicked_CreateFaceBoneForDev_CreateBeautyAlbum then
    ...
    if nil == Button_CaptureFaceBoneByBeautyAlbum_DEV_Title then
      ...
      Button_CaptureFaceBoneByBeautyAlbum_DEV_Title:SetText("ë·°í‹°ì•¨ë²” ì •ë³´ë¡œ ì´ˆìƒí™” ì´¬ì˜(Devìš©)")
    end
...
local mainText = {
  ...
  [25] = PAGetString(Defines.StringSheet_GAME, "LUA_CUSTOMIZATIONMAIN_ZODIAC")
}
...
function chracterCraeation_SeasonSetting()
  ...
  if nil == FGlobal_getIsPremiumCharacter or nil == PaGlobalFunc_CharacterSelect_All_CanMakeSeason then
    return
  end
  ...
  local canMakeSeason = true == PaGlobalFunc_CharacterSelect_All_CanMakeSeason() and false == FGlobal_getIsPremiumCharacter()
  ...
  btn_createCharacterButton:addInputEvent("Mouse_LUp", "Panel_CharacterCreateOK_NewCustomization()")
  btn_duplicate_season:addInputEvent("Mouse_LUp", "HandleClicked_DuplicateName()")
  btn_duplicationChk:addInputEvent("Mouse_LUp", "HandleClicked_DuplicateName()")
...
function PaGlobal_CheckCharacterCreationArea()
  if true == btn_duplicationChk:GetShow() then
    btn_duplicationChk:EraseAllEffect()
    btn_duplicationChk:AddEffect("fUI_InGame_Light_04A", false, 0, 0)
    txt_duplicationChkComment:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERCREATE_MAKEANAME"))
  end
end
...
function CustomizationMain_OpenSeasonBanner(isShow)
  ...
  if nil ~= FGlobal_getIsPremiumCharacter and true == FGlobal_getIsPremiumCharacter() then
    return
  end
...
function EventSelectClass()
  ...
  PaGlobalFunc_Customization_ResetUrlRandBeauty()
  changeCreateCharacterMode_SelectClass(FGlobal_getIsPremiumCharacter())
...
function HandleClicked_RandomBeauty()
  ...
  url = url .. "/customizing" .. tostring(userNo) .. "&userNickname=" .. tostring(userNickName) .. "&certKey=" .. tostring(cryptKey) .. "&classType=" .. tostring(classType) .. "&isCustomizationMode=" .. tostring(true) .. "&isGm=" .. tostring(isGm) .. "&isRandom=" .. tostring(true)
